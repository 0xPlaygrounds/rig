...
pub async fn create_quote_tweet(
    ...
    if let Some(media_files) = media_data {
        ...
        for (file_data, media_type) in media_files {
            let media_id = upload_media(file_data, &media_type, auth).await?;
...
pub async fn like_tweet(tweet_id: &str, auth: &dyn TwitterAuth) -> Result<Value> {
    let mut headers = HeaderMap::new();
    auth.install_headers(&mut headers).await?;
...
pub async fn retweet(tweet_id: &str, auth: &dyn TwitterAuth) -> Result<Value> {
    let mut headers = HeaderMap::new();
    auth.install_headers(&mut headers).await?;
...
pub async fn upload_media(
    ...
    if is_video {
        ...
        upload_video_in_chunks(file_data, media_type, headers, auth).await
    } else {
        // Handle image upload directly
        let form = reqwest::multipart::Form::new()
            .part("media", reqwest::multipart::Part::bytes(file_data));

        let (response, _) = request_multipart_api::<Value>(upload_url, headers, form).await?;
...
async fn upload_video_in_chunks(
    ...
    for chunk in file_data.chunks(chunk_size) {
        ...
        let (_, _) = request_multipart_api::<Value>(upload_url, headers.clone(), form).await?;
    ...
    if finalize_response.get("processing_info").is_some() {
        check_upload_status(&media_id, &headers).await?;
    }

    Ok(media_id)
}

async fn check_upload_status(media_id: &str, headers: &HeaderMap) -> Result<()> {
    let upload_url = "https://upload.twitter.com/1.1/media/upload.json";
...
pub async fn get_tweet(id: &str, auth: &dyn TwitterAuth) -> Result<Tweet> {
    let mut headers = HeaderMap::new();
    auth.install_headers(&mut headers).await?;
    let tweet_detail_request = Endpoints::tweet_detail(id);
    let url = tweet_detail_request.to_request_url();

    let (response, _) = request_api::<Value>(&url, headers, Method::GET, None::<Value>).await?;
...
pub async fn create_tweet_request(
    ...
    auth: &dyn TwitterAuth,
    reply_to: Option<&str>,
    media_data: Option<Vec<(Vec<u8>, String)>>, // (data, media_type)
) -> Result<Value> {
    let mut headers = HeaderMap::new();
    auth.install_headers(&mut headers).await?;
