...
impl<M> StreamingPromptHook<M> for AssertToolCallIdHook
where
    M: CompletionModel,
{
    async fn on_tool_call_delta(
        ...
        assert!(
            ...
            "expected tool_call_id to be non-empty"
        );
    }
...
async fn anthropic_streaming_tool_call_id() -> Result<(), anyhow::Error> {
...
async fn openai_streaming_tool_call_id() -> Result<(), anyhow::Error> {
    let _ = tracing_subscriber::fmt::try_init();
