        ...
        if response.status().is_success() {
            ...
            match serde_json::from_str::<ApiResponse<ZhipuAICompletionResponse>>(&t)? {
                ApiResponse::Ok(response) => {
                    ...
                    let content_text = response.choices[0].message.content.clone().unwrap_or_default();
                    let choice = OneOrMany::one(completion::message::AssistantContent::text(content_text));
