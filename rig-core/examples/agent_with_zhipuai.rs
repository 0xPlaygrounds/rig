...
use rig::providers::zhipuai::CompletionModel;
...
async fn basic_zhipuai() -> Result<(), anyhow::Error> {
    ...
    let response = poet_agent
        .prompt("Write a poem about the Yangtze River")
        .await?;
    println!("{}", response);
...
async fn context_zhipuai() -> Result<(), anyhow::Error> {
    let model = client().completion_model("glm-4-flashx");
    ...
    // Prompt the agent and print the response
    let response = agent
        .prompt("What are the customs of Qingming Festival?")
        .await?;
