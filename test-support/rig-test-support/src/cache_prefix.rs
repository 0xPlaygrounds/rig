...
pub type PrefixBlock = (&'static str, String);
...
pub fn canonical_prefix_blocks(path: &str, body: &Value) -> Option<Vec<PrefixBlock>> {
...
pub const NON_CONVERSATIONAL_ENDPOINTS: &[(&str, &str)] = &[
    (
...
pub fn classify_endpoint(path: &str) -> EndpointKind {
...
pub fn endpoint_is_modeled(path: &str) -> bool {
...
/// conversation.
pub fn continues_the_same_conversation(earlier: &[PrefixBlock], later: &[PrefixBlock]) -> bool {
...
/// The first mismatch between an earlier request and a continuation's prefix.
pub struct Violation {
    /// Scenario label supplied to the comparison.
    pub scenario: String,
    /// Request-pair index supplied by the caller.
    pub pair: usize,
    /// Prefix level of the mismatching earlier block.
    pub level: &'static str,
    /// Zero-based position of the mismatching or dropped block.
    pub block_index: usize,
    /// Truncated diagnostic representation of the earlier block.
    pub earlier: String,
    /// Truncated later block, or a marker indicating that it was dropped.
    pub later: String,
}
...
pub fn truncate(block: &str) -> String {
