...
//!    (`test-support/rig-test-support/src/cassettes.rs`, `matching_interaction_index`), so two
...
//! (`request_matches`, `test-support/rig-test-support/src/cassettes.rs`), so a rig change that
...
pub fn cache_padding(repetitions: usize) -> String {
...
pub struct CacheProbe {
    /// System instruction, including deterministic cache padding.
    pub preamble: String,
    /// Tool definitions in the stable order sent by each probe turn.
    pub tools: Vec<ToolDefinition>,
    /// Opening prompt used for the first two requests.
    pub prompt: &'static str,
    /// Continuation prompt that extends the cached prefix.
    pub follow_up: &'static str,
    /// Maximum output tokens requested for each turn.
    pub max_tokens: u64,
...
pub const CACHE_PROBE_PROMPT: &str =
...
pub const EMPTY_ASSISTANT_TURN_PLACEHOLDER: &str = "cache probe ready";

/// The follow-up that grows the prefix on turn 3.
pub const CACHE_PROBE_FOLLOW_UP: &str =
...
impl CacheProbe {
    ...
    pub fn with_padding(mut self, repetitions: usize, label: &str) -> Self {
...
pub struct CacheObservation {
    /// Reported usage for each completed probe turn, in request order.
    pub turns: Vec<Usage>,
}
...
impl CacheObservation {
    ...
    pub fn report(&self, support: &CacheSupport) -> String {
        self.turns
            .iter()
            .enumerate()
            .map(|(index, usage)| {
...
pub fn assert_warms(observation: &CacheObservation, support: &CacheSupport, context: &str) {
...
pub fn assert_hit_ratio(observation: &CacheObservation, support: &CacheSupport, context: &str) {
...
/// determines the bill.
pub fn assert_growth_still_hits(
...
pub fn assert_prefix_stable(provider: &str, scenario: &str) {
        ...
        let blocks = cache_prefix::canonical_prefix_blocks(request_path, &body).unwrap_or_else(|| {
            panic!(
                "[{provider}] {scenario}: request {index} speaks {request_path}, which the cache \
                 ...
                 test-support/rig-test-support/src/cache_prefix.rs."
            )
        });
...
pub fn assert_cache_key_stable(provider: &str, scenario: &str, support: &CacheSupport) {
...
pub struct CacheProbeLookupTool;

#[derive(Debug, thiserror::Error)]
#[error("cache probe lookup failed")]
/// Error type required by the deterministic cache lookup tool.
pub struct CacheProbeLookupError;

#[derive(serde::Deserialize)]
/// Arguments accepted by the cache probe's lookup tool.
pub struct CacheProbeLookupArgs {
    /// Topic echoed by the deterministic lookup response.
    pub topic: String,
}

impl rig_core::tool::Tool for CacheProbeLookupTool {
...
pub const AGENT_CACHE_PROMPT: &str = "\
...
pub fn observation_from_completion_calls(
...
pub fn assert_agent_growth_still_hits(
    observation: &CacheObservation,
    support: &CacheSupport,
    context: &str,
) {
...
pub fn assert_no_meaningful_prefix_cache(
    observation: &CacheObservation,
    support: &CacheSupport,
    context: &str,
) {
...
pub fn assert_cache_warms_over_turns(
    observation: &CacheObservation,
    support: &CacheSupport,
    context: &str,
) {
...
pub fn assert_cache_read_is_surfaced(
    observation: &CacheObservation,
    support: &CacheSupport,
    context: &str,
) {
...
///   is configured.
pub fn assert_breakpoints_match_support(provider: &str, scenario: &str, support: &CacheSupport) {
    let interactions = crate::cassettes::recorded_interaction_bodies(provider, scenario);
    assert!(
