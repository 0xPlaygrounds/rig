...
$crate::goldens::world_golden_test(async {
            {
                ...
                $wrapper($scenario, |client| async move {
                    let mut ecs = EcsAgent::new(rig::Model::new(client.completion(MODEL), rig::transport()), CONCISE_PREAMBLE, 1);
                    ecs.app
                        .world_mut()
...
$crate::goldens::world_golden_test(async {
            {
                ...
                let observed = probe.clone();
                $wrapper($scenario, |client| async move {
                    let mut ecs = EcsAgent::new(rig::Model::new(client.completion(MODEL), rig::transport()), TOOL_PREAMBLE, 1);
                    ecs.app
                        .world_mut()
                        .entity_mut(ecs.agent)
                        .insert((Temperature(Some(0.0)), MaxTokens(Some(ROOMY_CAP))));
