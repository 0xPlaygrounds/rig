...
```rust
// On native: requires Send
// On WASM: no requirement
pub trait WasmCompatSend: Send {}  // native
pub trait WasmCompatSend {}        // wasm

// On native: requires Sync  
// On WASM: no requirement
...
- `StreamingPromptHook` - For streaming requests (has additional callbacks for deltas)

They are separate because streaming requires callbacks that don't apply to non-streaming (`on_text_delta`, `on_tool_call_delta`, etc.).

### Hook Callbacks

- `on_completion_call` - Before sending prompt to model
- `on_completion_response` - After receiving response
- `on_tool_call` - Before invoking a tool (can reject with `ToolCallHookAction::Skip`)
- `on_tool_result` - After tool returns result

### Control Flow

- `ToolCallHookAction::Continue` - Allow tool execution
- `ToolCallHookAction::Skip { reason }` - Reject tool; reason becomes tool result
- `CancelSignal::cancel()` - Terminate the agentic loop early
...
- [ ] All error cases handled properly (no `.unwrap()` on fallible operations)
