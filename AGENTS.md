...
Finish implementation, examples, tests, migration notes, and scope review first. Inspect the full intended diff against its actual merge base, including staged, unstaged, and relevant untracked files. Run formatting, targeted tests, and cheap checks, then obtain independent full-diff review and fix confirmed findings before expensive final verification. Freeze verification inputs for the final plan; keep progress reports outside the repository. This does not require committing unfinished work. After a late fix, rerun affected checks and final review; reuse only matching successes under the existing policy and report every mandatory fresh check still outstanding. Validate findings, fix confirmed P0/P1 and in-scope issues, then rerun affected checks and review. After publishing, inspect committed-head required CI and actionable review threads. Do not declare completion while checks are pending/failing or required work remains. Report exact unrelated failures or missing prerequisites without broadening scope. These requirements do not independently authorize commits, pushes, or PR creation.
