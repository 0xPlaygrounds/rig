...
> **Historical migration plan.** This document maps baseline
> `87f3f5b77a3caeffa10d60225c41e386753bf05e`; its target was implemented in
> `940483b4fb30aa77d83dab52a1599652cb9e0c2a`. “Current” below means current at
> the baseline, and baseline source links are commit-pinned.

This document maps the source revision
`87f3f5b77a3caeffa10d60225c41e386753bf05e`. “Proposed owner” describes the
target boundary adopted by the implementation. Built-in provider movement is
explicitly staged outside the initial runtime extraction.
...
| `Tool`, `ToolEmbedding`, `DynamicTool` metadata, `ToolDefinition` | `rig-core::tool` | narrow authoring contracts in `rig-core` | Split/redesign | WASM-compatible portable bounds | Must be usable by both runtimes. Do not accept `World` or ECS IDs. The implementation uses context-free `Tool` plus classic `ContextualTool`. |
...
## Completion audit applied to the implementation

The split was evaluated against the following criteria, not merely whether the
crates compiled:
