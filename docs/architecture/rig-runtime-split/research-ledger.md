...
| F-07 | `rig-core` declares 25 public top-level modules at this revision (including feature-gated modules and `telemetry`). | [`rig-core/src/lib.rs:152-198`](../../../crates/rig-core/src/lib.rs#L152) |
| F-08 | The root `rig` facade glob re-exports `rig_core` and exposes 18 companion modules behind features (memory is always a core module with optional companion additions). | [`src/lib.rs:30-158`](../../../src/lib.rs#L30), [`Cargo.toml:254-311`](../../../Cargo.toml#L254) |
...
| F-10 | `CompletionClient` imports classic `AgentBuilder` and `ExtractorBuilder`; its `agent()` and `extractor()` methods return those builders. | [`client/completion.rs:1-60`](../../../crates/rig-core/src/client/completion.rs#L1) |
| F-11 | Current main defines `Prompt`, `Chat`, and `TypedPrompt`, but no public `Completion` facade trait; `CompletionModel` is the low-level provider trait. | [`completion/request.rs:357-631`](../../../crates/rig-core/src/completion/request.rs#L357), repository symbol search |
| F-12 | `PromptError` contains completion, memory, max-turn, cancellation, and unknown-tool variants. | [`completion/request.rs:140-184`](../../../crates/rig-core/src/completion/request.rs#L140) |
| F-13 | `CompletionResponse<T>` stores canonical choice/usage and a typed raw provider response. | [`completion/request.rs:448-458`](../../../crates/rig-core/src/completion/request.rs#L448) |
| F-14 | `streaming.rs` contains both raw provider streaming types and high-level traits returning classic runtime requests. | [`streaming.rs:67-261`](../../../crates/rig-core/src/streaming.rs#L67), [`streaming.rs:565-626`](../../../crates/rig-core/src/streaming.rs#L565) |
| F-15 | `AgentRun` is serializable sans-I/O state; `drive_agent` is shared by blocking and streaming surfaces. | [`agent/run/mod.rs:277-317`](../../../crates/rig-core/src/agent/run/mod.rs#L277), [`prompt_request/streaming.rs:471-489`](../../../crates/rig-core/src/agent/prompt_request/streaming.rs#L471) |
| F-16 | `AgentHook` has event-specific action types. `HookStack` merges completion patches, chains tool rewrites/results, and short-circuits terminal actions. | [`agent/hook.rs:726-1031`](../../../crates/rig-core/src/agent/hook.rs#L726), [`agent/hook.rs:1251-1377`](../../../crates/rig-core/src/agent/hook.rs#L1251) |
| F-17 | PR #2182 adds model-turn retry/stop actions; retry is documented as tool-free and within the total call budget. | [`agent/hook.rs:427-478`](../../../crates/rig-core/src/agent/hook.rs#L427), [`agent/hook.rs:940-949`](../../../crates/rig-core/src/agent/hook.rs#L940) |
| F-18 | Current `Tool` uses `WasmCompatSend`/`WasmCompatSync`, accepts `&mut ToolContext`, and is distinct from `ToolSet` and tool server types in the same module tree. | [`tool/mod.rs:133-180`](../../../crates/rig-core/src/tool/mod.rs#L133), [`tool/mod.rs:511`](../../../crates/rig-core/src/tool/mod.rs#L511), [`tool/server.rs:126-230`](../../../crates/rig-core/src/tool/server.rs#L126) |
| F-19 | `ConversationMemory` is a WASM-compatible backend contract over canonical messages. | [`memory.rs:85-117`](../../../crates/rig-core/src/memory.rs#L85) |
| F-20 | `Extractor` stores an `Agent`, uses its runner/output tool mode, and exposes hook registration. | [`extractor.rs:37-79`](../../../crates/rig-core/src/extractor.rs#L37), [`extractor.rs:199-335`](../../../crates/rig-core/src/extractor.rs#L199) |
| F-21 | `CompletionSpanBuilder` checks for the classic target `rig::agent_chat`; `ProviderResponseExt` is otherwise provider metadata. | [`telemetry/mod.rs:66-156`](../../../crates/rig-core/src/telemetry/mod.rs#L66), [`telemetry/mod.rs:426-461`](../../../crates/rig-core/src/telemetry/mod.rs#L426) |
| F-22 | `rig-derive` resolves `rig-core` then `rig` and emits `rig_core::tool::Tool` plus `ToolContext` paths for `rig_tool`. | [`rig-derive/src/lib.rs:22-34`](../../../crates/rig-derive/src/lib.rs#L22), [`rig-derive/src/lib.rs:691-735`](../../../crates/rig-derive/src/lib.rs#L691) |
...
| F-32 | OpenAI defines an inherent `GenericCompletionModel::into_agent_builder()` that returns classic `AgentBuilder` directly from provider code. | [`openai/completion/mod.rs:1898-1901`](../../../crates/rig-core/src/providers/openai/completion/mod.rs#L1898) |
...
| ID | Question | Required decision/evidence |
| --- | --- | --- |
