...
- *(core)* Restore production custom HTTP transport injection without
  reintroducing provider generics: `HttpTransport` is erased once into
  `HttpRuntime::from_transport`, non-success buffered/multipart/streaming
  responses preserve status and body at the adapter boundary, unsupported
  multipart has a typed capability error, and `ClientWithMiddleware` works
  through the existing `reqwest-middleware` feature and example.
- [**breaking**] *(agent)* high-level out-of-tree completion providers are restored without generic agents: `ProviderConfig::External` stores exact-version identity/model/settings as honest serde data, while a host-owned `Runtime` registry stores concrete `ExternalCompletionProviderEntry` records. Typed authors implement the non-object `ExternalCompletionProvider` contract and erase it with `from_provider`; closure constructors, including `with_state`, also accept ordinary futures and box only inside private dispatch callbacks. Unary and streaming fulfillment, sessions, tools, hooks, extraction, cancellation, and resume then use the ordinary agent drivers. External capability descriptors live only on registered handlers, and `ProviderConfig::descriptor` becomes runtime-aware.
- [**breaking**] *(core)* `CompletionStream` now has typed `Running`/`Exhausted`/`Cancelled`/`Failed` lifecycle semantics. Provider errors are terminal and yielded once, cancellation is never inferred from error text, paused reads park on an `AtomicWaker` and can be resumed through `CompletionStreamPauseHandle`, and metadata-only events are consumed iteratively. `From<CompletionStream> for CompletionResponse` is removed: drain to natural EOF, verify `termination() == Some(Exhausted)`, then call checked `into_response()`; running, cancelled, and failed aggregates remain provisional
- [**breaking**] *(agent)* model calls are transactional across `AgentSession` and `AgentStream`. Preparation/open/provider/midstream failures, async cancellation, and `close_turn` restore the exact one-turn `RequestPatch` and logical turn position while discarding provisional history, tool calls, usage, and terminal metadata. Every emitted `CallModel` authorization remains charged to `max_turns`, including local preparation failures and provider-operation reissues, so repeatedly polling a raw session or stream after errors is finite. A reissue keeps the same logical turn number but receives a fresh attempt identity and consumes another budget unit; an already-answered `BeforeModelCall` decision is not redispatched, so append-only patch fields are not duplicated.
- [**breaking**] *(agent)* provider attempts own their real prompt plus the effective Tool-mode `{ output_tool_name, effective_schema }` contract until commit. `AgentRun::with_output_validation` now accepts `Option<schemars::Schema>` instead of an untyped JSON value, and malformed scalar/array schemas fail at deserialization rather than silently dropping Tool mode. `AgentRunStep::CallModel` exposes an opaque `ModelAttemptId`; hand-driven callers pass it to `prepare_request` (along with `AgentRun::inherited_output_contract()` on corrective calls), direct `ModelTurn`/`StreamedTurnAssembler` construction, and streamed completion recording. Stale or replayed unary turns, streamed partial/final turns, and completion records are rejected without poisoning the current reissue. The single-use `PreparedModelAttempt` receipt is no longer `Clone`; unary callers consume it with `into_model_turn`, while streaming callers consume it with `into_streamed_turn_assembler`, which carries the exact prompt/schema/identity and validation name sets through partial invalid-call recovery and final assembly. Corrective Retry/Skip transitions preserve the violated contract atomically without a later private-driver repair. `ModelTurnOutcome` is `#[must_use]`, and hand-driven callers must match `Continue`, `NeedsResolution`, and `TurnRetried` before advancing.
- [**breaking**] *(agent)* streaming now enforces one terminal-record invariant in normal ingestion and invalid-call recovery: every provider item after the first `Final` is rejected. Cumulative text aggregation exists only when a hook opted into delta events, avoiding a duplicate full-response buffer on the common no-observer path. Stream deltas remain observable but provisional until natural EOF and may repeat after retry; invalid-tool Retry/Skip likewise commits only after checked EOF. `AgentStreamItem::TurnFinished` now carries its own `message_id`, and a successful EOF-only turn clears `last_response()` instead of exposing a prior turn's terminal record. `AgentRun` represents “verdict required” and “ready to advance” as distinct serializable states, and `AgentSession::advance` exhaustively handles every private pending state.
- [**breaking**] *(agent)* tool execution uses one ordered `ToolExecutionRecord` per invocation (`batch_index` plus Rig `internal_call_id`, original/effective call, structured result, model-visible result, invocation disposition, and optional span). `ToolBatchOutput` exposes `records`, visible result projections, and identity-bearing `submissions()`/`into_submissions()` instead of parallel result/raw-result/span vectors. Manual `AgentSession`/`AgentStream` result submission likewise uses `ToolResultSubmission { internal_call_id, result, disposition }`, so duplicate provider call IDs and reverse completion/submission order remain correctly paired; submissions validate both provider `id` and `call_id`, while the provider-ID-only `AgentRun::tool_results` convenience rejects ambiguous duplicate-ID batches. Identity joins are committed in the pending calls' model-emitted source order, not host submission order. This wire-visible ordering change affects manual hosts that call `provide_tool_results` out of order; Rig's executor already returns source-ordered records. Rig IDs are generated in durable `AgentRun` state before a tool batch surfaces, and serialized pending calls retain original/effective calls plus positional pre-resolved results and typed invocation disposition. Unary and streamed invalid-call repairs retain the provider-emitted call as `original_call` and the repaired invocation as `effective_call`. `ToolResultReady` now exposes `internal_call_id`; `ResolvedToolCall` carries the complete effective call with a normalized `ResolvedToolCallDisposition`, and hook event projections are derived from the same `ToolCallResolution` accumulator, so rewrite-then-skip and invalid-peer suppression cannot lose rewritten arguments, call identity, or classification. `ToolExecutionCommitted` is emitted only when a local body ran or the host explicitly supplied external execution. Unknown tools are rejected during preflight: they have a durable model-visible not-found result but no `execute_tool` span, no execution observation, and do not contribute to `last_span_id`. A record's `raw_result` is authoritative for live executor and policy-skip outcomes; host-supplied or serialized pre-resolved content without a live record is reconstructed lossily as success, while `ToolInvocationDisposition` remains the authoritative execution-provenance field
- [**breaking**] *(core, agent)* stored concurrency and invocation concurrency are separate: `PortableTool`, dynamic-tool callbacks, hook callbacks/state, and internal HTTP backends are now `Send + Sync` on every target, while invocation-local tool types, futures, and streams retain target-relaxed bounds on `wasm32-unknown-unknown`. Browser-wasm code that retained `Rc`, `RefCell`, or JavaScript-affine handles in a hook/tool value must acquire them inside the returned future, use `thread_local!`, or move ordinary Rust state to `Arc<Mutex<_>>`/`Arc<RwLock<_>>`. On browser wasm, `ToolExecutionError::from_error` keeps diagnostics but drops every non-envelope concrete source; `from_send_sync_error` and `with_source` explicitly retain shareable sources on every target
- [**breaking**] *(core, agent)* `PortableDynamicTool::new`, `HookEntry::new`, and `HookEntry::with_state` accept ordinary async closures and perform future boxing only inside their private `Arc<dyn Fn + Send + Sync>` callback fields. `with_state` moves an `Arc<S>` into each invocation future instead of exposing a borrowed-state boxed-future signature. `Agent::into_tool(name, description)` is restored solely as a data-to-data conversion into a strict one-field `PortableDynamicTool` record; the removed classic `DynamicTool`/`ToolContext`/server architecture and implicit `From<Agent>` conversion remain gone
- *(core, agent)* fluent builders are restored for the surfaces that construct concrete owned data, without reintroducing provider/model/transport generics: `CompletionRequest::builder(prompt)` returns a **non-generic** `CompletionRequestBuilder`; concrete provider clients expose `agent(model)` and `completion_model(model)` through the prelude; and the concrete `BoundCompletionRequest` mirrors the plain request setters before terminating in `.build()`, `.send()`, or `.stream()`. Customized provider configs bind once through `BindCompletionExt`; `Config + functions::{complete, open_stream}` remains the low-level path
- *(providers)* completion-capable `ProviderConfig` arms expose monomorphic provider `Client`/`ClientBuilder` connection handles. Clients own one canonical connection record plus a reusable concrete runtime, materialize operation configs for a supplied model, preserve explicit OpenAI Responses/Chat Completions and Gemini generateContent/Interactions selectors, and redact credentials in debug output. Bedrock and Gemini gRPC retain and seed their caller-supplied live transports
- [**breaking**] *(agent, providers)* one compile-time provider-capability registry now generates the closed `ProviderConfig`/`EmbedderConfig` vocabularies, descriptor/model accessors, `From` conversions, and completion/streaming/embedding dispatch. Bedrock and Gemini gRPC lightweight configs/descriptors live unconditionally in `rig-core` (their companion crates re-export the same types), so Cargo features no longer mutate public enum or serde shape; disabled transports fail at fulfillment. `EmbedderConfig` adds Llamafile, Mistral, OpenRouter, and Together, while FastEmbed remains an explicit loaded-weights exception. The deterministic `Mock` variants are always present, production-visible serde variants that exhaustive matches must handle; hosts accepting provider configuration across an untrusted boundary must validate or allowlist variants and fields before fulfillment
- [**breaking**] *(providers)* provider extension parameters now pass through one collision-aware request boundary: non-object payloads and attempts to replace canonical request fields return typed request errors instead of inheriting serde-flatten or merge precedence. Nested objects merge at leaf ownership boundaries, provider defaults remain overridable, and provider-native fields that deliberately share a wire name are parsed into typed data first, including Anthropic output configuration, Groq native tools, and Gemini safety/image settings. Generic OpenRouter `max_tokens`, xAI audio speed, and supported Gemini/xAI image dimensions now reach their provider wire fields; unsupported image ratios require an explicit provider-native ratio instead of silently falling back
- *(providers)* VoyageAI embedding and rerank configs now reuse the secret-safe `HttpConnectionConfig`; inline API keys and extra-header values remain faithful through serde and request construction but are redacted in both inner and outer `Debug` output. ChatGPT and Copilot auth contexts likewise redact live access/refresh tokens
- [**breaking**] *(agent)* `agent.extractor(prompt)` returns a concrete `ExtractionRunner` holding agent config, provider config, runtime, prompt, and `ExtractOptions`, defaulted to the byte-compatible `ExtractOptions::classic_extractor()` protocol. The redundant, order-dependent `.classic()` setter is removed; existing calls should delete it, while fluent setters (`retries`, `history`, `preamble`, `context(s)`, `additional_params`, `max_tokens`, `temperature`, `tool_choice`, `max_turns`, output-tool configuration) override the preset field by field. The runner is **not** generic over the extracted type — it is selected at the terminal `.run::<T>()` / `.run_with_usage::<T>()`, so there is no `ExtractionRunner<T>` and no `PhantomData<T>`
- *(core)* `EmbeddingJob` accumulates documents, batch size, and concurrency as data and accepts the provider closure only at `.run(..)`/`.run_with_usage(..)`; it stores no embedding model or callback. `EmbeddingJob::for_provider(&DESCRIPTOR)` adopts a provider's declared per-request document limit. `embed_documents`/`default_concurrency` are unchanged
- [**breaking**] *(agent)* `ToolExecutorBuilder` is removed: the concrete `ToolExecutor` is its own fluent data surface, with `.tool(..)` for immediate typed erasure, `.register(..)` for one dynamic record, and `.register_all(..)` for an ordered batch; delete `ToolExecutor::builder()` and the no-op `.build()` terminator. `ToolExecutor::default()` now delegates to `new()`, so its stored concurrency consistently starts at the documented sequential value of 1
- [**breaking**] *(core, agent)* completion-stream ownership now follows the stream lifecycle: live `CompletionStream` inspection remains allocation-free through borrowed `choice()`/`final_record()`/`message_id()` accessors; `StreamedTurnAssembler::partial_turn` borrows `Option<&str>` and copies the ID only into its owned rollback snapshot; and `finish` consumes its final `OneOrMany<AssistantContent>` plus committed `Usage`. At EOF, finalize the exhausted stream with checked `into_response()` and move `message_id`/`choice`/`usage` into completed-turn assembly
- [**breaking**] *(core)* tool discovery is owned data: the `PortableToolEmbedding` trait (with its `InitError`/`Context`/`State` associated types and `init`) is deleted in favour of building `rig::embeddings::ToolSchema` records directly — `ToolSchema::new(name, embedding_docs)`, plus `with_context`/`try_with_context`; `ToolSchema::try_from(&tool)` is gone. Nothing called `init`, so no reconstruction path is lost; `context` remains as an owned, uninterpreted slot for hosts keeping their own registry keyed by tool name. Dynamic tool selection is unchanged — register tools as executable records, embed the discovery batch, and narrow the advertised set per turn with `RequestPatch::active_tools`
- [**breaking**] *(stores)* the portable filter is concrete: `Filter<V>` becomes `Filter`, whose operands are `serde_json::Value` (every instantiation in the workspace already was `Filter<serde_json::Value>`). Constructors now take `impl Into<serde_json::Value>`, so `Filter::eq("k", "v")` works without a `json!` wrapper. The serde wire format is unchanged, and `VectorSearchRequest<F>` keeps its parameter so backend-native filters with richer operators still reach a query
- [**breaking**] *(core)* the SSE event source drops its retry type parameter: `GenericEventSource<HttpClient, Retry>` becomes `GenericEventSource<HttpClient>` and the `RetryPolicy` trait is deleted. `ExponentialBackoff` is now plain configuration with inherent `retry`/`set_reconnection_time`. There was one implementation and the `Stream` impl only covered the default specialization, so the parameter could never vary; reconnection timing, backoff clamping, and retry limits are unchanged
- [**breaking**] *(rig-mongodb, rig-neo4j)* generic wrappers that were already erased internally are concrete: `MongoDbVectorIndex<C>` becomes `MongoDbVectorIndex` storing a `Collection<bson::Document>` (`new` still accepts any typed collection and converts on the way in), and Neo4j's `RowResultNode<T>` is a private concrete wire record
...
- *(anthropic)* `functions::Config` gains `prompt_caching`, `automatic_caching` and `automatic_caching_ttl` (with `with_prompt_caching`/`with_automatic_caching`/`with_automatic_caching_1h`), restoring the deleted `CompletionModel` builders. The caching machinery shipped in 0.41 with no way to enable it, so every request was built with caching off. Defaults unchanged, so recorded bodies are identical
- *(openai)* `functions::EmbeddingConfig::ndims()` reports the configured model's vector width (explicit `dimensions` override, else the native width from the per-model table), replacing seven hardcoded `EMBEDDING_DIMS = 1536` constants in the store examples. The `dimensions` request field is unchanged and stays opt-in
- [**breaking**] *(core)* the provider trait stack is inverted onto the `functions` modules: `OpenAICompatibleProvider`'s five-stage hook pipeline (`build_completion_request`/`prepare_request`/`finalize_request_body`/`finalize_request_body_with_options`/`decorate_streaming_tool_call`) and its capability consts (`PROVIDER_NAME`, `SUPPORTS_TOOLS`, `SUPPORTS_RESPONSE_FORMAT`, `STREAM_INCLUDE_USAGE`, `EMITS_COMPLETE_SINGLE_CHUNK_TOOL_CALLS`) and `StreamingUsage` assoc type are deleted — every dialect quirk is now straight-line code in that provider's own `functions::build_body`, parameterized by its `ProviderDescriptor`; `AnthropicCompatibleProvider::{PROVIDER_NAME, default_max_tokens}` collapse into one plain-data `AnthropicDialect`
- [**breaking**] *(core)* stream parsing is sans-IO: `CompatibleStreamProfile`, `OpenAICompatibleProfile`, and `openai::send_compatible_streaming_request` are deleted in favour of pure per-dialect chunk normalizers (`&str -> NormalizedCompatibleChunk`) selected by a concrete `ChunkNormalizer` enum, driven by one state machine over the type-erased `http_client::sse::BoxedEventSource`; provider stream drivers are no longer generic over `HttpClientExt`, and the new `providers::descriptor::{ChatCompletionsDialect, ChatCompletionsUsageDialect}` carry the dialect data
- [**breaking**] *(core)* stream erasure is no longer exposed by the raw Gemini Interactions API: the public `InteractionEventStream = Pin<Box<dyn Stream<...>>>` alias becomes a concrete live handle with private transport state and an inherent pin-free `next()` method. The boxed byte and SSE transport aliases are crate-private implementation details

### Other

- [**breaking**] *(core)* non-completion request typestate builders are gone: `VectorSearchRequestBuilder`, `TranscriptionRequestBuilder`, `ImageGenerationRequestBuilder`, `AudioGenerationRequestBuilder` and the trait methods that returned them (`transcription_request`/`image_generation_request`/`audio_generation_request`) are deleted in favour of plain constructors plus `with_*` setters — `VectorSearchRequest::new(query, samples)`, `TranscriptionRequest::new(data)` (`from_file` replaces `load_file`), `ImageGenerationRequest::new(prompt)`, `AudioGenerationRequest::new(text, voice)`; build the request and call the provider's `functions::*` free function; `markers::Provided<T>` is removed
- [**breaking**] *(agent)* one driver: `AgentRunner`, `StreamingPromptRequest`, `MultiTurnStreamItem`, `StreamingResult`, and `StreamingError` are deleted along with the second agent engine — `AgentSession`/`AgentStream` are the only drivers, `Agent` is a thin record over them (the R1 `SessionAgent` merged into it), and `agent.runner(p)` now returns `SessionRunner` with the same setters. The host-driven `AgentStream` yields the exhaustive decision-and-observation protocol as `AgentStreamItem`; `agent.stream_run(p)` / `agent.runner(p)….stream_run()` yields the distinct observation-only `AgentRunItem`, so driven consumers cannot receive or match unreachable host decisions. Both surfaces use `PromptError` as the error type
...
- [**breaking**] *(agent)* generic `Extractor`/`ExtractorBuilder`/`client.extractor::<T>()`/`ExtractionResponse` are removed. `agent.extractor(prompt)` returns a concrete `ExtractionRunner` and selects `T` only at `.run::<T>()`/`.run_with_usage::<T>()`; `rig::extract::{extract_with_options, ExtractOptions, ExtractionOutcome}` remains the low-level form, and `ExtractOptions::classic_extractor()` reproduces the old request shape byte-for-byte

### Other

- [**breaking**] *(agent)* hooks are concrete records: `AgentHook`/`HookStack`/`HookContext`/`Scratchpad`/`StepEventKind` removed in favour of `rig::hooks::{Hooks, HookEntry, HookEvent, HookDecision}` — `.add_hook()` now takes a `HookEntry`; the decision vocabulary at `rig::agent::hook` is unchanged; delta observation requires `.observing_deltas()`; run identity and the run-scoped scratchpad are host-owned closure state; tool-call argument rewrites chain as `serde_json::Value`. `HookEvent` remains owned across await points while immutable payloads use shallow-cloned `Arc` handles built once per dispatch, including streamed delta snapshots; effective tool-call/result projections allocate only on actual rewrites. Event `Debug` is content-free, and a wrong-lane non-`Continue` decision remains "no opinion" while emitting a diagnostic containing only the hook name and event/decision kinds
...
- [**breaking**] *(agent)* the classic tool system collapsed onto portable records: `ToolContext`/`ToolSet`/`ToolServer`/`DynamicTool`/`ToolEmbedding` and the old zero-argument `Agent::into_tool()` adapter were removed; `Tool` is the portable contract (context parameter dropped); tool execution rides `ToolExecutor`; the `IntoToolOutput` blanket impl (and its `Any` dispatch) was replaced by explicit impls + `serialize_to_tool_output`; `#[rig_tool]` gains `.portable()`; MCP integration is `rig::tool::mcp` (the `rmcp` feature now aliases `mcp`). A new explicit `Agent::into_tool(name, description)` now creates only a concrete portable record
...
- *(core,agent)* [**breaking**] concrete completion/streaming payloads: `CompletionResponse<T>` is now the concrete `CompletionResponse` (no `raw_response` — call the provider's own conversion for wire-typed data); the provider-typed streaming final is now the normalized `StreamFinal` (usage is a field; the `GetTokenUsage` trait is removed); `RawStreamingChoice`/`CompletionStream`/`StreamedAssistantContent`/`MultiTurnStreamItem` lose their type parameters, while the public `StreamingResult = Pin<Box<dyn Stream<...>>>` alias is removed entirely. The live handle formerly named `StreamingCompletionResponse` is now `CompletionStream`: providers pass a concrete stream to `CompletionStream::from_stream`, which owns pinning and erasure privately, and callers read its private aggregate state through `choice()`, `final_record()`, and `message_id()`. Both `CompletionStream` and `AgentRunStream` now have inherent `.next().await` methods for the pin-free default loop without a `StreamExt` import while retaining `Stream` implementations for opt-in combinators; `CompletionModel` loses `type Response`/`type StreamingResponse`; `StreamingPrompt`/`StreamingChat` lose their response parameter; provider-specific streaming-final aliases (deepseek, groq, mistral, openrouter, gemini, copilot) are removed. The unrelated, unused `openai::responses_api::streaming::StreamingCompletionResponse` wire DTO is also removed; Responses stream events already carry `responses_api::CompletionResponse`
- *(agent)* [**breaking**] `Agent<M>` is now `Agent`: every classic runtime type loses its `M: CompletionModel` parameter (`AgentBuilder`, `AgentRunner`, `PromptRequest<S>`, `StreamingPromptRequest`, `Extractor<T>`, `ExtractorBuilder<T>`, `StreamingPrompt`/`StreamingChat`); agents hold a `ProviderConfig` plus an `Arc<Runtime>` instead of a model, `AgentBuilder::new` takes a `ProviderConfig`, and the new `rig::client::ToProviderConfig` trait (in the prelude) captures a client's connection details as configuration; `AgentModelExt`/`into_agent_builder` are removed, `rig-candle` and `rig-vertexai` no longer construct classic agents (drive the sans-IO protocol directly), and agent mocking moves from `MockCompletionModel` to the scripted `MockScript` provider
- *(core)* [**breaking**] generic model-bound request builders are removed. `CompletionRequest` stays plain data and has a concrete `CompletionRequestBuilder`; the root facade can bind that builder to a concrete provider/runtime handle for `.send()` and `.stream()` without restoring `CompletionModel`
- *(core)* [**breaking**] modalities as free functions: embedding, transcription, image-generation, audio-generation, and rerank calls are extracted to per-provider free functions over plain configs (each provider's `functions` module, e.g. free `embed`/`embed_batches`). The old `EmbeddingsBuilder`/`EmbeddingModel` surface is removed; completion clients that support another modality expose config factories which preserve their connection data
- *(core)* [**breaking**] vector stores are de-genericized: `VectorStoreIndex`/`VectorStoreIndexDyn`/`InsertDocuments` are deleted with no shared trait replacing them; every store crate exposes concrete inherent `top_n`/`top_n_ids`/`top_n_as`/`insert`/`insert_as` over a pre-embedded vocabulary (`VectorSearchRequest` carries `OneOrMany<Embedding>`, results are `SearchHit`, inserts take `StoreRecord`); store constructors no longer take an `EmbeddingModel`
- *(agent)* [**breaking**] `AgentBuilder::dynamic_context`/`ExtractorBuilder::dynamic_context`/`AgentBuilder::retrieved_tools` are removed again: passive RAG is a documented `AgentHook` recipe (embed → store `top_n` → `RequestPatch::extra_context`; see the `rig::agent` module docs and `examples/rag`), and dynamic tool retrieval is per-turn `RequestPatch::active_tools`
