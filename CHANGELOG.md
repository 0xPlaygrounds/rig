...
- *(rig)* the published `rig` crate now ships only its source, README, licence and changelogs. The root manifest is both the workspace root and the facade package, so its package directory is the whole repository and cargo shipped everything not gitignored: 0.41.0 packaged 1705 files and 9.51 MiB compressed against crates.io's 10 MiB hard cap, 27.3 MiB of it recorded test cassettes, leaving 4.9% of headroom after growing 1.54 MiB in one cycle — 0.43.0 would have failed to publish. It also swept up whatever untracked files happened to be in a maintainer's working tree, so what shipped depended on local state. `[package].include` is an allowlist rather than an `exclude` denylist deliberately: a denylist cannot name a file that does not exist yet. The facade is now 0.507 MiB. Consumers building from the registry tarball no longer get the integration test suite ([#2349](https://github.com/0xPlaygrounds/rig/issues/2349))
