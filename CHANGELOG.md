...
- *(agent)* telemetry: `AgentBuilder::record_message_telemetry(true)` opts an agent into recording serialized per-turn `gen_ai.input.messages` / `gen_ai.output.messages` on agent chat spans as JSON-serialized Rig-normalized `completion::Message` values after preamble/document normalization, not provider-native wire payloads. The default remains disabled; enabling it can export prompts, history, documents, tool arguments/results, reasoning, multimodal payloads, and other sensitive high-cardinality content.
