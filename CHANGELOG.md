...
- *(agent)* `From<functions::Config>` is generated for every `ProviderConfig` variant (including the feature-gated Bedrock and Gemini-gRPC companions and the two second-surface providers), and `AgentBuilder::new` accepts `impl Into<ProviderConfig>`, so `AgentBuilder::new(cfg).preamble(..).build()` no longer needs explicit enum wrapping
