...
- *(core, agent)* fluent builders are restored for the surfaces that construct concrete owned data, without reintroducing generics: `CompletionRequest::builder(prompt)` returns a **non-generic** `CompletionRequestBuilder` carrying the former setters (`preamble`/`without_preamble`, `model(_opt)`, `message(s)`, `document(s)`, `tool(s)`, `provider_tool(s)`, merging `additional_params` vs replacing `additional_params_opt`, `temperature(_opt)`, `max_tokens(_opt)`, `tool_choice(_opt)`, `output_schema(_opt)`, `record_content_telemetry`) and ending in `.build()`; the model-bound `.send()`/`.stream()` terminals are deliberately **not** restored — pass the built request to the provider's `functions::complete`/`open_stream`
...
- [**breaking**] *(core)* request typestate builders are gone: `VectorSearchRequestBuilder`, `TranscriptionRequestBuilder`, `ImageGenerationRequestBuilder`, `AudioGenerationRequestBuilder` and the trait methods that returned them (`transcription_request`/`image_generation_request`/`audio_generation_request`) are deleted in favour of plain constructors plus `with_*` setters — `VectorSearchRequest::new(query, samples)`, `TranscriptionRequest::new(data)` (`from_file` replaces `load_file`), `ImageGenerationRequest::new(prompt)`, `AudioGenerationRequest::new(text, voice)`; the builders' `send()` is gone, so build the request and call the model's `transcription`/`image_generation`/`audio_generation` (or the provider `functions::*` free functions); `markers::Provided<T>` is removed
...
- *(core)* [**breaking**] `CompletionRequestBuilder` and `model.completion_request(...)` are removed: `CompletionRequest` is plain data — build it with the `CompletionRequest::with_history`/`CompletionRequest::from_prompt` constructors plus struct-update syntax
- *(core)* [**breaking**] modalities as free functions: embedding, transcription, image-generation, audio-generation, and rerank calls are extracted to per-provider free functions over plain configs (each provider's `functions` module, e.g. free `embed`/`embed_batches`); the classic `EmbeddingsBuilder`/`EmbeddingModel` surface still works and routes through the same functions
