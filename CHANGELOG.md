...
- [**breaking**] *(agent)* model calls are transactional across `AgentSession` and `AgentStream`. Preparation/open/provider/midstream failures, async cancellation, and `close_turn` restore the exact one-turn `RequestPatch` and model-call budget, discard partial history/tool calls/usage/output-tool metadata, and allow a clean retry. Retried provider I/O reuses the already-answered logical attempt instead of redispatching `BeforeModelCall`, so append-only patch fields are not duplicated. Stream deltas remain observable but provisional until natural EOF and may repeat after retry; invalid-tool retry/skip transitions likewise commit only after checked EOF. `AgentStreamItem::TurnFinished` now carries its own `message_id` rather than consulting a prior turn, and a successful EOF-only turn clears `last_response()` instead of exposing a prior turn's terminal record. `ModelTurnOutcome::Continue` carries the shared `AcceptedModelTurn` record, so a repaired turn crosses the normalized `ModelTurnFinished` steering boundary exactly once in blocking and streaming runs before tool preflight, independently of medium-specific provider-response observations
- [**breaking**] *(agent)* tool execution uses one ordered `ToolExecutionRecord` per invocation (`batch_index` plus Rig `internal_call_id`, original/effective call, structured result, model-visible result, invocation disposition, and optional span). `ToolBatchOutput` exposes `records`, visible result projections, and identity-bearing `submissions()`/`into_submissions()` instead of parallel result/raw-result/span vectors. Manual `AgentSession`/`AgentStream` result submission likewise uses `ToolResultSubmission { internal_call_id, result, disposition }`, so duplicate provider call IDs and reverse completion/submission order remain correctly paired; submissions validate both provider `id` and `call_id`, while the provider-ID-only `AgentRun::tool_results` convenience rejects ambiguous duplicate-ID batches. Rig IDs are generated in durable `AgentRun` state before a tool batch surfaces, and serialized pending calls retain original/effective calls plus positional pre-resolved results and typed invocation disposition. Unary and streamed invalid-call repairs retain the provider-emitted call as `original_call` and the repaired invocation as `effective_call`. `ToolResultReady` now exposes `internal_call_id`; `ResolvedToolCall` carries the complete effective call with a normalized `ResolvedToolCallDisposition`, so rewrite-then-skip and invalid-peer suppression cannot lose rewritten arguments, call identity, or classification. `ToolExecutionCommitted` is emitted only when a local body ran or the host explicitly supplied external execution; skips, unknown tools, and other pre-resolved outcomes retain durable model-visible results without an execution observation
