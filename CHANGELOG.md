...
- *(core)* [**breaking**] mark the receive-side public structs `#[non_exhaustive]` so adding a field to them is no longer a breaking change. Eighteen types are covered — `model::{Model, ModelList}`, `completion::message::ToolCall`, `streaming::RawStreamingToolCall`, and the provider-native response, terminal-stream and usage/billing types for anthropic, openai (chat and Responses), gemini, ollama and cohere. Every one of them except `ModelList` gained a field this cycle, and `Model`'s addition broke downstream struct literals precisely because it lacked the attribute. Only cross-crate *construction* changes: replace a struct literal with a constructor plus field assignment (`let mut m = Model::from_id("gpt-4"); m.name = Some("GPT-4".into());`) and add a trailing `..` to cross-crate exhaustive destructuring; fields stay public, readable and writable, and derived `Deserialize` still works everywhere. `completion::message::ToolResult` and `providers::anthropic::completion::AnthropicRequestParams` are deliberately excluded because the struct literal is their only construction path today — see `MIGRATING.md` ([#2330](https://github.com/0xPlaygrounds/rig/pull/2330))
