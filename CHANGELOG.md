...
- *(agent)* `ToolExecutor::builder()` returns a `ToolExecutorBuilder` with repeated `.tool(..)` (typed, erased to a record immediately), `.dynamic_tool(..)`, and `.dynamic_tools(..)`/`.tools(..)`, ending in `.build()`
