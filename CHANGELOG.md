...
- *(agent)* `agent.extractor(prompt)` returns a concrete `ExtractionRunner` holding agent config, provider config, runtime, prompt, and `ExtractOptions`, with fluent setters (`classic`, `retries`, `history`, `preamble`, `context(s)`, `additional_params`, `max_tokens`, `temperature`, `tool_choice`, `max_turns`, output-tool configuration). The runner is **not** generic over the extracted type — it is selected at the terminal `.run::<T>()` / `.run_with_usage::<T>()`, so there is no `ExtractionRunner<T>` and no `PhantomData<T>`
