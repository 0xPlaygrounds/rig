...
- *(openai)* `openai::TranscriptionUsage` (with `DurationTag`, `TokensTag` and `TranscriptionInputTokenDetails`), reached through the new `TranscriptionResponse::usage` field. The struct has public fields and no constructor, so code building one with a struct literal — a test double, a hand-made response — must add the field; it is deserialize-only in this repo, and every provider that shares it (Groq, Azure OpenAI, Venice, HuggingFace) keeps decoding responses that omit `usage` ([#2332](https://github.com/0xPlaygrounds/rig/pull/2332))

### Fixed

- *(openrouter)* model listings now report `context_length` and `max_output_tokens`. A stray `#[serde(rename_all = "camelCase")]` on the listing entry made serde look for `contextLength`, which OpenRouter never sends, so every model's context window decoded as `None` while the response carried a real one (262144 for the first entry in the recorded fixture). The output ceiling, which OpenRouter reports under `top_provider.max_completion_tokens`, was not read at all — the same class of drop [#2322](https://github.com/0xPlaygrounds/rig/pull/2322) added `max_output_tokens` to prevent. Found by recording the listing matrix ([#2079](https://github.com/0xPlaygrounds/rig/issues/2079))
- *(gemini)* model-listing failures now classify as `ModelListingError::ApiError` with the provider label, path, status and body preview, instead of a bare `RequestError`. Gemini built its own request and let a transport-level send error convert directly; because the reqwest transport reports a non-2xx *as an error* before returning a response, that was the path every real listing failure took, and Gemini's own status-check branch was dead — the same dead-arm shape [#2315](https://github.com/0xPlaygrounds/rig/pull/2315) fixed for `verify`. Routing through the shared fetch aligns Gemini with every other lister. Callers matching `RequestError` on a failed Gemini listing should match the accessors or `ApiError` instead ([#2079](https://github.com/0xPlaygrounds/rig/issues/2079))
...
- *(mistral)* stop sending a forced `tool_choice` beside a `response_format`: Mistral rejects that combination outright (`"`json_schema` response type with tools is only compatible with `tool_choice: auto`"`), and rig built it by itself — a structured-output agent defers `response_format` until a tool result exists, then emits it beside the caller's standing `tool_choice`, so the turn *after* the first tool call died with a 400, once the tool had already run. Finalization now relaxes the choice to `auto` for that turn, keeping the caller's schema ([#2337](https://github.com/0xPlaygrounds/rig/pull/2337))
...
- *(core)* preserve a failed response's headers so `Retry-After` survives onto the error: the transport already captured the header map (rig#2314 needed it to read each provider's request-id header), but every normalization path then discarded it, leaving rate-limit metadata structurally unrecoverable — including by Rig's own `http_client::retry::RetryPolicy::retry`, which is handed the error. Headers now ride through all four shared request drivers (completion, transcription, image generation, audio generation), the SSE handshake conversion, and `VerifyClient::verify`, reachable via a new `provider_response_headers()` on every capability error, `PromptError`/`StructuredOutputError`, and `http_client::Error::non_success_headers()`. `None` means "not captured", never "the response had no headers". Note for callers matching the transport error directly: contract-less providers' non-success completions and all three `verify()` branches now yield `http_client::Error::InvalidStatusCodeWithDetails` rather than `InvalidStatusCodeWithMessage` — both are `HttpError`, both `Display` identically, and every `provider_response_*` helper reads both, but a `match` arm naming the old variant will stop firing ([#2333](https://github.com/0xPlaygrounds/rig/pull/2333))
...
- *(openai)* preserve the provider's response when a Responses **websocket** upgrade is rejected: the upgrade is a plain HTTP request until the provider accepts it, and a rejected one answers with a status, an `x-request-id` and a JSON error body — a live handshake with an invalid key returns `401` with `{"error":{"code":"invalid_api_key",…}}`. `tungstenite` hands all three back, and rig flattened them to `ProviderError("HTTP error: 401 Unauthorized")`, so `provider_response_status()`, `provider_response_body()` and `provider_request_id()` were all `None` and a bad key was indistinguishable from a network fault. A rejected upgrade now classifies as `CompletionError::ProviderResponse` with the body and id attached — the contract the crate's other two completion transports have kept since [#2314](https://github.com/0xPlaygrounds/rig/pull/2314)/[#2315](https://github.com/0xPlaygrounds/rig/pull/2315) (the blocking path through `send_completion`, the SSE connect path through `sse_transport`), and which the websocket never had; failures that never reached the provider stay `ProviderError` ([#2336](https://github.com/0xPlaygrounds/rig/pull/2338))
...
- *(providers)* the cursor-paginated model listers (Anthropic, Gemini) share one loop. Each had hand-rolled its own and each shipped the same unbounded-loop bug in [#2334](https://github.com/0xPlaygrounds/rig/pull/2334); the termination rules — no next cursor ends the listing, a repeated cursor ends it — now live once in `internal::model_listing::paginate_models`, with each provider supplying only how it spells a cursor and how it reads one out of a page. Venice's lister, which was the shared macro's output written by hand, uses the macro. No public type changed ([#2079](https://github.com/0xPlaygrounds/rig/issues/2079))
- *(core)* [**breaking**] the response-identity fields on `StreamFinal`, `completion::CompletionResponse` and `completion::ResponseIdentity` — `message_id`, `response_id`, `provider_request_id` — are now `Option<WireId>` instead of `Option<String>`, making the empty-string-is-absent rule structural rather than a convention of the `with_*_id` setters. `WireId::new` is the only constructor and rejects `""`, so `Some("")` is unrepresentable however the value is built; previously a struct literal or a field assignment could reintroduce the sentinel and make the buffered and streaming results of one request compare unequal. `streaming::StreamingCompletionResponse`'s `message_id` — released as `Option<String>` in v0.41.0 — is retyped with them, and `rig-agent`'s identity-carrying types (`CompletionCall`, `ModelTurn`, `StreamedTurn`, `PartialStreamedTurn`) follow, since they round-trip these ids through `ResponseIdentity`. Reading is unchanged for most code — `WireId` derefs to `str`, so `.as_deref()`, `==` against a `&str` and `{}` formatting all still work; a site that needs an owned `String` uses `.map(String::from)`. **Serialized JSON is unchanged**: `WireId` serializes transparently as its bare string, and every deserialization path still normalizes a stored `""` to `None` rather than failing the record. `ProviderResponseError::provider_request_id` follows for the same reason — it is the same transport id on the failure path, and leaving it `Option<String>` would have let an error render `(request id: )` for a value the success path can no longer produce; its `provider_request_id()` accessor still returns `Option<&str>`, so no reader changes. `WireId` has `Serialize` but deliberately no `Deserialize` (a fallible one would turn a stored `""` into a hard load failure, a transparent one would accept `""` back), and a compile-fail cell pins that absence so it cannot be added by mistake. `completion::Message::Assistant`'s `id` follows too, which is the one that matters most: it is the id that is *persisted in history and replayed upstream*, where the transient response types are not. Because it now holds the same type as the response metadata it is copied from, the conversion at every hand-off is unnecessary rather than merely removed: the two ends of one hand-off hold the same type. The persisted JSON is unchanged: the handle serializes as its bare string, an absent id is still written as an explicit `null`, and the field remains optional to serde — a bare `Option<T>` was already implicitly optional, and because `deserialize_with` removes that special case, `serde(default)` is paired with it to preserve exactly the same accepted shape. A stored `""` now loads as `None` rather than as a sentinel. `message::Reasoning`'s `id` is migrated for the same reason and closes a gap **inherited from `main` rather than introduced here**: it is a durable handle that travels back upstream in a replayed request, and the sentinel was live — the OpenRouter converter carried a hand-spelled `.filter(|id| !id.is_empty())` with a comment noting the empty id actually occurs, while two request-building paths (`xai::api`, `openai::responses_api`) gated on `Some` and sent the value straight into the body, each with a comment asserting an invariant the type did not enforce. Those comments are now true by construction: the filter is deleted as unreachable, and the streaming hand-off drops its `WireId::into_string` downgrade because `Reasoning::id` can hold the handle directly. `Reasoning::with_id` widens from `String` to `impl Into<String>` (source-compatible) and normalizes, so an empty argument is absence. `WireId` also gains `PartialOrd`/`Ord`, which `Option<String>` had and the sibling `ToolCallId` keeps, so a downstream `sort_by_key` over these ids keeps compiling. `model` is deliberately left `Option<String>` — it is a label, not an identifier — with its setter still normalizing it ([#2336](https://github.com/0xPlaygrounds/rig/issues/2336))
