...
- [**breaking**] *(agent)* model calls are transactional across `AgentSession` and `AgentStream`. Preparation/open/provider/midstream failures, async cancellation, and `close_turn` restore the exact one-turn `RequestPatch` and model-call budget, discard partial history/tool calls/usage/output-tool metadata, and allow a clean retry. Every provider operation has a fresh attempt identity and owns its real prompt plus its effective Tool-mode `{ output_tool_name, effective_schema }` contract until commit; `PreparedRequest::output_tool_contract` replaces the former name-only `output_tool_name` field, while its opaque `PreparedModelAttempt` receipt binds that exact prompt/schema/identity and the validation name sets to hand-driven unary or streamed responses after provider I/O. Retried provider I/O reuses the already-answered logical request instead of redispatching `BeforeModelCall`, so append-only patch fields are not duplicated, while attempt identity and streamed-delta aggregation still reset. Tool-mode validation now uses the schema advertised by that attempt (separate from the provider-native `CompletionRequest::output_schema`, which stays unset in Tool mode); corrective retries inherit the violated contract unless a new patch replaces it, and ordinary turns return to the configured baseline. Response observations receive the real attempt prompt even when completion-call decisions are not surfaced. Stream deltas remain observable but provisional until natural EOF and may repeat after retry; invalid-tool retry/skip transitions likewise commit only after checked EOF. `AgentStreamItem::TurnFinished` now carries its own `message_id` rather than consulting a prior turn, and a successful EOF-only turn clears `last_response()` instead of exposing a prior turn's terminal record. `ModelTurnOutcome::Continue` carries the shared `AcceptedModelTurn` record, and `AgentRun` represents “verdict required” and “ready to advance” as distinct serializable states: drivers must call `continue_model_turn`, and `next_step` no longer silently consumes a pending verdict
