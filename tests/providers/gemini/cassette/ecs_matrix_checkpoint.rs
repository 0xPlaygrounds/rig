...
async fn multi_turn_unary() {
    with_gemini_cassette("checkpoint_matrix/multi_turn_unary", |client| async move {
        ...
        crate::ecs_matrix::checkpoint_world::run_world(
            &wire(&client),
            &cell,
            golden_gemini_checkpoint_multi_turn_unary,
        )
        .await;
    })
    .await;
    ...
    with_gemini_cassette(
        ...
        |client| async move {
            ...
            crate::ecs_matrix::checkpoint_world::run_world(
                &wire(&client),
                &cell,
                golden_gemini_checkpoint_multi_turn_streamed,
            )
            .await;
        },
    )
...
async fn parallel_batch() {
    with_gemini_cassette("checkpoint_matrix/parallel_batch", |client| async move {
        ...
        crate::ecs_matrix::checkpoint_world::run_world(
            &wire(&client),
            &cell,
            golden_gemini_checkpoint_parallel_batch,
        )
        .await;
    })
    .await;
...
async fn large_result() {
    with_gemini_cassette("checkpoint_matrix/large_result", |client| async move {
        ...
        crate::ecs_matrix::checkpoint_world::run_world(
            &wire(&client),
            &cell,
            golden_gemini_checkpoint_large_result,
        )
        .await;
    })
    .await;
