//! Cassette coverage for `AgentDriver` against real Gemini traffic.
//!
//! The driver is provider-agnostic; the request it builds is not. Gemini
...
//! a Gemini one, and only a recorded body would show it.
//!
...
/// The whole hand-driven loop over real Gemini traffic, locking both
/// request bodies.
#[tokio::test]
async fn drive_loop_round_trips_a_tool_call() {
    with_gemini_cassette("agent_driver/tool_call_round_trip", |client| async move {
        let agent = client
            .agent(MODEL)
...
/// A custom run's `tool_choice` must reach Gemini, whose wire spelling
/// differs from OpenAI's.
#[tokio::test]
async fn a_custom_runs_tool_choice_reaches_the_provider() {
    with_gemini_cassette("agent_driver/run_tool_choice", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .tool(Adder)
...
/// Gemini showed the model returning a genuinely empty turn — forbidden
...
async fn tool_choice_none_forbids_tools_on_the_wire() {
    with_gemini_cassette("agent_driver/tool_choice_none", |client| async move {
        let agent = client
            .agent(MODEL)
...
/// `ToolChoice::Specific` names one tool on the Gemini wire.
#[tokio::test]
async fn tool_choice_specific_names_the_tool_on_the_wire() {
    with_gemini_cassette("agent_driver/tool_choice_specific", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .tool(Adder)
...
async fn a_patched_preamble_replaces_the_agents_on_the_wire() {
    with_gemini_cassette("agent_driver/patch_preamble", |client| async move {
        let agent = client
            .agent(MODEL)
...
/// `active_tools` narrows the tools Gemini is shown.
#[tokio::test]
async fn a_patched_active_tools_narrows_the_advertised_set() {
    with_gemini_cassette("agent_driver/patch_active_tools", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .default_max_turns(3)
...
async fn a_patched_tool_choice_outranks_the_runs() {
    with_gemini_cassette("agent_driver/patch_tool_choice", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .tool(Adder)
...
/// A driver-level history leads the Gemini request.
#[tokio::test]
async fn driver_history_leads_the_request() {
    with_gemini_cassette("agent_driver/driver_history", |client| async move {
...
async fn a_run_suspended_awaiting_the_model_resumes_and_accepts_the_reply() {
    with_gemini_cassette("agent_driver/resume_awaiting_model", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .default_max_turns(2)
...
async fn a_run_suspended_executing_tools_resumes_and_completes() {
    with_gemini_cassette("agent_driver/resume_executing_tools", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .default_max_turns(3)
...
/// Tool output mode against Gemini: the synthetic output tool is advertised
/// and allowed, never executable, and the run finalizes on its call.
#[tokio::test]
async fn tool_output_mode_finalizes_via_the_output_tool() {
    with_gemini_cassette("agent_driver/output_mode_tool", |client| async move {
        let agent = client
            .agent(MODEL)
...
/// A streamed turn driven through the driver against real Gemini SSE.
#[tokio::test]
async fn a_streamed_turn_drives_through_the_driver() {
    with_gemini_cassette("agent_driver/streamed_turn", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .default_max_turns(3)
...
async fn a_provider_rejection_is_not_retryable() {
    with_gemini_cassette("agent_driver/provider_rejection", |client| async move {
        let agent = client
            .agent("gemini-this-model-does-not-exist")
            .preamble(FORCE_TOOLS_PREAMBLE)
            .build();
        ...
        assert!(
            body.contains("not found") || body.contains("NOT_FOUND") || body.contains("model"),
...
async fn max_turns_exhaustion_stops_before_a_second_send() {
    with_gemini_cassette("agent_driver/max_turns", |client| async move {
        let agent = client
            .agent(MODEL)
            .preamble(FORCE_TOOLS_PREAMBLE)
            .tool(Adder)
