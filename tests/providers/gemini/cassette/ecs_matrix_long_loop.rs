...
async fn long_unary() {
    with_gemini_cassette("long_loop_matrix/long_unary", |client| async move {
        ...
        long_loop_world::run_world(&wire(&client), &cell, golden_gemini_long_loop_long_unary).await;
    })
    .await;
}
...
async fn long_streamed() {
    with_gemini_cassette("long_loop_matrix/long_streamed", |client| async move {
        long_loop_world::run_world(
            ...
            golden_gemini_long_loop_long_streamed,
        )
        .await;
    })
    .await;
...
async fn parallel_calls() {
    with_gemini_cassette("long_loop_matrix/parallel_calls", |client| async move {
        long_loop_world::run_world(
            ...
            golden_gemini_long_loop_parallel_calls,
        )
        .await;
    })
    .await;
...
async fn big_result() {
    with_gemini_cassette("long_loop_matrix/big_result", |client| async move {
        long_loop_world::run_world(
            ...
            golden_gemini_long_loop_big_result,
        )
        .await;
    })
    .await;
...
async fn tool_error_midway() {
    with_gemini_cassette("long_loop_matrix/tool_error_midway", |client| async move {
        long_loop_world::run_world(
            ...
            golden_gemini_long_loop_tool_error_midway,
        )
        .await;
    })
    .await;
...
async fn max_turns_midway() {
    with_gemini_cassette("long_loop_matrix/max_turns_midway", |client| async move {
        long_loop_world::run_world(
            ...
            golden_gemini_long_loop_max_turns_midway,
        )
        .await;
    })
    .await;
...
fn golden_gemini_long_loop_long_unary(log: &rig::effect_log::EffectLog) {
    crate::ecs_goldens::golden_effects("gemini_long_loop_long_unary", log);
}

fn golden_gemini_long_loop_long_streamed(log: &rig::effect_log::EffectLog) {
    crate::ecs_goldens::golden_effects("gemini_long_loop_long_streamed", log);
}

fn golden_gemini_long_loop_parallel_calls(log: &rig::effect_log::EffectLog) {
    crate::ecs_goldens::golden_effects("gemini_long_loop_parallel_calls", log);
}

fn golden_gemini_long_loop_big_result(log: &rig::effect_log::EffectLog) {
    crate::ecs_goldens::golden_effects("gemini_long_loop_big_result", log);
}

fn golden_gemini_long_loop_tool_error_midway(log: &rig::effect_log::EffectLog) {
    crate::ecs_goldens::golden_effects("gemini_long_loop_tool_error_midway", log);
}

fn golden_gemini_long_loop_max_turns_midway(log: &rig::effect_log::EffectLog) {
    crate::ecs_goldens::golden_effects("gemini_long_loop_max_turns_midway", log);
}

fn golden_gemini_long_loop_output_cap_midway(log: &rig::effect_log::EffectLog) {
