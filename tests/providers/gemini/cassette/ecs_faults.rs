...
async fn setup_unary() {
    with_gemini_cassette("corpus_faults/setup_unary", |client| async move {
        run_world(&missing(&client), &SETUP_UNARY, |log| {
            crate::ecs_goldens::golden_effects("gemini_fault_setup_unary", log)
        })
        .await;
    })
    .await;
    ...
    with_gemini_cassette(
        ...
        |client| async move {
            run_world(&missing(&client), &SETUP_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("gemini_fault_setup_streamed", log)
            })
            .await;
        },
    )
...
async fn tool_error() {
    with_gemini_cassette("corpus_faults/tool_error", |client| async move {
        run_world(&wire(&client), &faults::TOOL_ERROR, |log| {
            crate::ecs_goldens::golden_effects("gemini_fault_tool_error", log)
        })
        .await;
    })
    .await;
...
async fn tool_error_streamed() {
    with_gemini_cassette("corpus_faults/tool_error_streamed", |client| async move {
        run_world(&wire(&client), &faults::TOOL_ERROR_STREAMED, |log| {
            crate::ecs_goldens::golden_effects("gemini_fault_tool_error_streamed", log)
        })
        .await;
    })
    .await;
...
async fn batch_second_fails() {
    with_gemini_cassette("corpus_faults/batch_second_fails", |client| async move {
        run_world(&wire(&client), &faults::BATCH_SECOND_FAILS, |log| {
            crate::ecs_goldens::golden_effects("gemini_fault_batch_second_fails", log)
        })
        .await;
    })
    .await;
        ...
        run_world(
            ...
            |log| {
                crate::ecs_goldens::golden_effects(
                    "gemini_fault_batch_second_fails_concurrent",
                    log,
                )
            },
        )
