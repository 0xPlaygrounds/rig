...
use super::super::hook_stress_support::{CHAIN_PREAMBLE, CountingMultiply, fact_doc};
...
async fn scratchpad_tally_grows_across_turns_and_is_read_by_second_hook_blocking() {
    ...
    with_gemini_cassette(
            ...
            |client| async move {
                ...
                assert_eq!(
                    * tallies.last().expect("a tally"), total_calls,
                    "the final tally must equal the total ToolCall count"
                );
                assert_eq!(
                    tap_probe.count("ToolCall"), total_calls,
        ...
        |client| async move {
            ...
            runtime::install_patch(
                ...
                rig_ecs::agent::RequestPatch {
                    extra_context: vec![fact_doc("harbor", "The harbor code is ALPHA-11.")],
            ...
            runtime::install_patch(
                &mut ecs,
                rig_ecs::agent::RequestPatch {
                    extra_context: vec![fact_doc("orchard", "The orchard code is BETA-22.")],
                    ..Default::default()
                },
                false,
            );
            let response = runtime::prompt(
                &mut ecs,
                "Tell me both the harbor code and the orchard code.",
                4,
                vec![],
                vec![],
            )
