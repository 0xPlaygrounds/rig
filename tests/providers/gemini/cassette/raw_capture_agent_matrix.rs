...
//! | 2 | `hooks_observe_raw_streamed` | `agent.stream_prompt` | `StreamResponseFinish` and `ModelTurnFinished` see `raw`; `response_id` matches fixture | recorded |
...
async fn hooks_observe_raw_streamed() {
    ...
    assert_eq!(recorded.len(), 1);
    assert_distinct_response_ids(&recorded, scenario);
    assert_ids_match_recording(&ids_of(&finishes, "response_id"), &recorded, scenario);
    assert_eq!(
        finishes[0]
