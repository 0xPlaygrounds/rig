...
fn prompt(cell: Cell) -> &'static str {
    match (cell.model, cell.termination) {
        ...
        (ModelVariant::Gpt4oMini, Termination::Length) => {
            "Write the word cobalt exactly 100 times, separated by spaces."
        }
...
async fn blocking_gpt_4o_mini_stop_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4o_mini_stop_top_absent";
    let cell = cell(
        Transport::Blocking,
    ...
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4o_mini_stop_top_absent",
...
async fn blocking_gpt_4o_mini_stop_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4o_mini_stop_top_zero";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt4oMini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4o_mini_stop_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4o_mini_stop_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4o_mini_stop_top_two";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt4oMini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4o_mini_stop_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4o_mini_length_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4o_mini_length_top_absent";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt4oMini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4o_mini_length_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4o_mini_length_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4o_mini_length_top_zero";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt4oMini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4o_mini_length_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4o_mini_length_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4o_mini_length_top_two";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt4oMini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4o_mini_length_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4_1_mini_stop_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4_1_mini_stop_top_absent";
    let cell = cell(
        Transport::Blocking,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4_1_mini_stop_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4_1_mini_stop_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4_1_mini_stop_top_zero";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt41Mini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4_1_mini_stop_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4_1_mini_stop_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4_1_mini_stop_top_two";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt41Mini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4_1_mini_stop_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4_1_mini_length_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4_1_mini_length_top_absent";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt41Mini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4_1_mini_length_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4_1_mini_length_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4_1_mini_length_top_zero";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt41Mini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4_1_mini_length_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn blocking_gpt_4_1_mini_length_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/blocking_gpt_4_1_mini_length_top_two";
    let cell = cell(
        Transport::Blocking,
        ModelVariant::Gpt41Mini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/blocking_gpt_4_1_mini_length_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4o_mini_stop_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4o_mini_stop_top_absent";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt4oMini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4o_mini_stop_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4o_mini_stop_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4o_mini_stop_top_zero";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt4oMini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4o_mini_stop_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4o_mini_stop_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4o_mini_stop_top_two";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt4oMini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4o_mini_stop_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4o_mini_length_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4o_mini_length_top_absent";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt4oMini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4o_mini_length_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4o_mini_length_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4o_mini_length_top_zero";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt4oMini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4o_mini_length_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4o_mini_length_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4o_mini_length_top_two";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt4oMini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4o_mini_length_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4_1_mini_stop_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4_1_mini_stop_top_absent";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt41Mini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4_1_mini_stop_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4_1_mini_stop_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4_1_mini_stop_top_zero";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt41Mini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4_1_mini_stop_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4_1_mini_stop_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4_1_mini_stop_top_two";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt41Mini,
        Termination::Stop,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4_1_mini_stop_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4_1_mini_length_top_absent() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4_1_mini_length_top_absent";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt41Mini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4_1_mini_length_top_absent",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4_1_mini_length_top_zero() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4_1_mini_length_top_zero";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt41Mini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4_1_mini_length_top_zero",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
...
async fn streaming_gpt_4_1_mini_length_top_two() -> Result<()> {
    const SCENARIO: &str = "streaming_logprobs_matrix/streaming_gpt_4_1_mini_length_top_two";
    let cell = cell(
        Transport::Streaming,
        ModelVariant::Gpt41Mini,
        Termination::Length,
    ...
    let capture = Arc::clone(&observed);
    with_openrouter_stream_logprobs_cassette_result(
        "streaming_logprobs_matrix/streaming_gpt_4_1_mini_length_top_two",
        |client| async move { run_cell(client, cell, capture).await },
    )
    .await?;
    assert_cell(SCENARIO, cell, observed);
