...
//! record — for OpenRouter the shared chat-completions terminal
//! [`StreamingCompletionResponse`] over OpenRouter's own
//! [`openrouter::Usage`] — serialized. It is the terminal record only, never
//! the stream's frames. OpenRouter's terminal usage carries the turn's
//! `cost`, and the terminal's accumulated `additional_params` carries the
//! routed `provider` the frames repeat; neither has a slot on the normalized
...
//! | 2 | `stream_capture_on_round_trips_terminal_type` | flag on | terminal `raw` deserializes into `StreamingCompletionResponse<openrouter::Usage>` and re-serializes equal | recorded |
//! | 3 | `stream_capture_on_exposes_terminal_cost_and_provider` | terminal-only field | `raw.usage.cost` and `raw.additional_params.provider` equal the recorded terminal frame's | recorded |
...
fn assert_terminal_reproduces_frame(terminal: &StreamFinal, frame: &Value, context: &str) {
    ...
    assert_eq!(
        ...
        "{context}: OpenRouter contracts no id header"
    );
}
...
#[tokio::test]
async fn stream_capture_on_exposes_terminal_cost_and_provider() {
    const SCENARIO: &str =
        "raw_stream_capture_matrix/stream_capture_on_exposes_terminal_cost_and_provider";
    let observed = std::sync::Arc::new(std::sync::Mutex::new(None));
    let sink = observed.clone();
    with_openrouter_cassette_result(
        "raw_stream_capture_matrix/stream_capture_on_exposes_terminal_cost_and_provider",
    ...
    .await
    .expect("stream_capture_on_exposes_terminal_cost_and_provider should replay from its cassette");
