...
async fn structured_output_smoke() {
    with_llamafile_cassette(
        ...
        |client| async move {
            let agent = client.agent(CASSETTE_CHAT_MODEL).build();
...
async fn prompt_typed_structured_output() {
    with_llamafile_cassette(
        "structured_output/prompt_typed_structured_output",
        |client| async move {
...
async fn prompt_typed_extended_details_structured_output() {
    with_llamafile_cassette(
        "structured_output/prompt_typed_extended_details_structured_output",
        |client| async move {
            let model = CASSETTE_CHAT_MODEL;
            let agent = client.agent(model).preamble(WEATHER_PREAMBLE).build();
...
async fn output_schema_structured_output() {
    with_llamafile_cassette(
        "structured_output/output_schema_structured_output",
        |client| async move {
            let model = CASSETTE_CHAT_MODEL;
