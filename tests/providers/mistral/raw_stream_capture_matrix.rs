...
//! record — for Mistral the shared chat-completions terminal
...
//! | 2 | `stream_capture_on_round_trips_terminal_type` | flag on | terminal `raw` deserializes into `StreamingCompletionResponse<mistral::Usage>` and re-serializes equal | recorded |
//! | 3 | `stream_capture_on_exposes_terminal_service_tier` | terminal-only field | `raw.usage.service_tier` equals the recorded terminal frame's | recorded |
...
fn assert_terminal_reproduces_frame(
    ...
    assert!(
        request_id.is_some(),
        "{context}: the recorded SSE response must carry mistral-correlation-id"
    );
    assert_matches_recorded_token(
...
#[tokio::test]
async fn stream_capture_on_exposes_terminal_service_tier() {
    const SCENARIO: &str =
        "raw_stream_capture_matrix/stream_capture_on_exposes_terminal_service_tier";
    let observed = std::sync::Arc::new(std::sync::Mutex::new(None));
    let sink = observed.clone();
    with_mistral_cassette_result(
        "raw_stream_capture_matrix/stream_capture_on_exposes_terminal_service_tier",
    ...
    .await
    .expect("stream_capture_on_exposes_terminal_service_tier should replay from its cassette");
