...
//! Mistral's own [`mistral::CompletionResponse`], serialized — onto the
...
//! field. Mistral's wire carries envelope metadata the normalized response
//! has no slot for — the `object` tag and the capacity tier Mistral reports
//! inside `usage` (`usage.service_tier`) — so those are the fields pinned
//! here as reachable only through `raw`.
//!
//! | # | Cell | Dimension | expected | Status |
//! |---|------|-----------|----------|--------|
//! | 1 | `capture_off_leaves_raw_none` | flag off (the default) | `raw == None` | recorded |
//! | 2 | `capture_on_round_trips_mistral_type` | flag on | `raw` deserializes into `mistral::CompletionResponse` and re-serializes equal | recorded |
//! | 3 | `capture_on_exposes_object_and_service_tier` | provider-only field | `raw.object` and `raw.usage.service_tier` equal the fixture body | recorded |
//! | 4 | `request_invariant_off_vs_on` | on-wire request | the flag-off and flag-on request bodies are byte-identical | recorded |
//! | 5 | `normalized_fields_identical_off_vs_on` | normalized view | both responses reproduce their own fixture bytes (including the `mistral-correlation-id` header); the on-response equals its raw re-normalized | recorded |
//!
//! Every cell is recorded. Each re-derives its premise from its own fixture
//! after the wrapper returns: cell 3 reads the tier out of the recorded body
//! rather than trusting the string the typed view reports, and cells 4 and 5
//! record the flag-off and flag-on turns as two interactions of one scenario
//! so the comparison is between bytes that crossed the wire in the same
//! session.
...
fn request(model: &mistral::CompletionModel, capture: bool) -> CompletionRequest {
    model
...
/// The `mistral-correlation-id` the recorded interaction at `index` carried.
...
fn assert_reproduces_fixture(
    ...
    assert!(
        request_id.is_some(),
        "{context}: the recorded response must carry mistral-correlation-id"
    );
    assert_matches_recorded_token(
...
// ================================================================
// 2. On round-trips Mistral's own type
// ================================================================

#[tokio::test]
async fn capture_on_round_trips_mistral_type() {
    const SCENARIO: &str = "raw_capture_matrix/capture_on_round_trips_mistral_type";
    with_mistral_cassette_result(
        "raw_capture_matrix/capture_on_round_trips_mistral_type",
        |client| async move {
            let model = client.completion_model(DEFAULT_MODEL);
    ...
    .await
    .expect("capture_on_round_trips_mistral_type should replay from its cassette");
...
#[tokio::test]
async fn capture_on_exposes_object_and_service_tier() {
    const SCENARIO: &str = "raw_capture_matrix/capture_on_exposes_object_and_service_tier";
    let observed = std::sync::Arc::new(std::sync::Mutex::new(None));
    let sink = observed.clone();
    with_mistral_cassette_result(
        "raw_capture_matrix/capture_on_exposes_object_and_service_tier",
        |client| async move {
            let model = client.completion_model(DEFAULT_MODEL);
            let response = model.completion(request(&model, true)).await?;
    ...
    .await
    .expect("capture_on_exposes_object_and_service_tier should replay from its cassette");
