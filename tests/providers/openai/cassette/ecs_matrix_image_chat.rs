//! The image matrix on the OpenAI Chat Completions wire (`gpt-5-mini`, no temperature: the gpt-5 family takes only its default): the image cells of
...
async fn inline_text_unary() {
    with_openai_cassette("image_matrix_chat/inline_text_unary", |client| async move {
        run_world(&wire(&client), &cells::IMAGE_INLINE_TEXT_UNARY, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_image_inline_text_unary", log)
        })
        .await;
    })
    .await;
...
async fn inline_text_streamed() {
    with_openai_cassette(
        "image_matrix_chat/inline_text_streamed",
        |client| async move {
            run_world(&wire(&client), &cells::IMAGE_INLINE_TEXT_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("openai_chat_image_inline_text_streamed", log)
            })
            .await;
        },
    )
...
async fn inline_mixed_order() {
    with_openai_cassette(
        "image_matrix_chat/inline_mixed_order",
        |client| async move {
            run_world(&wire(&client), &cells::IMAGE_INLINE_MIXED_ORDER, |log| {
                crate::ecs_goldens::golden_effects("openai_chat_image_inline_mixed_order", log)
            })
            .await;
        },
    )
...
async fn inline_tool_unary() {
    with_openai_cassette("image_matrix_chat/inline_tool_unary", |client| async move {
        run_world(&wire(&client), &cells::IMAGE_INLINE_TOOL_UNARY, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_image_inline_tool_unary", log)
        })
        .await;
    })
    .await;
...
async fn inline_tool_streamed() {
    with_openai_cassette(
        "image_matrix_chat/inline_tool_streamed",
        |client| async move {
            run_world(&wire(&client), &cells::IMAGE_INLINE_TOOL_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("openai_chat_image_inline_tool_streamed", log)
            })
            .await;
        },
    )
...
async fn inline_followup() {
    with_openai_cassette("image_matrix_chat/inline_followup", |client| async move {
        run_world(&wire(&client), &cells::IMAGE_INLINE_FOLLOWUP, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_image_inline_followup", log)
        })
        .await;
    })
    .await;
...
async fn url_text_unary() {
    with_openai_cassette("image_matrix_chat/url_text_unary", |client| async move {
        run_world(&wire(&client), &cells::IMAGE_URL_TEXT_UNARY, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_image_url_text_unary", log)
        })
        .await;
    })
    .await;
...
async fn url_tool_unary() {
    with_openai_cassette("image_matrix_chat/url_tool_unary", |client| async move {
        run_world(&wire(&client), &cells::IMAGE_URL_TOOL_UNARY, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_image_url_tool_unary", log)
        })
        .await;
    })
    .await;
