...
//! `OPENAI_PROMPT_CACHE_BREAKPOINT_KEY` sub-key of this wire's
//! `additional_params` extras (`OPENAI_RESPONSES_EXTRAS_KEY`) must serialize
...
use rig::message::{AdditionalParams, AssistantContent, Message, Text, UserContent};
use rig::prelude::*;
use rig::providers::openai;
use rig::providers::openai::responses_api::{
    OPENAI_PROMPT_CACHE_BREAKPOINT_KEY, OPENAI_RESPONSES_EXTRAS_KEY, PromptCacheBreakpoint,
};
...
fn breakpoint_prompt() -> Message {
    ...
    Message::User {
        content: vec![UserContent::Text(reference), UserContent::text(PROMPT)],
    }
}
