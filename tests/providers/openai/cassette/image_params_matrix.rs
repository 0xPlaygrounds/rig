...
async fn unlisted_model_generates_without_response_format() {
    ...
    with_openai_image_params_cassette(
        ...
        |client| async move {
            let model = client.chat.image_generation(UNLISTED_MODEL);
...
async fn allowlisted_model_still_generates() {
    ...
    with_openai_image_params_cassette(
        ...
        |client| async move {
            let model = client.chat.image_generation(openai::GPT_IMAGE_1);
...
async fn retired_model_reaches_model_validation() {
    ...
    with_openai_image_params_cassette(
        ...
        |client| async move {
            let model = client.chat.image_generation(openai::DALL_E_3);
...
/// reaches it through: `ResponsesApi` has no image surface of its own, and the
/// marker-type swap that used to reach `/images/generations` from the chat
...
async fn unlisted_dated_snapshot_reaches_its_own_validation() {
    ...
    with_openai_image_params_cassette(
        ...
        |client| async move {
            let model = client.chat.image_generation("gpt-image-2-2026-04-21");
...
async fn chatgpt_image_latest_reaches_its_own_validation() {
    ...
    with_openai_image_params_cassette(
        ...
        |client| async move {
            let model = client.chat.image_generation("chatgpt-image-latest");
...
async fn response_format_is_rejected_before_the_model_is_looked_at() {
    ...
    with_openai_image_params_cassette(
        "image_params_matrix/response_format_is_rejected_before_the_model_is_looked_at",
        |client| async move {
            let model = client.chat.image_generation("rig-nonexistent-image-model");
