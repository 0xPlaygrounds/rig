...
//! `corpus_matrix_responses.rs`, and asserted against that producer's golden,
...
async fn causal_completion_serial() {
    with_openai_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::CAUSAL_COMPLETION_SERIAL, |_| {}).await;
        },
    )
    .await;
}
...
async fn causal_completion_concurrent() {
    with_openai_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::CAUSAL_COMPLETION_CONCURRENT, |_| {}).await;
        },
    )
    .await;
}
...
async fn causal_completion_streamed() {
    with_openai_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::CAUSAL_COMPLETION_STREAMED, |_| {}).await;
        },
    )
    .await;
}
