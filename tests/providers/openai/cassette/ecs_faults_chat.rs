...
async fn setup_unary() {
    with_openai_cassette("corpus_faults_chat/setup_unary", |client| async move {
        run_world(&missing(&client), &SETUP_UNARY, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_fault_setup_unary", log)
        })
        .await;
    })
    .await;
    ...
    with_openai_cassette(
        ...
        |client| async move {
            run_world(&missing(&client), &SETUP_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("openai_chat_fault_setup_streamed", log)
            })
            .await;
        },
    )
...
async fn tool_error() {
    with_openai_cassette("corpus_faults_chat/tool_error", |client| async move {
        run_world(&wire(&client), &faults::TOOL_ERROR, |log| {
            crate::ecs_goldens::golden_effects("openai_chat_fault_tool_error", log)
        })
        .await;
    })
    .await;
    ...
    with_openai_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &faults::TOOL_ERROR_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("openai_chat_fault_tool_error_streamed", log)
            })
            .await;
        },
    )
    ...
    with_openai_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &faults::BATCH_SECOND_FAILS, |log| {
                crate::ecs_goldens::golden_effects("openai_chat_fault_batch_second_fails", log)
            })
            .await;
        },
    )
