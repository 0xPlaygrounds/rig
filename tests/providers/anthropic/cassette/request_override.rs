//! Verifies that a `CompletionAction::Patch` hook steers the model request per
//! turn, end-to-end through a real Anthropic round-trip.
//!
//! The agent registers two tools (`get_weather` and `get_time`). On the first
//! turn a hook returns `CompletionAction::patch(...)` that narrows the advertised
...
use rig::agent::{AgentHook, RequestPatch};
...
impl<M: CompletionModel> AgentHook<M> for ForceWeatherOnlyOnFirstTurn {
    async fn on_completion_call(
        ...
        if event.turn == 1 {
            return rig::agent::CompletionAction::patch(
                RequestPatch::new()
