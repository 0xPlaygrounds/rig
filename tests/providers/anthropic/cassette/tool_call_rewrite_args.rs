//! Verifies that a `ToolCallAction::Rewrite` hook action rewrites a tool call's
//! arguments before execution, end-to-end through a real Anthropic round-trip.
//!
//! The `get_weather` tool advertises only a `location` parameter, so the model
//! never sends a `units` field. A default hook injects `units: "celsius"` via
//! `ToolCallAction::rewrite`, and the tool records what it was
