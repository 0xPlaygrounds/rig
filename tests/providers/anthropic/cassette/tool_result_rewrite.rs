//! Verifies that a `ToolResultAction::Rewrite` hook action redacts a tool's
//! output before the model sees it, end-to-end through a real Anthropic round-trip.
//!
//! The `get_user_record` tool returns a record containing a (fake) SSN. A default
//! hook redacts the SSN via `ToolResultAction::rewrite`, so
