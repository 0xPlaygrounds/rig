...
//! DeepSeek's own [`deepseek::CompletionResponse`], serialized — onto the
//! normalized [`rig::completion::CompletionResponse::raw`]. Off by default,
//! never serialized into the request, and never a substitute for a normalized
//! field. DeepSeek is worth its own matrix because its usage block carries a
//! `prompt_cache_hit_tokens` / `prompt_cache_miss_tokens` split rig only
//! half-normalizes: the hit count reaches `Usage::cached_input_tokens`, the
//! miss count has no slot at all and is reachable only through `raw`.
//!
//! | # | Cell | Dimension | expected | Status |
//! |---|------|-----------|----------|--------|
//! | 1 | `capture_off_leaves_raw_none` | flag off (the default) | `raw == None` | recorded |
//! | 2 | `capture_on_round_trips_deepseek_type` | flag on | `raw` deserializes into `deepseek::CompletionResponse` and re-serializes equal | recorded |
//! | 3 | `capture_on_exposes_prompt_cache_miss_tokens` | provider-only field | `raw.usage.prompt_cache_miss_tokens` and `raw.system_fingerprint` equal the fixture body | recorded |
...
//! after the wrapper returns: cell 3 reads the miss count out of the recorded
...
fn request(model: &deepseek::CompletionModel, capture: bool) -> CompletionRequest {
    model
...
fn assert_reproduces_fixture(response: &CompletionResponse, body: &Value, context: &str) {
    ...
    assert_eq!(
        ...
        "{context}: cached input tokens come from prompt_cache_hit_tokens"
    );
    assert_eq!(
...
// ================================================================
// 2. On round-trips DeepSeek's own type
// ================================================================

#[tokio::test]
async fn capture_on_round_trips_deepseek_type() {
    const SCENARIO: &str = "raw_capture_matrix/capture_on_round_trips_deepseek_type";
    with_deepseek_cassette_result(
        "raw_capture_matrix/capture_on_round_trips_deepseek_type",
        |client| async move {
            let model = client.completion_model(MODEL);
    ...
    .await
    .expect("capture_on_round_trips_deepseek_type should replay from its cassette");
...
#[tokio::test]
async fn capture_on_exposes_prompt_cache_miss_tokens() {
    const SCENARIO: &str = "raw_capture_matrix/capture_on_exposes_prompt_cache_miss_tokens";
    let observed = std::sync::Arc::new(std::sync::Mutex::new(None));
    let sink = observed.clone();
    with_deepseek_cassette_result(
        "raw_capture_matrix/capture_on_exposes_prompt_cache_miss_tokens",
        |client| async move {
            let model = client.completion_model(MODEL);
            let response = model.completion(request(&model, true)).await?;
    ...
    .await
    .expect("capture_on_exposes_prompt_cache_miss_tokens should replay from its cassette");
