            ...
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_endings_tool_dispatch_cancelled",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_endings_answer_outcome_cancelled",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::ENDINGS_TURN_FINISHED_STOP, |log| {
                crate::ecs_goldens::golden_effects("deepseek_endings_turn_finished_stop", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::ENDINGS_ANSWER_TURN_STOP, |log| {
                crate::ecs_goldens::golden_effects("deepseek_endings_answer_turn_stop", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::ENDINGS_TEXT_DELTA_STOP, |log| {
                crate::ecs_goldens::golden_effects("deepseek_endings_text_delta_stop", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects("deepseek_endings_tool_call_delta_stop", log)
                },
            )
            .await;
        },
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOOKS_OBSERVE_EVERYTHING, |log| {
                crate::ecs_goldens::golden_effects("deepseek_hooks_observe_everything", log)
            })
            .await;
        },
    )
...
async fn hooks_patch_tool_args() {
    with_deepseek_cassette("corpus_matrix/hooks_patch_tool_args", |client| async move {
        run_world(&wire(&client), &cells::HOOKS_PATCH_TOOL_ARGS, |log| {
            crate::ecs_goldens::golden_effects("deepseek_hooks_patch_tool_args", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_hooks_patch_tool_args_streamed",
                        log,
                    )
                },
            )
...
async fn hooks_deny_tool() {
    with_deepseek_cassette("corpus_matrix/hooks_deny_tool", |client| async move {
        run_world(&wire(&client), &cells::HOOKS_DENY_TOOL, |log| {
            crate::ecs_goldens::golden_effects("deepseek_hooks_deny_tool", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOOKS_DENY_TOOL_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_hooks_deny_tool_streamed", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOOKS_REPLACE_TOOL_RESULT, |log| {
                crate::ecs_goldens::golden_effects("deepseek_hooks_replace_tool_result", log)
            })
            .await;
        },
    )
...
async fn hooks_replace_answer() {
    with_deepseek_cassette("corpus_matrix/hooks_replace_answer", |client| async move {
        run_world(&wire(&client), &cells::HOOKS_REPLACE_ANSWER, |log| {
            crate::ecs_goldens::golden_effects("deepseek_hooks_replace_answer", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOOKS_PREAMBLE_OVERRIDE, |log| {
                crate::ecs_goldens::golden_effects("deepseek_hooks_preamble_override", log)
            })
            .await;
        },
    )
...
async fn hooks_demand_done() {
    with_deepseek_cassette("corpus_matrix/hooks_demand_done", |client| async move {
        run_world(&wire(&client), &cells::HOOKS_DEMAND_DONE, |log| {
            crate::ecs_goldens::golden_effects("deepseek_hooks_demand_done", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOOKS_LOOKUP_BEFORE_RUN, |log| {
                crate::ecs_goldens::golden_effects("deepseek_hooks_lookup_before_run", log)
            })
            .await;
        },
    )
...
async fn hooks_two_hooks() {
    with_deepseek_cassette("corpus_matrix/hooks_two_hooks", |client| async move {
        run_world(&wire(&client), &cells::HOOKS_TWO_HOOKS, |log| {
            crate::ecs_goldens::golden_effects("deepseek_hooks_two_hooks", log)
        })
        .await;
    })
    .await;
...
async fn host_custom_at_start() {
    with_deepseek_cassette("corpus_matrix/host_custom_at_start", |client| async move {
        run_world(&wire(&client), &cells::HOST_CUSTOM_AT_START, |log| {
            crate::ecs_goldens::golden_effects("deepseek_host_custom_at_start", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_host_custom_at_completion_call",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOST_CUSTOM_AT_OUTCOME, |log| {
                crate::ecs_goldens::golden_effects("deepseek_host_custom_at_outcome", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOST_CUSTOM_AT_SETTLED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_host_custom_at_settled", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_host_custom_start_and_settled",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::HOST_CUSTOM_TWICE_SERIAL, |log| {
                crate::ecs_goldens::golden_effects("deepseek_host_custom_twice_serial", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects("deepseek_host_custom_twice_concurrent", log)
                },
            )
            .await;
        },
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_host_custom_at_start_streamed",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_host_custom_at_outcome_streamed",
                        log,
                    )
                },
            )
...
async fn host_custom_unserved() {
    with_deepseek_cassette("corpus_matrix/host_custom_unserved", |client| async move {
        run_world(&wire(&client), &cells::HOST_CUSTOM_UNSERVED, |log| {
            crate::ecs_goldens::golden_effects("deepseek_host_custom_unserved", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_serving_serial_concurrency_one",
                        log,
                    )
                },
            )
...
async fn serving_capacity_one() {
    with_deepseek_cassette("corpus_matrix/serving_capacity_one", |client| async move {
        run_world(&wire(&client), &cells::SERVING_CAPACITY_ONE, |log| {
            crate::ecs_goldens::golden_effects("deepseek_serving_capacity_one", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::SERVING_SERIAL_MEMORY_TOOLS, |log| {
                crate::ecs_goldens::golden_effects("deepseek_serving_serial_memory_tools", log)
            })
            .await;
        },
    )
...
async fn serving_model_route() {
    with_deepseek_cassette("corpus_matrix/serving_model_route", |client| async move {
        run_world(&wire(&client), &cells::SERVING_MODEL_ROUTE, |log| {
            crate::ecs_goldens::golden_effects("deepseek_serving_model_route", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_serving_model_route_unselected",
                        log,
                    )
                },
            )
...
async fn serving_host_bus() {
    with_deepseek_cassette("corpus_matrix/serving_host_bus", |client| async move {
        run_world(&wire(&client), &cells::SERVING_HOST_BUS, |log| {
            crate::ecs_goldens::golden_effects("deepseek_serving_host_bus", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::SERVING_HOST_BUS_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_serving_host_bus_streamed", log)
            })
            .await;
        },
    )
...
async fn layers_deny_tool() {
    with_deepseek_cassette("corpus_matrix/layers_deny_tool", |client| async move {
        run_world(&wire(&client), &cells::LAYERS_DENY_TOOL, |log| {
            crate::ecs_goldens::golden_effects("deepseek_layers_deny_tool", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::LAYERS_PATCH_TOOL_ARGS, |log| {
                crate::ecs_goldens::golden_effects("deepseek_layers_patch_tool_args", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::LAYERS_REPLACE_TOOL_RESULT, |log| {
                crate::ecs_goldens::golden_effects("deepseek_layers_replace_tool_result", log)
            })
            .await;
        },
    )
...
async fn layers_two_layers() {
    with_deepseek_cassette("corpus_matrix/layers_two_layers", |client| async move {
        run_world(&wire(&client), &cells::LAYERS_TWO_LAYERS, |log| {
            crate::ecs_goldens::golden_effects("deepseek_layers_two_layers", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_layers_host_deny_over_host_bus",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_layers_patch_beneath_hook_patch",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::LAYERS_MEMORY_LOAD_REPLACED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_layers_memory_load_replaced", log)
            })
            .await;
        },
    )
...
async fn memory_clear_at_start() {
    with_deepseek_cassette("corpus_matrix/memory_clear_at_start", |client| async move {
        run_world(&wire(&client), &cells::MEMORY_CLEAR_AT_START, |log| {
            crate::ecs_goldens::golden_effects("deepseek_memory_clear_at_start", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::MEMORY_CLEAR_AT_SETTLED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_memory_clear_at_settled", log)
            })
            .await;
        },
    )
...
async fn memory_two_runs() {
    with_deepseek_cassette("corpus_matrix/memory_two_runs", |client| async move {
        run_world(&wire(&client), &cells::MEMORY_TWO_RUNS, |log| {
            crate::ecs_goldens::golden_effects("deepseek_memory_two_runs", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::MEMORY_TWO_RUNS_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_memory_two_runs_streamed", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_memory_clear_at_settled_two_runs",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_memory_clear_at_start_two_runs",
                        log,
                    )
                },
            )
...
async fn memory_history_bypass() {
    with_deepseek_cassette("corpus_matrix/memory_history_bypass", |client| async move {
        run_world(&wire(&client), &cells::MEMORY_HISTORY_BYPASS, |log| {
            crate::ecs_goldens::golden_effects("deepseek_memory_history_bypass", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::MEMORY_HOST_BUS_MEMORY, |log| {
                crate::ecs_goldens::golden_effects("deepseek_memory_host_bus_memory", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::MEMORY_SERIAL_TWO_TOOLS, |log| {
                crate::ecs_goldens::golden_effects("deepseek_memory_serial_two_tools", log)
            })
            .await;
        },
    )
...
async fn memory_failing_append() {
    with_deepseek_cassette("corpus_matrix/memory_failing_append", |client| async move {
        run_world(&wire(&client), &cells::MEMORY_FAILING_APPEND, |log| {
            crate::ecs_goldens::golden_effects("deepseek_memory_failing_append", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_memory_failing_append_streamed",
                        log,
                    )
                },
            )
...
async fn output_tool_unary() {
    with_deepseek_cassette("corpus_matrix/output_tool_unary", |client| async move {
        run_world(&wire(&client), &cells::OUTPUT_TOOL_UNARY, |log| {
            crate::ecs_goldens::golden_effects("deepseek_output_tool_unary", log)
        })
        .await;
    })
    .await;
...
async fn output_tool_streamed() {
    with_deepseek_cassette("corpus_matrix/output_tool_streamed", |client| async move {
        run_world(&wire(&client), &cells::OUTPUT_TOOL_STREAMED, |log| {
            crate::ecs_goldens::golden_effects("deepseek_output_tool_streamed", log)
        })
        .await;
    })
    .await;
...
async fn output_prompted_unary() {
    with_deepseek_cassette("corpus_matrix/output_prompted_unary", |client| async move {
        run_world(&wire(&client), &cells::OUTPUT_PROMPTED_UNARY, |log| {
            crate::ecs_goldens::golden_effects("deepseek_output_prompted_unary", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::OUTPUT_PROMPTED_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_output_prompted_streamed", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::OUTPUT_TOOL_WITH_REAL_TOOL, |log| {
                crate::ecs_goldens::golden_effects("deepseek_output_tool_with_real_tool", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_output_prompted_with_real_tool",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::OUTPUT_TOOL_CHOICE_REQUIRED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_output_tool_choice_required", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_output_tool_under_none_degrades",
                        log,
                    )
                },
            )
...
async fn shaping_extra_context() {
    with_deepseek_cassette("corpus_matrix/shaping_extra_context", |client| async move {
        run_world(&wire(&client), &cells::SHAPING_EXTRA_CONTEXT, |log| {
            crate::ecs_goldens::golden_effects("deepseek_shaping_extra_context", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_shaping_extra_context_streamed",
                        log,
                    )
                },
            )
...
async fn shaping_merged_three() {
    with_deepseek_cassette("corpus_matrix/shaping_merged_three", |client| async move {
        run_world(&wire(&client), &cells::SHAPING_MERGED_THREE, |log| {
            crate::ecs_goldens::golden_effects("deepseek_shaping_merged_three", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::SHAPING_ROUTE_ON_FIRST_TURN, |log| {
                crate::ecs_goldens::golden_effects("deepseek_shaping_route_on_first_turn", log)
            })
            .await;
        },
    )
...
async fn shaping_late_route() {
    with_deepseek_cassette("corpus_matrix/shaping_late_route", |client| async move {
        run_world(&wire(&client), &cells::SHAPING_LATE_ROUTE, |log| {
            crate::ecs_goldens::golden_effects("deepseek_shaping_late_route", log)
        })
        .await;
    })
    .await;
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects(
                        "deepseek_shaping_max_tokens_second_turn",
                        log,
                    )
                },
            )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects("deepseek_shaping_preamble_second_turn", log)
                },
            )
            .await;
        },
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::SHAPING_HISTORY_FIRST_TURN, |log| {
                crate::ecs_goldens::golden_effects("deepseek_shaping_history_first_turn", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::CAUSAL_COMPLETION_SERIAL, |log| {
                crate::ecs_goldens::golden_effects("deepseek_causal_completion_serial", log)
            })
            .await;
        },
    )
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(
                ...
                |log| {
                    crate::ecs_goldens::golden_effects("deepseek_causal_completion_concurrent", log)
                },
            )
            .await;
        },
    ...
    with_deepseek_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::CAUSAL_COMPLETION_STREAMED, |log| {
                crate::ecs_goldens::golden_effects("deepseek_causal_completion_streamed", log)
            })
            .await;
        },
    )
