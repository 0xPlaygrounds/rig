...
) -> (ProviderCassette, Bound<ResponsesApi>) {
...
async fn chatgpt_noninteractive_oauth_cassette(
    spec: impl Into<CassetteSpec>,
) -> (ProviderCassette, Bound<ResponsesApi>, TempDir) {
    let cassette = ProviderCassette::start(
        &crate::cassettes::cassette_root(),
        "chatgpt",
        spec,
    ...
    // The credential exchange is not a wire: `ResponsesApi` holds an
