...
async fn raw_try_into_matches_completion() {
    ...
    with_cohere_cassette(
        ...
        |client| async move {
            ...
            // Same response, two views: the captured payload typed as Cohere's
            // own says what the normalized response says. There is one
            // mapping now, so the honest assertion is provider-native field
            // against normalized field, not a second normalization compared
            // with the first.
            let typed = CompletionResponse::deserialize(&second.raw)
