...
/// provider's wire on the bundled transport, and an agent over it.
#[test]
fn completion_client_single_import_surface() {
    use rig::prelude::*;

    // Pairing a wire with the bundled transport performs no network call, so
    ...
    let http = rig::rig_reqwest::bundled().expect("the bundled transport builds");
    let _model = Model::new(openai.completion("gpt-4o"), http.clone());
    let _agent = AgentBuilder::new(Model::new(openai.completion("gpt-4o"), http)).build();
}
...
fn completion_provider_explicit_facade_import_surface() {
    ...
    let http = rig::rig_reqwest::bundled().expect("the bundled transport builds");
    let _model = Model::new(openai.completion("gpt-4o"), http.clone());
    let _agent = AgentBuilder::new(Model::new(openai.completion("gpt-4o"), http.clone())).build();
    let _extractor =
        ExtractorBuilder::<Extracted>::new(Model::new(openai.completion("gpt-4o"), http)).build();
}
