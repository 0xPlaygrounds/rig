...
The two scenarios independently execute the original
...
The original goldens helper has explicit record/regeneration modes. Regression runs use replay mode. Native-golden generation runs only
the selected native producers against replayed HTTP; ecs_goldens first compares with
...
delivery batches are omitted from stable golden equality because they depend on
scheduling; originals have no delivery metadata.
