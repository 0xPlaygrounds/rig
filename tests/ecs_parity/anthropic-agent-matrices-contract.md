...
The four original agent exceptions in reasoning_usage_matrix, stop_sequence_terminal_matrix and empty_stop_sequence_matrix execute through native ECS systems and the real provider bus. Original source edits widen sibling visibility only. The batch pins exact IDs, original helper sources and fixtures.
...
No legacy agent runner or completion record executes in the native tests. Original undeclared turn budgets stay undeclared. These are recorded workloads, not proof of all provider settings or aggregate programme completion. Machine-readable assertion/fixture inventory is tracked separately from semantic review and paired execution.
