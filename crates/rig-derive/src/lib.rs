...
pub(crate) fn rig_agent_path() -> proc_macro2::TokenStream {
...
fn is_tool_context_type(ty: &Type) -> bool {
    ...
    let resolved_agent = match proc_macro_crate::crate_name("rig-agent") {
        Ok(proc_macro_crate::FoundCrate::Itself) => Some("crate".to_string()),
        Ok(proc_macro_crate::FoundCrate::Name(name)) => Some(name),
        Err(_) => None,
    };
    let resolved_facade = match proc_macro_crate::crate_name("rig") {
        Ok(proc_macro_crate::FoundCrate::Itself) => Some("crate".to_string()),
        Ok(proc_macro_crate::FoundCrate::Name(name)) => Some(name),
        Err(_) => None,
    };
    ...
    matches!(
        ...
        [root, tool, context]
            if (matches!(root.as_str(), "rig" | "rig_agent")
                || resolved_agent.as_deref() == Some(root.as_str())
                || resolved_facade.as_deref() == Some(root.as_str()))
                && tool == "tool"
                && context == "ToolContext"
    )
}
