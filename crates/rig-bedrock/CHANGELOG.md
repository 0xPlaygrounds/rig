...
- Bedrock **Mantle** OpenAI-compatible client (`rig_bedrock::mantle`):
  ...
  - Short-term IAM bearer token mint (TTL = min(12h, source AWS session); cache keyed by region + access key id)
  ...
  - Unversioned model constants (`openai.gpt-oss-20b`, Luna/Sol/Terra, …); versioned ids documented as Runtime/Converse-only
  - Example: `agent_with_bedrock_mantle`
  - Cassette + live smokes for GPT-OSS and GPT-5.6 Luna/Sol/Terra
