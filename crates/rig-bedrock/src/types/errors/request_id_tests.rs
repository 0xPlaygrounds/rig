...
fn a_request_id_attaches_to_provider_responses_only() {
    let attached =
        ProviderError::ProviderResponse(ProviderResponseError::without_status("aws said no"))
            .with_provider_request_id(Some(
                rig_core::id::RequestId::new("aws-req-1".to_string()).expect("a non-empty id"),
            ));
    ...
    let untouched = ProviderError::Provider("rig diagnostic".to_string()).with_provider_request_id(
        Some(rig_core::id::RequestId::new("aws-req-1".to_string()).expect("a non-empty id")),
    );
