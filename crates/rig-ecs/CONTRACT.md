...
| `ToolPolicy { concurrency }` (the run's, else the agent's, else 1) | `release_batch`, before the bus's `Gate`, holds (`Held` + `BatchHeld`) every tool child beyond the concurrency and releases them in call order as earlier ones land; 1 is serial. It lifts only its own holds: a `Held` a `Gate` system wrote stands until that system removes it, and the call under it keeps its slot | `rig_agent::AgentRunner::tool_concurrency` docs; `anthropic_serving_concurrent_concurrency_two` (the trace does not change) |
