...
| host model assembly | provider-specific constructors or optional rig-core registry configuration → `CompletionModel` → `ModelAdapter::completion` → `Handlers::register_erased`; no provider recipe or credential resolver in ECS |
