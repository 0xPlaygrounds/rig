...
//! | a sibling reorder re-renders | `a_sibling_reorder_re_renders` |
...
use rig_ecs::{
    agent::{
        Failed, Failure, Grant, MaxTurns, MessageParts, Run, Settled, Turn, Utterance,
...
impl Fixture {
    ...
    fn turn(&mut self) -> (CompletionRequest, AssemblyStats) {
        ...
        self.world.spawn((Turn, Fresh, ChildOf(self.run)));
...
#[test]
fn a_sibling_reorder_re_renders() {
    let mut f = Fixture::new();
    f.turn_expecting(3);
    let results = f.utterances()[1];
    let result = f.part::<ToolResultPart>(results);
    let json = parts_of::<JsonPart>(&f.world, result)[0];
    f.world.entity_mut(result).insert_children(0, &[json]);
    let second = f.turn_expecting(1);
    let Message::User { content } = &second.chat_history[1] else {
        panic!("the results");
    };
...
fn reparenting_a_part_re_renders_both_utterances() {
    ...
    f.world.entity_mut(text).insert(ChildOf(prompt));
...
fn a_component_removed_from_a_live_entity_is_seen() {
    ...
    f.world.spawn((Turn, Fresh, ChildOf(f.run)));
...
fn a_request_part_edit_is_rendered_uncached_and_the_view_survives() {
    ...
    let prompt = f.utterances()[2];
    let text = f.part::<TextPart>(prompt);
    let turn = f.world.spawn((Turn, Fresh, ChildOf(f.run))).id();
    f.world.spawn((
...
fn streaming_and_unary_assemble_the_same_request() {
    ...
    let before = stats(&f.world);
    f.world.spawn((Turn, Fresh, ChildOf(streamed)));
    f.world.run_schedule(RigSchedule);
...
fn observe(
    ...
    for (effect, turn) in &effects {
        ...
        let fresh = children
            ...
            .into_iter()
            .flat_map(|children| children.iter())
            .filter(|child| utterances.get(*child).is_ok())
            .map(|entity| content.message(entity).unwrap().to_message())
            .collect();
        observed
            .0
