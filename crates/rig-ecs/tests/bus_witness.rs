...
//! | scene restore retains a bare barrier with explicitly unknown ownership | `restored_hold_is_unknown_until_the_host_reevaluates_it` |
//! | a judge replacement exposes the recorded and the consumed answer | `a_judge_replacement_exposes_both_answers` |
//! | a layer's patch keeps its before; a layer's denial is a serve-side fact | `layer_patches_are_recorded_and_denials_are_witnessed` |
//! | a stream that ends before its terminal keeps its tail | `a_stream_that_ends_before_its_terminal_keeps_its_tail` |
//! | driver refusals and deferrals are facts, once | `driver_refusals_are_witnessed_under_intake_limits` |
...
#[test]
fn empty_and_error_first_streams_keep_error_outcomes() {
...
fn explicit_operations_keep_retry_identity_and_current_dispatch_subjects() {
    ...
    register(
        &mut app,
        "model",
        ModelAdapter::completion("test-model", model),
    );
...
#[test]
fn driver_refusals_are_witnessed_under_intake_limits() {
    let counters = Arc::new(Counters::default());
...
fn same_pass_parent_is_kept_in_fallback_adapter_and_layer_facts() {
    use rig_core::{
        completion::CompletionModel as _, serve::adapters::ModelAdapter,
        test_utils::RecordingHttpClient,
    };
    let mut app = app();
    let log = witnessed(&mut app);
    ...
    register(
        &mut app,
        "parent",
        ModelAdapter::completion("test-model", model.clone()),
    );
    register(
        &mut app,
        "child",
        ErasedHandler::new(ModelAdapter::completion("test-model", model)).layered(Warmer),
    );
