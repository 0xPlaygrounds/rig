...
///   dispatch context), and starts one initial task, owned by the effect
///   entity in `Serving`, with an empty [`Streamed`] for a streaming consumer;
    ...
    for (entity, _, effect, reserved, inputs, operation) in candidates {
        ...
        match served {
            Served::Task(handler) => {
                ...
                executions.as_mut().own_task(&mut entity_commands, task);
                if effect.is_stream() {
