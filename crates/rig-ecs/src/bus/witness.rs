...
//! deferred by the intake bound, refused by the driver, replaced after the
...
/// The driver deferred this intent at least once: the deferral was
/// observed then, not on every pass it stays deferred. Runtime-only.
#[derive(Component, Debug, Clone, Copy, Default)]
pub struct Deferred;
...
/// The dispatcher's view of the witness: the sink, the subjects, and which
/// intents were already observed deferred. One system parameter, so the
...
pub struct DispatchWitness<'w, 's> {
    ...
    /// Intents already observed deferred.
    pub deferred: Query<'w, 's, (), With<Deferred>>,
}
