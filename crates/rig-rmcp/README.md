...
- `McpTool` — one MCP server tool; convert it into a `rig_core::tool::DynamicTool` (`From`) and register it wherever dynamic tools go (`rig-agent`'s `AgentBuilder::dynamic_tool`, `ToolServer::dynamic_tool`, or any other runtime). The dynamic tool carries a liveness probe bound to the MCP transport, so registries can retire it on disconnect. `tools_from_server` converts a whole tool list.
