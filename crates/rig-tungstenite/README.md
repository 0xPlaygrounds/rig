...
The backend itself enables no provider. Enable this crate's `openai` feature
for the OpenAI session construction traits and prelude, together with the
existing TLS selection. Through the facade, enable both `rig/openai` and
`rig/websocket`. Generic WebSocket contracts and the bundled transport remain
usable without the OpenAI feature.
