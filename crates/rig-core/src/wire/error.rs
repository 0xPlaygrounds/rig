...
/// What a credential-less request says, for every error enum that carries
/// the variant.
///
/// Rust has no macro expansion in enum-variant position, so the three
/// declarations (two hand-written, one generated by `provider_error_enum!`)
/// cannot be folded into one. The *text* can be, and is: a hand-written copy
/// drifting from a generated one is how `VerifyError` came to report a local
/// refusal as the provider's verdict.
pub(crate) fn missing_credential_message(env_var: &str) -> String {
    format!(
