...
//! and later denied it, a layer patched a request, a judge replaced an
...
//! decision, and the typed [`Action`] with its before/after data or a
//! structured [`Reason`].
//!
...
//! and [`compare`] ignores it. Nothing here reads a clock or a random
//! source on its own.
...
use crate::{
    effect::{EffectFamily, EffectId, EffectKind, HandlerKey, Outcome},
    error::ErrorReport,
...
pub enum Stage {
    /// Before dispatch: a policy patched, held, released or denied an intent.
    Gate,
    /// The driver took, deferred or refused an intent.
    Dispatch,
    ...
    Judge,
    /// The agent runtime: turns, batches, retries, endings.
    Runtime,
...
pub enum Action {
    ...
    /// A held intent was approved by a policy (the release follows).
    Approved {
        /// The approving policy's reason, when it gave one.
        reason: Option<Reason>,
    },
    /// A request was rewritten before a handler served it. Both forms are
    /// the effect kind's serde value: what began and what was served (the
    /// record holds the latter as the request).
    Patched {
        /// What began.
    ...
    Issued,
    /// The driver left an intent for a later pass.
    Deferred {
        /// Why (`intake_bound`, `serial_key_busy`).
        reason: Reason,
    },
    ...
    /// A cancellation was requested for a program.
    CancelRequested {
        /// The requester's reason.
        reason: Reason,
    },
    /// A turn was retried with feedback.
    Retry {
        /// The feedback the model reads.
        feedback: Option<String>,
    },
    /// A call to an unadvertised tool was resolved.
    InvalidCall {
        /// The tool the model named.
        name: String,
        /// How it was resolved (`fail`, `ignore`, `retry`, `repair`, `skip`).
        resolution: Reason,
    },
    /// A program ended.
    Ended {
...
/// The most bytes one observation payload (a patched kind, a truncation
/// tail) may hold in its serialized form. A larger value is elided to a
...
/// `value`, or an `{"elided": {"bytes": n}}` marker when its serialized
/// form exceeds [`LARGEST_PAYLOAD_BYTES`].
pub fn bounded(value: serde_json::Value) -> serde_json::Value {
    let bytes = serde_json::to_vec(&value).map_or(usize::MAX, |bytes| bytes.len());
    if bytes > LARGEST_PAYLOAD_BYTES {
        serde_json::json!({ "elided": { "bytes": bytes } })
    } else {
        value
    }
}
...
impl Action {
    /// A patch observation from the two kinds, each bounded by
    /// [`LARGEST_PAYLOAD_BYTES`]; an unserializable kind is an error, never
    /// a silently empty field.
    pub fn patched(before: &EffectKind, after: &EffectKind) -> Result<Self, serde_json::Error> {
        Ok(Self::Patched {
            before: bounded(serde_json::to_value(before)?),
            after: bounded(serde_json::to_value(after)?),
        })
    }
