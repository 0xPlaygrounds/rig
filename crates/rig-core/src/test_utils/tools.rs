...
use crate::{
    tool::{Tool, ToolContext, ToolExecutionError, ToolExecutionErrorKind, ToolSet},
...
pub struct MockToolExecutionError;
...
///
/// `Tool::call` records `session:<id>` (or `no-session` when no
/// [`SessionId`] is present). The plain `call` body records `call-no-context` as
/// a sentinel: because an overridden `Tool::call` is the single dispatch
...
/// A tool that always fails by constructing a [`ToolExecutionError`] with a configured
/// [`ToolExecutionErrorKind`]. Used to exercise structured
...
impl MockFailingTool {
    ...
    pub fn new(kind: ToolExecutionErrorKind) -> Self {
        Self { kind }
    }
}
...
impl Tool for MockFailingTool {
    ...
    async fn call(
        ...
        Err(match self.kind {
            ToolExecutionErrorKind::NotFound => error.with_http_status(404),
            ToolExecutionErrorKind::RateLimited => error.with_http_status(429),
            _ => error,
        })
    }
}

/// A tool that returns a classified [`ToolExecutionError`] with separate model feedback.
/// Registered under the name `lookup`.
#[derive(Clone)]
...
/// [`ToolExecutionError::denied`]), producing a [`ToolExecutionErrorKind::Denied`](crate::tool::ToolExecutionErrorKind::Denied)
/// outcome — as opposed to a hook `skip action`, which is `Skipped`. Registered
...
impl Tool for MockDeniedTool {
    ...
    async fn call(
        ...
        Err(
            ToolExecutionError::denied("internal authorization check refused the call")
                .with_model_feedback("access to this resource is not permitted"),
        )
    }
}

/// Cloneable metadata a [`MockMetadataTool`] attaches to its result through
/// [`ToolContext`], to verify metadata reaches hooks without being sent to the model.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct MockRequestId(pub String);

/// A tool whose success carries a [`MockRequestId`] as result metadata.
