...
mod tests {
    ...
    async fn test_build_multiple_text() {
        ...
        assert_eq!(
            second_definition.1[1].document, "A fictional creature found in the distant, swampy marshlands of the planet Glibbo in the Andromeda galaxy.".to_string()
        )
    }
    ...
    async fn test_build_single_text() {
        ...
        assert_eq!(
            second_definition.1[0].document, "An ancient tool used by the ancestors of the inhabitants of planet Jiro to farm the land.".to_string()
        )
    }
