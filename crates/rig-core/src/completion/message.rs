...
// ================================================================
// Message content emptiness
// ================================================================

/// A content list was empty where at least one item is required.
#[derive(Debug, thiserror::Error)]
#[error("Empty list error: expected at least one item")]
pub struct EmptyListError;

/// Reject an empty content list, with the caller's own error.
///
/// Message content was `OneOrMany<T>` until this migration: an empty list was
/// unrepresentable, so every producer had to prove non-emptiness to the type
/// system and every consumer got that proof for free. `Vec<T>` carries no such
/// proof, so the check it used to perform by construction has to happen
/// somewhere explicit — this is that somewhere.
///
/// The error is a closure rather than a fixed type because the call sites are
/// provider response parsers, and each one already has a diagnostic that names
/// the wire it was reading. Collapsing those into one generic message would
...
/// assert!(require_non_empty(Vec::<&str>::new(), || EmptyListError).is_err());
/// ```
