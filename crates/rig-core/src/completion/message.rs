...
/// nothing to send — and several call sites depend on that rejection as control
/// flow, not merely as a diagnostic (`chatgpt::normalized_completion` matches on
/// it to rebuild from the raw event stream).
///
...
///
/// The guards check **whole-list** emptiness, never per-item emptiness: a
/// visibly empty item can carry data that must survive the round trip (Gemini
/// attaches thought signatures and OpenAI attaches encrypted reasoning to
/// blocks whose visible text is empty). Do not tighten them to reject empty
/// items.
