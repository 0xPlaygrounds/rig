...
impl CompletionRequest {
    ...
    fn normalized_documents_from(documents: &[Document]) -> Option<Message> {
        ...
        (!messages.is_empty()).then_some(Message::User { content: messages })
    }
...
impl<M: CompletionModel> CompletionRequestBuilder<M> {
    ...
    fn into_model_and_request(self) -> (M, CompletionRequest) {
        ...
        // Pushing the prompt above is what makes this non-empty, so the old
        // single-item fallback here was unreachable.
