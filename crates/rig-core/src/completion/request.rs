...
//! [`CompletionRequest`] into their native HTTP request format. Higher-level
//! prompting and orchestration APIs are supplied by runtime crates.
//!
...
/// classic runtime prompting errors.
///
/// ```
...
pub trait CompletionModel: Clone + WasmCompatSend + WasmCompatSync {
    ...
    /// (e.g. OpenAI, Anthropic) override this to `true`, which lets runtime
    /// output-mode selection keep native structured output when tools are present.
