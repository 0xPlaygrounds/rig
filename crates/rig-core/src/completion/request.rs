...
//! [`CompletionRequest`] into their native HTTP request format. Runtime-level
//! prompting and orchestration live outside `rig-core`.
//!
...
/// the high-level runtime's prompt error.
///
/// ```
...
pub trait CompletionModel: Clone + WasmCompatSend + WasmCompatSync {
    ...
    /// (e.g. OpenAI, Anthropic) override this to `true`, allowing runtimes to use
    /// guaranteed native structured output even when tools are advertised.
