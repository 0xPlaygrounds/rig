...
/// Deserialize a wire `tool_calls` array, dropping the calls whose `arguments`
/// were truncated mid-JSON rather than failing the whole response.
///
/// An OpenAI-compatible provider still emits the tool call when the output
/// budget runs out mid-arguments: the turn comes back with
/// `finish_reason: "length"` and `arguments` cut off partway through the object.
/// Verified live against DeepSeek at 24, 32, 48 and 64-token budgets
/// (`{"summary": `, `{"summary": "Log this incident: the`, …) and previously
/// against Mistral at 24/32/48/64/96. Parsing that strictly failed the *whole*
/// response — the text, usage, id, model and finish reason went with it — where
/// the streaming path keeps the turn and drops the unusable call
/// ([`UnparseableToolInput::Drop`](crate::streaming::UnparseableToolInput)).
///
/// The drop happens here, at the array, rather than by mapping the arguments to
/// a sentinel value, because no sentinel can be told apart from a payload that
/// legitimately parses to the same thing. `null` in particular is unavailable:
/// Groq spells a parameterless call `"arguments": "null"` (every zero-argument
/// call in
/// `tests/cassettes/groq/agent_tool_sessions/parallel_tool_calls_single_turn_nonstreaming.yaml`
/// carries it), so a `null` sentinel drops real calls — and, because a
/// normalized call is echoed back on the next turn, it also changes what rig
/// sends.
///
/// Only an unparseable `arguments` *string* is a reason to drop. Any other
/// decode failure (a missing `id`, an unknown `type`) is a response defect and
/// still fails deserialization, so this stays a truncation tolerance rather
/// than a blanket "skip whatever does not fit".
pub fn drop_truncated_tool_calls<'de, D, T>(deserializer: D) -> Result<Vec<T>, D::Error>
where
    D: Deserializer<'de>,
    T: serde::de::DeserializeOwned,
{
    fn arguments_are_truncated(call: &serde_json::Value) -> bool {
        call.get("function")
            .and_then(|function| function.get("arguments"))
            .and_then(serde_json::Value::as_str)
            .is_some_and(|raw| parse_tool_arguments(raw).is_err())
    }

    let raw = Option::<Vec<serde_json::Value>>::deserialize(deserializer)?.unwrap_or_default();
    let mut calls = Vec::with_capacity(raw.len());
    for call in raw {
        let truncated = arguments_are_truncated(&call);
        match serde_json::from_value::<T>(call) {
            Ok(call) => calls.push(call),
            Err(_) if truncated => {
                tracing::debug!("dropping a tool call whose arguments were truncated mid-JSON");
            }
...
mod tests {
    ...
    fn wire_calls(json: &str) -> serde_json::Result<Vec<WireCall>> {
        #[derive(serde::Deserialize)]
        struct Holder {
            #[serde(default, deserialize_with = "drop_truncated_tool_calls")]
            tool_calls: Vec<WireCall>,
        }
        Ok(serde_json::from_str::<Holder>(json)?.tool_calls)
    }

    /// The truncated call goes; its complete sibling stays. These are the exact
    /// `arguments` DeepSeek returned at the 56-token parallel-call budget.
    #[test]
    fn drop_truncated_tool_calls_keeps_the_complete_sibling() {
        let calls = wire_calls(
            r#"{"tool_calls":[
                {"id":"a","function":{"name":"page_oncall","arguments":"{\"team\": \"platform\"}"}},
                {"id":"b","function":{"name":"file_report","arguments":"{\"summary\": "}}
            ]}"#,
        )
        .expect("a truncated entry must not fail the array");

        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].function.name, "page_oncall");
        assert_eq!(
    ...
    /// Every parseable spelling survives untouched — `"null"` above all, which
    /// is how Groq spells a parameterless call and therefore why the drop
    /// cannot be expressed as a sentinel value.
    #[test]
    fn drop_truncated_tool_calls_leaves_parseable_payloads_alone() {
        let calls = wire_calls(
            r#"{"tool_calls":[
                {"id":"a","function":{"name":"empty","arguments":""}},
                {"id":"b","function":{"name":"object","arguments":"{}"}},
                {"id":"c","function":{"name":"groq_null","arguments":"null"}}
            ]}"#,
        )
        .expect("nothing here is truncated");

        assert_eq!(calls.len(), 3);
        assert_eq!(calls[0].function.arguments, serde_json::json!({}));
        assert_eq!(calls[1].function.arguments, serde_json::json!({}));
        assert_eq!(calls[2].function.arguments, serde_json::Value::Null);
    }

    /// A missing array, and a null one, are both "no calls" rather than errors.
    #[test]
    fn drop_truncated_tool_calls_tolerates_absent_and_null_arrays() {
        assert!(wire_calls(r#"{}"#).expect("absent").is_empty());
        assert!(
            wire_calls(r#"{"tool_calls":null}"#)
                .expect("null")
                .is_empty()
        );
    }

    /// Anything that is not a truncated `arguments` string is still a response
    /// defect: the drop must not become "skip whatever does not fit".
    #[test]
    fn drop_truncated_tool_calls_still_rejects_other_decode_failures() {
        let error =
            wire_calls(r#"{"tool_calls":[{"function":{"name":"no_id","arguments":"{}"}}]}"#)
                .expect_err("a missing id is a defect, not a truncation");
        assert!(
            error.to_string().contains("id"),
            "the underlying decode error is preserved: {error}"
        );
    }
