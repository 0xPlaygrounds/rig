...
macro_rules! forward_prompt_setters {
    ($recv:ident) => {
        ...
        /// via [`Tool::call`](crate::tool::Tool::call),
...
mod tests {
    ...
    use crate::{
        agent::{
            AgentBuilder,
            hook::{
                AgentHook, CompletionResponseEvent, HookContext, InvalidToolCallAction,
                InvalidToolCallContext, ObserveAction, ToolCallAction, ToolCallEvent,
            },
        },
        completion::{
    ...
    /// Context values persist for the whole run, across *multiple* tool-call
    /// rounds. The model calls the probe in two consecutive
    ...
    /// Without a context, the same tool runs with an empty one (no panic and no
    /// stale value).
    #[tokio::test]
    async fn tool_runs_with_empty_context_when_none_supplied() {
        ...
        assert_eq!(out, "done");
        // The canonical call receives an empty context when none was supplied.
        assert_eq!(probe.observed().as_deref(), Some("no-session"));
    }

    /// Direct typed calls use the same canonical context-aware method as runtime
    /// dispatch, including the empty-context behavior.
    #[tokio::test]
    async fn probe_direct_call_uses_empty_context() {
        let probe = MockExtensionsProbeTool::default();
        let out = probe
