...
fn caches(response: MockHttpResponse) -> crate::driver::Model<CachedContents, SequencedHttpClient> {
    crate::driver::Model::new(crate::providers::gemini::Gemini::new("test-key").cached_contents(), SequencedHttpClient::new(vec![response]))
}
...
async fn a_status_error_with_no_body_still_carries_its_status() {
    ...
    // responses run out.
    let caches = crate::driver::Model::new(crate::providers::gemini::Gemini::new("test-key").cached_contents(), SequencedHttpClient::new(Vec::new()));
