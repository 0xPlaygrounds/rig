...
mod tests {
    ...
    use crate::providers::openai::responses_api::{
        ...
        ResponsesUsage, terminal_metadata_from_incomplete_details,
    };
    use futures::{SinkExt, StreamExt};
    use serde_json::json;
    use std::time::Duration;
    ...
    fn terminal_response_accepts_completed_and_incomplete_statuses() {
        ...
        let mut incomplete_response = sample_response(ResponseStatus::Incomplete);
        incomplete_response.incomplete_details = Some(IncompleteDetailsReason {
            reason: "max_output_tokens".to_string(),
        });
        let incomplete = terminal_response_result(incomplete_response)
            .expect("incomplete response should retain partial output");
        let metadata =
            terminal_metadata_from_incomplete_details(incomplete.incomplete_details.as_ref())
