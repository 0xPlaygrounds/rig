...
use crate::{completion, message, require_non_empty};
...
impl ReasoningSummary {
    /// Build a summary list from plain summary strings.
    ///
    /// A constructor rather than a `From` impl: now that the source is a
    /// `Vec`, `impl From<Vec<String>> for Vec<ReasoningSummary>` names no
    /// local type and the orphan rule rejects it.
    pub fn many_from_strings(value: Vec<String>) -> Vec<Self> {
...
impl TryFrom<ResponsesRequestParams> for CompletionRequest {
    ...
    fn try_from(params: ResponsesRequestParams) -> Result<Self, Self::Error> {
        ...
        let input = require_non_empty(input, || {
...
mod tests {
    ...
    fn file_id_document_serializes_as_input_file_content() {
        ...
        let json = serde_json::to_value(&content[0]).expect("serialize content");
