...
use crate::{completion, json_utils, message, require_non_empty};
...
impl Message {
    /// Convert generic user content into OpenAI chat messages.
    ///
    /// A constructor rather than a `TryFrom` impl: now that message content is
    /// a `Vec`, `impl TryFrom<Vec<message::UserContent>> for Vec<Message>`
    /// names no local type and the orphan rule rejects it.
    pub fn try_from_user_content(
        ...
        fn flush_user_content(
            ...
            let content = require_non_empty(std::mem::take(pending), || {
                message::MessageError::ConversionError(
    ...
    /// Convert generic assistant content into OpenAI chat messages.
    ///
    /// A constructor rather than a `TryFrom` impl, for the same orphan-rule
    /// reason as [`Message::try_from_user_content`].
    pub fn try_from_assistant_content(
...
impl TryFrom<message::Message> for Vec<Message> {
    ...
    fn try_from(message: message::Message) -> Result<Self, Self::Error> {
        match message {
            ...
            message::Message::User { content } => Message::try_from_user_content(content),
            message::Message::Assistant { content, .. } => {
                Message::try_from_assistant_content(content)
            }
        }
    }
}
        ...
        Ok(match message {
            ...
            Message::Assistant {
                ...
                message::Message::Assistant {
                    id: None,
                    content: require_non_empty(assistant_content, || {
                        message::MessageError::ConversionError(
...
impl crate::completion::NormalizeCompletionResponse for CompletionResponse {
    fn normalize(self, provider: &str) -> Result<completion::CompletionResponse, CompletionError> {
        ...
        let choice = require_non_empty(content, || {
...
mod tests {
    ...
    fn mixed_user_content_preserves_order_around_tool_results() {
        ...
        let messages = Message::try_from_user_content(content).expect("message conversion");
    ...
    fn assistant_reasoning_alone_is_dropped() {
        ...
        let converted =
            Message::try_from_assistant_content(assistant_content).expect("conversion should work");
