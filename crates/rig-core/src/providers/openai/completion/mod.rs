...
where
    S: Serializer,
{
    if let [UserContent::Text { text, .. }] = content.as_slice() {
...
///
/// A free function rather than a `TryFrom` impl: with `Vec` on both sides the
/// orphan rule rejects the impl, since neither type is local to this crate.
...
///
/// A free function for the same orphan-rule reason as
