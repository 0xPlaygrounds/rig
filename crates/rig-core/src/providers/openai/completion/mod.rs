...
pub enum Message {
    ...
    User {
        #[serde(
            deserialize_with = "crate::json_utils::string_or_vec",
...
/// Convert a user turn's content blocks into the wire's message sequence.
///
/// A free function rather than `TryFrom<Vec<_>> for Vec<_>`: with the content
...
mod tests {
    ...
    fn minted_tool_ids_replay_as_a_consistent_pair() {
        ...
        let crate::message::Message::Assistant {
        ...
        let assistant_wire: Vec<super::Message> =
            assistant_content_to_messages(assistant_content).expect("assistant converts");
