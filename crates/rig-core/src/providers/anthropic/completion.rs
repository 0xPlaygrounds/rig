...
pub fn apply_cache_control(system: &mut [SystemContent], messages: &mut [Message]) {
    ...
    if let Some(last_msg) = messages.last_mut() {
        // `last_mut()` is an `Option` now: a message with no content blocks
        // has nothing to mark, and spending the cache breakpoint on it would
        // silently waste it.
        if let Some(last_block) = last_msg.content.last_mut() {
            set_content_cache_control(last_block, Some(CacheControl::ephemeral()));
        }
    }
}
...
fn apply_message_cache_control(
    ...
    // A message with no content blocks has nothing to mark; spending a
    // breakpoint on it would consume one of the few available markers for
    // nothing, so the counter only decrements when a block was actually marked.
    if let Some(last_msg) = messages.last_mut()
...
mod tests {
    ...
    fn test_plaintext_roundtrip_rig_to_anthropic_and_back() {
        ...
        match (&original, &back) {
            ...
            ) => match (orig_content.iter().next(), back_content.iter().next()) {
                (
    ...
    fn web_search_tool_result_error_object_is_preserved_raw() {
        ...
        assert!(
            matches!(round_trip.content.first(), Some(Content::WebSearchToolResult {
                    tool_use_id,
                    content
                }) if tool_use_id == "srvtoolu_01"
    ...
    fn code_execution_tool_result_is_preserved_and_round_trips() {
        ...
        .unwrap();
        assert!(
            matches!(round_trip.content.first(), Some(Content::CodeExecutionToolResult {
                    tool_use_id,
                    content
                }) if tool_use_id == "srvtoolu_01"
                    && content["type"] == "code_execution_result"
    ...
    fn document_additional_params_forward_to_anthropic_document() {
        ...
        let block = converted.content.into_iter().next();
    ...
    fn anthropic_document_metadata_survives_reverse_round_trip() {
        ...
        let Some(Content::Document {
            ...
            citations,
            ..
        }) = round_trip.content.into_iter().next()
        else {
