...
impl ToolOutput {
    ...
    pub fn as_text(&self) -> Option<&str> {
        // A slice pattern states "exactly one block, of this shape" directly,
        // so the separate length guard the non-empty type needed is gone.
...
mod tests {
    ...
    fn structured_values_remain_json_until_terminal_rendering() {
        ...
        let content = output.into_content();
        assert!(matches!(
            content.first(), Some(ToolResultContent::Json { value: content_value }) if content_value == &value));
    }
    ...
    fn explicit_image_content_preserves_its_type() {
        ...
        let content = output.into_content();
        assert!(matches!(
            content.first(), Some(ToolResultContent::Image(image)) if image.media_type == Some(ImageMediaType::JPEG)
