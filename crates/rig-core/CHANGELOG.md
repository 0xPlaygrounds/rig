...
- *(agent)* in-flight **message injection**: feed a message into a running agent from outside the loop. `AgentRunner::message_injector()` (also on `PromptRequest`/`StreamingPromptRequest`) returns a cloneable, `Send` `MessageInjector` bound to the run; `injector.inject(impl Into<Message>)` queues a message that the driver folds into the conversation immediately before the next model call (any tool calls already in flight on the current turn finish first). The message is delivered as its **own user turn** — never merged into a preceding tool-result message, which some providers' converters (e.g. OpenAI) silently drop. To keep the resulting conversation valid everywhere, a new shared `providers::coalesce` pass merges adjacent same-role **user** turns on each provider's converted message list (so an injected user turn next to a tool-result user turn, RAG documents before the prompt, or a hoisted-system gap never sends two consecutive user messages); it is applied across the in-core providers and `rig-bedrock` — required by AWS Bedrock's strictly-alternating Converse API and harmless (a no-op for already-alternating histories) elsewhere. Parallel tool-result messages are never coalesced. Delivery is **best-effort and in-flight**: if the run finishes before that next model call, the message is not delivered (a warning is logged) and a subsequent `inject` returns `MessageInjectError::RunFinished`; injection does not resurrect a completed run. The mechanism is split across the two layers — the sans-IO `AgentRun` owns folding (`inject_message`), a plain serializable transition a hand-driver can call, while the `AgentRunner` owns the async `futures::channel::mpsc` transport drained before each step — so it behaves identically on the blocking and streaming drivers. The "fold input between steps" point follows pydantic-ai's `AgentRun.enqueue`, Vercel AI SDK's `prepareStep`, and LangGraph's `Command(resume=…)`, without requiring a checkpointer. ([#1969](https://github.com/0xPlaygrounds/rig/pull/1969), closes [#1858](https://github.com/0xPlaygrounds/rig/pull/1858))
- *(agent)* [**breaking**] unified, composable hook system integrated into the agent run loop. A single-method `AgentHook<M>` trait (`async fn on_event(&self, event: StepEvent<'_, M>) -> Flow`) replaces the 8-method `PromptHook`. Hooks compose via `HookStack` and run in registration order (the first to return a non-`Continue` `Flow` short-circuits the rest). A new `AgentRunner<M>` driver — obtained with `agent.runner(prompt)` and run via `.run().await` (blocking) or `.stream()` (incremental) — pairs the sans-IO `AgentRun` state machine with hooks, model IO, tool execution and memory; both `agent.prompt(..)` and `agent.stream_prompt(..)` are now thin wrappers over it. Attach/compose hooks with `.add_hook(h)` (append-only; call it again to stack more) on the agent builder, the prompt request, or the runner. This closes the gap where hand-driving `AgentRun` had no hook support.
...
- *(agent)* concurrent tool execution on the **streaming** driver, gated behind the existing `tool_concurrency(n)` knob (default `1` = unchanged), bringing it to parity with the blocking path. `tool_concurrency(n)` is now exposed on `StreamingPromptRequest` (alongside `PromptRequest`/`AgentRunner`), and both drivers share one call-ordered `buffered` executor, so the persisted message history is identical regardless of `concurrency` or driver. A turn with several independent slow tools (HTTP/MCP) now finishes in ≈`max` rather than ≈`sum` under streaming. At `concurrency > 1` the stream emits all of a turn's `ToolCall` items (call order) and then the `ToolResult`s, rather than strictly interleaving one pair per tool; the default `1` keeps the existing interleaved, byte-identical behavior. ([#1955](https://github.com/0xPlaygrounds/rig/issues/1955))

### Changed

- *(anthropic)* the streaming completion path now honors the request's `tool_choice` instead of hardcoding `auto`, so a caller's tool choice (including one set per-turn via `Flow::OverrideRequest`) takes effect under `stream_prompt`/streaming as it already did on the blocking path. A `tool_choice` Anthropic cannot represent (a multi-name `ToolChoice::Specific`) now surfaces as a request error on both the streaming and blocking paths instead of being silently downgraded to `auto`. ([#1966](https://github.com/0xPlaygrounds/rig/pull/1966))
- *(agent)* [**breaking**] `Flow` no longer implements `Eq` (it remains `PartialEq`), because `Flow::OverrideRequest` carries a `RequestOverride` with an `f64` `temperature`. Code that relied on `Flow: Eq` (e.g. an `Eq` derive on a type embedding `Flow`, or an `Eq`/`Hash` bound) must drop that requirement. ([#1966](https://github.com/0xPlaygrounds/rig/pull/1966))
...
- *(agent)* [**behavioral**] observable changes from the unified runner (the default `tool_concurrency` of 1 is unaffected by the ordering change):
  - Tool results now land in **tool-call order** under `tool_concurrency(>1)`: the blocking path switched from `buffer_unordered` to `buffered`, so the persisted history/memory order is deterministic and matches the streaming path. The previous completion-order was non-deterministic, so nothing could rely on it.
