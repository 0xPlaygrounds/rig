...
- *(agent)* `AgentRunner::into_stepper()` returns an `AgentStepper<M>` that hand-drives the agent loop one `AgentRunStep` at a time (`step() -> StepReport`) with hooks, memory, invalid-tool-call recovery and tracing all firing — the gap where raw `AgentRun` stepping had no hooks. The blocking `AgentRunner::run()` is now a thin fold over the stepper, so both share exactly one core ([#1947](https://github.com/0xPlaygrounds/rig/issues/1947), [#1946](https://github.com/0xPlaygrounds/rig/issues/1946) item 3).
- *(agent)* typed step-control layer for the run loop ([#1947](https://github.com/0xPlaygrounds/rig/issues/1947)): each observable step now encodes the `Flow` actions it admits as a distinct `Control` type (`ObserveControl` / `ToolControl` / `RecoveryControl`), produced by an exhaustive, fallback-free `Step::parse`. The driver's three fail-closed resolvers fold through this typed layer with a single, type-derived diagnostic instead of per-event hand-written strings; `on_event(StepEvent) -> Flow` remains the ergonomic hook surface, and the typed controls are exposed (and round-trip `Into<Flow>`) for advanced hooks. Foundation for the unified-driver work tracked in [#1947](https://github.com/0xPlaygrounds/rig/issues/1947) / [#1946](https://github.com/0xPlaygrounds/rig/issues/1946).

### Fixed

- *(agent)* best-effort structured-output extraction (`Prompted` / non-JSON fallback) now scans every JSON opener in the model's text and returns the first value that deserializes as the target type, so a leading array no longer shadows a later object the caller wanted ([#1946](https://github.com/0xPlaygrounds/rig/issues/1946)).
...
- *(agent)* [**behavioral**] observable changes from the unified runner (the default `tool_concurrency` of 1 is unaffected by the ordering change):
  ...
  - Streaming `StreamResponseFinish` now fires on **every non-recovered turn**, including turns that emit only a tool call or only reasoning (it was previously gated on streamed assistant text). This makes it a one-to-one match for the blocking `CompletionResponse`, so a hook observing model responses now sees identical coverage across `run()` and `stream()` ([#1947](https://github.com/0xPlaygrounds/rig/issues/1947), [#1946](https://github.com/0xPlaygrounds/rig/issues/1946) item 2). The streamed final *item* forwarded to the consumer is unchanged (still only on text turns), so output and history are unaffected.
