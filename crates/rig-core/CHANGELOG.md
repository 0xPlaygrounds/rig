...
- *(completion)* [**breaking**] message content is `Vec<T>`, not `OneOrMany<T>`: `Message::User.content`, `Message::Assistant.content` and `ToolResult.content` are plain `Vec`s and the `one_or_many` module is deleted. The container bought "no content is unrepresentable" at the price of a bespoke collection whose method names read like slice methods without behaving like them — `first()` returned an owned `T` rather than `Option<&T>`, and `is_empty()` always returned `false`, so every branch testing it was dead — while the invariant it protected was still violable one layer down by providers fabricating an empty-text part to satisfy the constructor. Where the type rejected empty input, `message::require_non_empty(items, || error)` now does, taking the caller's own error so per-wire diagnostics survive; `EmptyListError` moves to `message::`. Serialization is unchanged (the container already emitted a bare sequence), so request bodies, persisted histories and recorded fixtures are byte-identical — pinned by new tests in `completion/message.rs`. See MIGRATING for the method-translation table
- *(completion)* [**behavior**] message content decoding accepts two shapes it used to reject, both now yielding an empty list where they were local parse errors: `[]`, which the removed container rejected outright, and `null` on the six fields that moved onto `json_utils::string_or_vec` (anthropic `Message.content` and `Content::ToolResult.content`; the openai chat and Responses-API `System`/`User` content). `string_or_vec` keeps the `visit_none`/`visit_unit` arms its predecessor lacked because the openai assistant-content field already using it depends on them for OpenAI's `"content": null` tool-calls-only messages; its `visit_map` arm — a bare object decoding as one element, which several providers send where an array is also legal — is now pinned by tests in `json_utils`
- *(tool)* [**behavior**] a tool declaring `type Output = Vec<ToolResultContent>` compiles unchanged and sends a different payload. `IntoToolOutput`'s rich-content guard, which keeps an image from silently becoming a JSON object, used to name `OneOrMany<ToolResultContent>`; it now names `Vec<ToolResultContent>`, so such an output takes the rich path (N ordered content blocks, images as image parts) instead of falling through to serialization as one JSON tool result. An empty `Vec<ToolResultContent>` yields a contentless `ToolOutput` where it used to yield `json([])`
- *(openai)* [**breaking**] three conversions are inherent constructors rather than trait impls, forced by the orphan rule: with `Vec<X>` as the source neither type is crate-local (E0117). `Message::try_from_user_content`, `Message::try_from_assistant_content` and `ReasoningSummary::many_from_strings` replace the former `TryFrom`/`From` impls with identical bodies; conversions whose source is a rig-owned type are unaffected
