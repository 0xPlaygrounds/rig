...
- *(tool)* [**breaking**] flattened the tool authoring/runtime API. Tool authors no longer implement `Tool::definition(prompt)`, and object-safe tools no longer expose `ToolDyn::definition()`. Implement `Tool::description()` and `Tool::parameters()` alongside `Tool::NAME`; Rig now generates provider-facing `ToolDefinition` values from registered tools at request boundaries. `Tool::NAME` / `Tool::name()` / `ToolDyn::name()` is the single source of truth for dispatch and provider-advertised tool names, eliminating mismatches between registered tool identity and `ToolDefinition.name`.
