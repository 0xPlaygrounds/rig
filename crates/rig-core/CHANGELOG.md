...
- *(agent)* concurrent tool execution in the **streaming** driver, reaching parity with the blocking path's `tool_concurrency`. `StreamingPromptRequest::tool_concurrency(n)` now exists (delegating to `AgentRunner::tool_concurrency`); with `n > 1` the streaming agent loop runs a turn's tool calls concurrently, so a turn of several independent slow tools finishes in ≈`max` rather than ≈`sum` of their latencies. Both drivers share one call-ordered `buffered` helper (`execute_tools_buffered`), so the persisted message history is identical regardless of concurrency. The default of 1 is unchanged — the streaming driver keeps its sequential, interleaved `ToolCall`/`ToolResult` emission; under `n > 1` it yields all of a turn's `ToolCall` items first, then the `ToolResult`s (both in call order). ([#1956](https://github.com/0xPlaygrounds/rig/pull/1956), closes [#1955](https://github.com/0xPlaygrounds/rig/issues/1955))
