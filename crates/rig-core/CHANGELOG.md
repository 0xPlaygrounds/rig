...
- *(core)* [**breaking**] API paper-cut cleanups — collapse duplicate public names onto one canonical spelling per concept ([#2047](https://github.com/0xPlaygrounds/rig/issues/2047)):
  ...
  - The low-level `crate::streaming::stream_to_stdout` (which takes a `StreamingCompletionResponse`) is renamed to `stream_completion_to_stdout` to disambiguate it from the high-level `agent::stream_to_stdout` (which drives a `StreamingResult`); the latter is unchanged.
