...
- *(core)* [**breaking**] the response-identity fields on `StreamFinal`, `completion::CompletionResponse` and `completion::ResponseIdentity` — `message_id`, `response_id`, `provider_request_id` — are now `Option<WireId>` instead of `Option<String>`, so the empty-string-is-absent rule is enforced by the type rather than by the `with_*_id` setters. `streaming::StreamingCompletionResponse`'s `message_id` (released as `Option<String>` in v0.41.0) and `message::Reasoning`'s `id` are retyped with them — the latter closing a gap inherited from `main`, where a wire `""` could reach a replayed request body. `Reasoning::with_id` widens to `impl Into<String>` and normalizes. `WireId` gains `PartialOrd`/`Ord` for parity with `ToolCallId`. Serialized JSON is unchanged and a stored `""` still loads as `None`. `model` stays `Option<String>` ([#2336](https://github.com/0xPlaygrounds/rig/issues/2336))
