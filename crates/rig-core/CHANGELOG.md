...
- *(tool)* [**breaking**] make tool identity structurally single-source. Typed tools keep `Tool::NAME`, `description()`, and `parameters()` but no longer have `Tool::name()`. Public `ToolDyn` is now a concrete named dynamic tool value; the erased runtime executor is internal and nameless. Provider-facing `ToolDefinition`s are generated from typed `Tool::NAME`, concrete `ToolDyn` names, or registered `ToolSet` keys, so runtime executors cannot recompute or diverge from dispatch identity. The stored name is readable via `ToolDyn::name()` / `ToolEmbeddingDyn::name()` and renameable before registration via `ToolDyn::with_name(..)` (e.g. to register one implementation under instance-specific names); after registration the `ToolSet` map key is the only identity. `ToolDynBuilder` accepts a structured executor via `call_structured(..)` returning a `ToolExecutionResult`, so dynamic tools can emit precise outcomes (`denied`, timeout/cancelled failures) that the string executors cannot express; when it is the only executor, the string call paths are derived from it. Also note: `impl<T: Tool> From<T> for ToolDyn` permanently forecloses `impl Tool for ToolDyn` (trait coherence) — intentionally, so a dynamic tool can never masquerade as a typed one with a divergent `NAME`.
