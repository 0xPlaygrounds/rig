...
- *(message)* [**breaking**] `OneOrMany<T>` and `EmptyListError` are removed; message content, tool results, `CompletionResponse::choice`, `CompletionRequest::chat_history` and `Vec<Embedding>` are plain `Vec<T>`. The serialized format is unchanged (`OneOrMany` already emitted a sequence and decoded only sequences), so persisted histories and stored embeddings need no migration — this is a source-only break. `one_or_many::string_or_one_or_many` is superseded by `json_utils::string_or_vec`, which additionally maps `null` to an empty list; `OneOrMany::merge`, `string_or_option_one_or_many` and `first_mut` had no callers and are deleted
- *(message)* [**behavior**] an assistant turn that carried no content aggregates to an EMPTY choice. Five sites used to fabricate a single `AssistantContent::text("")` sentinel purely to satisfy the non-empty container (`PartsAccumulator::finish`, anthropic's empty `end_turn`, agent tool-call resolution, candle's protocol parse, ollama's assistant conversion); consumers detecting an empty turn must check `is_empty()` instead of matching the sentinel. Histories persisted by earlier versions still carry it and are still recognised
- *(completion)* [**breaking**] an invalid `CompletionRequest` is unconstructible: the fields are private, `CompletionRequest::new(CompletionRequestParts)` is the only way to build one, and it rejects an empty `chat_history` and empty `User`/`Assistant` message content naming the offending role and index — replacing the non-empty container's context-free constructor error. Field reads become accessor calls (`request.tools` → `request.tools()`), by-value consumption goes through `into_parts()`, `CompletionRequestBuilder::build` returns `Result`, and deserialization routes through the constructor via `#[serde(try_from = "CompletionRequestParts")]` so persisted requests with empty content fail to deserialize (serialization is byte-identical). `System` content is a `String` the removed type never constrained and stays unchecked
- *(test-utils)* [**breaking**] `MockTurn::from_contents` returns `Self` instead of `Result<Self, EmptyListError>`
