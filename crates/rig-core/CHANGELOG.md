...
- *(message)* [**behavior**] an assistant turn that carried no content aggregates to an EMPTY choice. Seven sites used to fabricate a single `AssistantContent::text("")` sentinel purely to satisfy the non-empty container; consumers detecting an empty turn must check `is_empty()` instead of matching the sentinel. Histories persisted by earlier versions still carry it and are still recognised
- *(completion)* [**breaking**] empty request content is rejected at the boundary by `CompletionRequest::validate_message_content` (run from the builder's `send`/`stream` and from every agent model call), naming the offending message's role and index — replacing the non-empty container's context-free constructor error. It covers an empty `chat_history`, empty `User`/`Assistant` content, and an empty `ToolResult::content`; `System` content is a `String` the removed type never constrained and stays unchecked
- *(providers)* [**breaking**] the OpenAI chat conversions `TryFrom<OneOrMany<UserContent>>`/`TryFrom<OneOrMany<AssistantContent>> for Vec<Message>` become the free functions `user_content_to_messages`/`assistant_content_to_messages` (with `Vec` on both sides the orphan rule rejects the impl); `From<OneOrMany<String>> for Vec<ReasoningSummary>` had no callers and is deleted
- *(test-utils)* [**breaking**] `MockTurn::from_contents` returns `Self` instead of `Result<Self, EmptyListError>`; `MockCompletionModel::from_turns_and_stream_turns` is added for tests that drive both surfaces
