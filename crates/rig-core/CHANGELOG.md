...
- *(completion)* [**breaking**] empty message content is rejected at the request boundary by `CompletionRequest::validate_message_content` (run from the builder's `send`/`stream` and from every agent model call), naming the offending message's role and index — replacing the non-empty container's context-free constructor error. `User` and `Assistant` content only: `System` content is a `String` the removed type never constrained. Per-wire conversion guards are unchanged
