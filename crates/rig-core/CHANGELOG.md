...
- *(telemetry)* [**breaking**] opt-in message-content telemetry for GenAI spans. `CompletionRequestBuilder::record_message_telemetry(bool)`, `AgentBuilder::record_message_telemetry(bool)`, `PromptRequest::record_message_telemetry(bool)`, `TypedPromptRequest::record_message_telemetry(bool)`, `StreamingPromptRequest::record_message_telemetry(bool)`, and `AgentRunner::record_message_telemetry(bool)` can explicitly enable recording model input/output message contents to `gen_ai.input.messages` / `gen_ai.output.messages` on providers and agent drivers that support those message fields. The default remains disabled to avoid leaking prompts/responses and high-cardinality telemetry; provider paths that previously recorded message contents now honor the same opt-in. Low-level request builders set the opt-in flag on the built request, while providers and higher-level agent drivers own the actual span attributes; low-level streaming output recording remains provider- or agent-driver-dependent because stream aggregation happens while the returned stream is consumed. Source-breaking API change: `CompletionRequest` has a new public `record_message_content` field (serde-defaulted and skipped when false for wire compatibility).
