...
- *(tool)* deferred tools + a built-in `tool_search` meta-tool, the replacement for vector-backed dynamic tools and the answer to "more tools than fit in a prompt". Register a tool with `AgentBuilder::deferred_tool(..)` / `ToolServer::deferred_tool(..)` and it is executable but **withheld** from the advertised tool set — zero schema tokens and zero `definition()` calls — until the model discovers it by calling the built-in `tool_search` tool with a capability query. Once a `tool_search` result names a tool, it is advertised on subsequent turns and dispatched like any other tool. Reveal state is reconstructed from the conversation transcript (`revealed_deferred_tools`), so runs stay stateless/resumable and the request prefix stays prompt-cache friendly. The search strategy is pluggable via the async `ToolServer::tool_search_fn(..)` (default: keyword-overlap over each deferred tool's name + description; swap for an async BM25/embeddings/LLM picker via `ToolSearchFn`). This is the pattern Pydantic AI (`defer_loading` + `ToolSearch`) and the OpenAI Agents SDK (`defer_loading` + `ToolSearchTool`) converge on. `ToolServerHandle::get_tool_defs` now takes the revealed-tool set (`&BTreeSet<String>`) instead of an optional RAG query string. The `tool_search` name is **reserved**: registering a user tool with that name is rejected (`ToolServerError::ReservedToolName` on the `Result`-returning `ToolServerHandle` methods, or a warning + skip on the infallible builders) rather than being silently shadowed by the built-in.
...
- [**breaking**] the core vector-store abstraction and built-in RAG are removed; retrieval becomes a user-land pattern (a tool or a hook), not a built-in API. This makes Rig's core smaller and more composable — the stance Pydantic AI, the Vercel AI SDK, and the OpenAI Agents SDK all take — while keeping embedding models/builders (`rig::embeddings`) intact.
  - **`rig-core::vector_store` deleted.** `VectorStoreIndex`, `VectorStoreIndexDyn`, `InsertDocuments`, `VectorStoreError`, `VectorSearchRequest`, the `SearchFilter`/`Filter` DSL, `InMemoryVectorStore`/`InMemoryVectorIndex`, `IndexStrategy`, and the LSH support are gone, along with the blanket `impl Tool for T: VectorStoreIndex` (`search_vector_store`).
  - **Built-in `dynamic_context` RAG removed.** `AgentBuilder::dynamic_context`, `ExtractorBuilder::dynamic_context`, the `DynamicContextStore` alias, and the vector-search step in the completion-request builder are gone. Inject retrieved context from an `AgentHook` via `RequestPatch::extra_context` (passive RAG), or expose retrieval as an ordinary `Tool` the model calls (active RAG). See the new `hook_passive_rag` and `tool_active_rag` examples.
  - **Vector-backed dynamic tools removed**, replaced by deferred tools + `tool_search` (see Added). `ToolEmbedding`, `ToolEmbeddingDyn`, `ToolSet::schemas`, `ToolSetBuilder::dynamic_tool`, `AgentBuilder::dynamic_tools`, and `ToolServer::dynamic_tools` are gone.
  - **The 12 vector-store companion crates are removed** from the workspace (`rig-lancedb`, `rig-mongodb`, `rig-neo4j`, `rig-postgres`, `rig-qdrant`, `rig-s3vectors`, `rig-scylladb`, `rig-sqlite`, `rig-surrealdb`, `rig-vectorize`, `rig-milvus`, `rig-helixdb`) along with their `rig` facade features/modules. `rig-fastembed` (embedding model) stays.
