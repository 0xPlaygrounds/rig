...
- *(anthropic)* a streamed turn's `Usage::input_tokens` prefers the terminal `message_delta` and falls back to `message_start`, instead of always reading `message_start`. Anthropic proper reports the count on both frames and they agree, so nothing changes there; Anthropic-*compatible* gateways need not, and OpenRouter's Messages endpoint sends `input_tokens: 0` on `message_start` with the real count on `message_delta` — which silently surfaced as `Usage { input_tokens: 0 }`, worse than a missing value for a consumer sizing a context window from it. A zero on the delta is read as "not reported" so a gateway with the inverse split cannot erase a count `message_start` got right
