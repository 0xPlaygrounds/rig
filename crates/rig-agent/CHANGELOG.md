...
- *(agent)* [**behavior**] an empty assistant turn is no longer an error. The run loop rejected one as "model turn lost its assistant content", and `retry_model_turn` rejected a content-less rejected turn — guards that could not fire while providers fabricated an empty-text sentinel, and that fired on exactly the legitimate cases once it was removed (an Anthropic turn ending empty after a tool result, a textless tool-call turn). Such turns are dropped from history by `is_empty_assistant_turn`, which now recognises both a genuinely empty choice and the legacy sentinel
- *(agent)* [**behavior**] every model call validates request content before the round trip, failing with the offending turn's role and index instead of a provider 400
