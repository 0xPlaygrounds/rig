...
- *(agent)* [**breaking**] bump rmcp from 2.2 to 3.0 — `Meta` split into `RequestMetaObject`/`MetaObject` (both re-exported at `rig::tool::rmcp`), MCP server handlers return the SEP-2322 outcome enums (`CallToolResponse`, …); see the 0.41 → 0.42 section of MIGRATING.md
