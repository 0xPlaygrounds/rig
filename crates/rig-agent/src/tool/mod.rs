...
//! [`ToolResultContent`](rig_core::message::ToolResultContent) or a
//! `Vec` of content blocks can also be used directly as a typed tool
//! output without being mistaken for ordinary JSON.
//!
//! ```
//! use rig_core::{
...
pub trait Tool: Sized + WasmCompatSend + WasmCompatSync {
    ...
    /// and a `Vec` of them preserve rich content when returned directly;
    /// use [`ToolOutput`] when constructing the presentation explicitly.
