...
pub use rig_core::tool::{
    IntoToolOutput, Tool, ToolErrorKind, ToolExecutionError, ToolOutput, ToolResult, builtin,
};
...
/// A classic-runtime tool with mutable per-dispatch context.
///
...
pub trait ContextualTool: Sized + WasmCompatSend + WasmCompatSync {
...
pub trait ToolEmbedding: ContextualTool {
    ...
    T: ContextualTool,
{
    ...
    fn description(&self) -> String {
        ContextualTool::description(self)
    }

    fn parameters(&self) -> serde_json::Value {
        ContextualTool::parameters(self)
    }
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            match ContextualTool::call(self, context, args).await {
                Ok(output) => match output.into_tool_output() {
                    Ok(output) => ToolResult::success(output),
                    Err(error) => ToolResult::failed(error),
                },
                Err(error) => ToolResult::failed(ContextualTool::map_error(self, error)),
            }
        })
    }
}

struct PortableTool<T>(T);

impl<T> ErasedTool for PortableTool<T>
where
    T: Tool,
{
    fn name(&self) -> String {
    ...
    fn description(&self) -> String {
        Tool::description(&self.0)
    }

    fn parameters(&self) -> serde_json::Value {
        Tool::parameters(&self.0)
    }
...
struct PortableDynamicTool(rig_core::tool::DynamicTool);

impl ErasedTool for PortableDynamicTool {
    fn name(&self) -> String {
        self.0.name().to_string()
    }

    fn description(&self) -> String {
        self.0.definition().description
    }

    fn parameters(&self) -> serde_json::Value {
        self.0.definition().parameters
    }
...
/// A runtime-defined tool accepted by classic registries.
///
/// Both portable [`rig_core::tool::DynamicTool`] values and classic contextual
/// [`DynamicTool`] values implement this registration adapter.
pub trait DynamicToolRegistration: Sized {
    /// Insert this tool into a classic runtime set and return its name.
    fn register(self, tools: &mut ToolSet) -> String;
}

impl DynamicToolRegistration for DynamicTool {
    fn register(self, tools: &mut ToolSet) -> String {
        tools.insert(RegisteredTool::Static(Arc::new(self)))
    }
}

impl DynamicToolRegistration for rig_core::tool::DynamicTool {
    fn register(self, tools: &mut ToolSet) -> String {
        tools.insert(RegisteredTool::Static(Arc::new(PortableDynamicTool(self))))
    }
}

/// Generate the provider-facing definition for a typed tool.
pub fn tool_definition<T: ContextualTool>(tool: &T) -> ToolDefinition {
    ToolDefinition {
...
impl ToolSet {
    ...
    pub fn from_dynamic_tools<T>(tools: Vec<T>) -> Self
    where
        T: DynamicToolRegistration,
    {
        let mut set = Self::default();
        for tool in tools {
    ...
    /// Register a portable context-free tool through the classic adapter.
    pub fn add_portable_tool<T>(&mut self, tool: T) -> String
    where
        T: Tool + 'static,
    {
        self.insert(RegisteredTool::Static(Arc::new(PortableTool(tool))))
    }
...
impl ToolSetBuilder {
    ...
    /// Add a portable context-free tool through the classic adapter.
    pub fn portable_tool<T>(mut self, tool: T) -> Self
    where
        T: Tool + 'static,
    {
        self.tools.add_portable_tool(tool);
        self
    }
    ...
    where
        T: DynamicToolRegistration,
    {
        self.tools.add_dynamic_tool(tool);
        self
    }
    ...
    where
        T: ToolEmbedding + 'static,
    {
        self.tools.insert(RegisteredTool::Embedding(Arc::new(tool)));
        self
    }
...
mod tests {
    ...
    impl ContextualTool for PendingTool {
    ...
    impl ContextualTool for ForeignErrorTool {
    ...
    impl ContextualTool for ClassifiedErrorTool {
    ...
    impl ContextualTool for DirectRichOutput {
    ...
    impl ContextualTool for TypedRichError {
...
mod migrated_tests {
    ...
    fn retrieved_tool_schemas_use_canonical_name() {
        ...
        impl ContextualTool for RetrievedTool {
    ...
    async fn json_shaped_string_output_stays_literal_text_through_dispatch() {
        ...
        impl ContextualTool for JsonShapedStringTool {
    ...
    async fn null_args_are_normalized_to_empty_object() {
        ...
        impl ContextualTool for NoArgTool {
