...
pub use rig_core::tool::{
    IntoToolOutput, Tool, ToolEmbedding, ToolErrorKind, ToolExecutionError, ToolOutput, ToolResult,
};
...
/// A classic-runtime tool that explicitly receives mutable [`ToolContext`].
///
/// Use the portable [`Tool`] trait unless execution genuinely needs classic
/// inbound context or result metadata.
...
struct ContextualRegistration<T>(T);

impl<T> ErasedTool for ContextualRegistration<T>
where
    T: ContextualTool,
{
    fn name(&self) -> String {
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            match self.0.call(context, args).await {
                Ok(output) => match output.into_tool_output() {
                    Ok(output) => ToolResult::success(output),
                    Err(error) => ToolResult::failed(error),
                },
...
/// Generate the provider-facing definition for a classic contextual tool.
pub fn contextual_tool_definition<T: ContextualTool>(tool: &T) -> ToolDefinition {
    ToolDefinition {
        name: T::NAME.to_string(),
        description: tool.description(),
        parameters: tool.parameters(),
...
impl ToolSet {
    ...
    /// Register a classic contextual tool.
    pub fn add_contextual_tool<T>(&mut self, tool: T) -> String
    where
        T: ContextualTool + 'static,
    {
        self.insert(RegisteredTool::Static(Arc::new(ContextualRegistration(
            tool,
        ))))
    }
...
mod tests {
    ...
    async fn cancelled_toolset_dispatch_does_not_retain_stale_result_metadata() {
        ...
        set.add_contextual_tool(PendingTool(started.clone()));
        let mut context = ToolContext::new();
