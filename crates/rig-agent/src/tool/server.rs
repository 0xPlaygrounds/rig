...
use crate::{
    ...
    tool::{
        ContextualTool, DynamicToolRegistration, RegisteredTool, Tool, ToolContext, ToolDispatch,
...
impl ToolServer {
    ...
    pub fn tool(mut self, tool: impl ContextualTool + 'static) -> Self {
        self.toolset.add_tool(tool);
        self
    }

    /// Add a context-free portable tool through the classic adapter.
    pub fn portable_tool(mut self, tool: impl Tool + 'static) -> Self {
        self.toolset.add_portable_tool(tool);
        self
    }
...
impl ToolServerHandle {
    ...
    /// Register a context-free portable tool through the classic adapter.
    pub async fn add_portable_tool<T>(&self, tool: T)
    where
        T: Tool + 'static,
    {
        let mut state = self.0.write().await;
        let _name = state.toolset.add_portable_tool(tool);
        #[cfg(feature = "rmcp")]
        state.managed_generations.remove(&_name);
    }

    /// Register a runtime-defined static tool.
    pub async fn add_dynamic_tool<T>(&self, tool: T)
    where
        T: DynamicToolRegistration,
    {
        let mut state = self.0.write().await;
...
mod tests {
    ...
    use crate::{
        ...
        tool::{
            ContextualTool, ToolContext, ToolEmbedding, ToolExecutionError, ToolSet,
    ...
    impl ContextualTool for ReplacementTool {
    ...
    impl crate::tool::ContextualTool for ContextReader {
