...
impl ToolServer {
    ...
    #[doc(hidden)]
    pub fn add_tools(mut self, tools: ToolSet) -> Self {
        self.toolset = tools;
        self
    }

    #[doc(hidden)]
    pub fn add_retrieval_indexes(
        mut self,
    ...
    /// Add a classic contextual tool.
    pub fn contextual_tool(mut self, tool: impl crate::tool::ContextualTool + 'static) -> Self {
        self.toolset.add_contextual_tool(tool);
        self
    }
...
impl ToolServerHandle {
    ...
    #[doc(hidden)]
    pub async fn snapshot_tool_defs(
        &self,
        prompt: Option<String>,
...
mod tests {
    ...
    use crate::{
        ...
        tool::{
            ContextualTool, Tool, ToolContext, ToolEmbedding, ToolExecutionError, ToolSet,
    ...
    async fn context_reaches_the_single_execute_path() {
        let handle = ToolServer::new().contextual_tool(ContextReader).run();
    ...
    impl crate::tool::ContextualTool for PendingTool {
    ...
    async fn cancelled_server_dispatch_does_not_retain_stale_result_metadata() {
        ...
        let handle = ToolServer::new()
            .contextual_tool(PendingTool(started.clone()))
            .run();
        let mut context = ToolContext::new();
