...
pub struct ScenarioReport {
    ...
    /// Completed model operations observed by the scenario.
    pub model_calls: usize,
    /// Distinct invalid-call policy decisions exercised by the scenario.
...
fn report_from_response(
    ...
    Ok(ScenarioReport {
        name,
        tool_calls,
        model_calls: usize::from(tool_calls > 0) + 1,
        policy_decisions: 0,
...
mod tests {
    ...
    async fn execute_all_classic_structured_modes() -> usize {
        let mut completed = 0;
        for mode in [OutputMode::Native, OutputMode::Prompted, OutputMode::Auto] {
            let model = MockCompletionModel::text(
                r#"{"first_name":"Ada","last_name":"Lovelace","job":"mathematician"}"#,
            );
            let response = AgentBuilder::new(model)
                ...
                .extended_details()
                .await
                .expect("structured text mode");
            serde_json::from_str::<ExtractedPerson>(&response.output)
                .expect("structured text output");
            completed += 1;
        }
        ...
        let response = AgentBuilder::new(model)
            ...
            .extended_details()
            .await
            .expect("structured tool mode");
        serde_json::from_str::<ExtractedPerson>(&response.output).expect("structured tool output");
        completed + 1
    }
    ...
    fn concrete_conformance_evidence(scenario: Scenario) -> Outcome {
        let report = match scenario {
            Scenario::ToolPairing | Scenario::Concurrency => {
                ...
                .expect("fixture");
                futures::executor::block_on(parallel_tools(
                    MockCompletionModel::new([first, MockTurn::text("7 and 8")]),
                    |builder| builder,
                    Some(1),
                ))
            ...
            Scenario::InvalidToolRecovery | Scenario::ToolSuppression => {
                futures::executor::block_on(invalid_tool_recovery(
                    MockCompletionModel::new([MockTurn::tool_call(
                        "invalid-add",
                        "add",
                        serde_json::json!({ "x": 2, "y": 3 }),
                ...
                .expect("invalid-tool report")
            }
            Scenario::StopAndCancellation | Scenario::ModelCallBudgets => {
                futures::executor::block_on(cancellation_and_max_turns(
                    MockCompletionModel::new([
                        MockTurn::tool_call(
                            "cancel-add",
                            "add",
                ...
                .expect("run-control report")
            }
            Scenario::StructuredOutput => {
                futures::executor::block_on(structured_extraction(MockCompletionModel::new([
                    MockTurn::tool_call(
                        "submit_call",
                        "submit",
                        serde_json::json!({
            ...
            _ => {
                ...
                futures::executor::block_on(streaming_tool(model, |builder| builder))
                    .expect("streaming report")
            }
        };
        let usage = Usage {
            input_tokens: report.prompt_tokens,
            output_tokens: report.generated_tokens,
            total_tokens: report.prompt_tokens + report.generated_tokens,
            ..Usage::new()
        };
        Outcome {
    ...
    impl Adapter for ClassicAdapter {
        ...
        fn run(&mut self, scenario: Scenario) -> Result<Outcome, Self::Error> {
            ...
            let observations = match scenario {
                Scenario::ModelCallBudgets => {
                    cancellation_and_max_turn_controls_retain_diagnostics()
                        .expect("budget conformance");
                    [
                ...
                Scenario::CanonicalTranscript => {
                    streaming_contract_checks_events_history_and_usage()
                        .expect("streaming conformance");
                    [
                ...
                Scenario::ToolPairing => {
                    parallel_contract_validates_batch_and_correlation()
                        .expect("parallel conformance");
                    [Observation::CallsPaired, Observation::CommitOrderStable].into()
                }
                Scenario::UsageAccounting => {
                    streaming_contract_checks_events_history_and_usage()
                        .expect("accounting conformance");
                    [Observation::AccountingIdempotent].into()
                }
                Scenario::InvalidToolRecovery => {
                    invalid_recovery_paths_do_not_execute_tools()
                        .expect("invalid-tool conformance");
                    [
                ...
                Scenario::ResponseRetryRollback => {
                    crate::agent::runner::migrated_tests::span_safety_net::response_retry_records_only_accepted_content_on_both_surfaces();
                    crate::agent::run::tests::model_turn_retry_rejects_tool_calls_without_advancing_to_execution();
                    [
                ...
                Scenario::StopAndCancellation => {
                    cancellation_and_max_turn_controls_retain_diagnostics()
                        .expect("cancellation conformance");
                    [
                ...
                Scenario::StructuredOutput => {
                    executed_output_modes =
                        futures::executor::block_on(execute_all_classic_structured_modes());
                    extraction_contract_requires_fields_and_usage()
                        .expect("structured conformance");
                    typed_validators_reject_bad_structured_output_and_protocol_leaks();
                    crate::agent::run::tests::tool_mode_finalizes_best_effort_when_output_retry_budget_exhausted();
                    [
                ...
                Scenario::Memory => {
                    crate::agent::prompt_request::tests::memory_loads_into_request_history();
                    crate::agent::prompt_request::tests::memory_appends_full_turn_after_success();
                    crate::agent::prompt_request::tests::memory_unchanged_on_provider_error();
                    [
                ...
                Scenario::BlockingStreamingParity => {
                    crate::agent::runner::migrated_tests::stream_and_run_same_message_history_for_parallel_tool_calls_under_concurrency();
                    [Observation::BlockingStreamingEqual].into()
                }
                Scenario::ProviderFinalExposure => {
                    streaming_contract_checks_events_history_and_usage()
                        .expect("provider-final conformance");
                    [
                ...
                Scenario::ToolSuppression => {
                    invalid_recovery_paths_do_not_execute_tools().expect("suppression conformance");
                    crate::agent::run::tests::invalid_tool_call_skip_suppresses_all_peer_executions(
                    );
                    [
                ...
                Scenario::Concurrency => {
                    parallel_contract_validates_batch_and_correlation()
                        .expect("concurrency conformance");
                    crate::agent::runner::migrated_tests::structured_tool_results::concurrent_tools_preserve_order_and_both_outcomes(
                    );
                    crate::agent::runner::migrated_tests::stream_and_run_same_message_history_for_parallel_tool_calls_under_concurrency();
                    [
                ...
                Scenario::StaleResultHandling => {
                    cancellation_and_max_turn_controls_retain_diagnostics()
                        .expect("stale-result conformance");
                    [
    ...
    #[test]
    fn shared_conformance_ledger_passes_for_classic_adapter() {
