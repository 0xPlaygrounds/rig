...
/// A two-dimension embedding transport: each text embeds to its length and
/// 1.0.
#[derive(Clone)]
struct Tiny;

impl Tiny {
    fn wire() -> Local<rig_core::operation::Embedding> {
...
impl Transport<Local<rig_core::operation::Embedding>> for Tiny {
    ...
    > {
        let reply = EmbeddingResponse::new(
            texts
                .into_iter()
...
fn bus() -> (
    ...
    driver
        .register(
            "embed",
            ModelAdapter::new("tiny", Model::new(Tiny::wire(), Tiny)),
        )
        .expect("register");
