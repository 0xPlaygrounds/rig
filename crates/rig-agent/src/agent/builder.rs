...
use crate::{
    ...
    message::ToolChoice,
    tool::{
        ContextualTool, DynamicTool, Tool, ToolSet,
    ...
    M: CompletionModel,
{
    ...
    /// Add a tool that uses classic mutable runtime context.
    pub fn contextual_tool<T>(self, tool: T) -> AgentBuilder<M, WithBuilderTools>
    where
        T: ContextualTool + 'static,
    {
        let mut tools = ToolSet::default();
        tools.add_contextual_tool(tool);
        AgentBuilder {
            name: self.name,
            description: self.description,
            model: self.model,
    ...
    M: CompletionModel,
{
    ...
    /// Add another classic contextual tool.
    pub fn contextual_tool<T>(mut self, tool: T) -> Self
    where
        T: ContextualTool + 'static,
    {
        self.tool_state.tools.add_contextual_tool(tool);
        self
    }
