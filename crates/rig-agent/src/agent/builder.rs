...
use crate::{
    ...
    message::ToolChoice,
    tool::{
        ContextualTool, DynamicToolRegistration, Tool, ToolSet,
    ...
    M: CompletionModel,
{
    ...
    /// Add a context-free [`Tool`] through the classic runtime adapter.
    pub fn portable_tool<T>(self, tool: T) -> AgentBuilder<M, WithBuilderTools>
    where
        T: Tool + 'static,
    {
        let mut tools = ToolSet::default();
        tools.add_portable_tool(tool);
        AgentBuilder {
            name: self.name,
            description: self.description,
            model: self.model,
    ...
    pub fn dynamic_tool<T>(self, tool: T) -> AgentBuilder<M, WithBuilderTools>
    where
    ...
    pub fn dynamic_tools<T>(self, tools: Vec<T>) -> AgentBuilder<M, WithBuilderTools>
    where
        T: DynamicToolRegistration,
    {
    ...
    M: CompletionModel,
{
    ...
    /// Add another context-free [`Tool`] through the classic adapter.
    pub fn portable_tool<T>(mut self, tool: T) -> Self
    where
        T: Tool + 'static,
    {
        self.tool_state.tools.add_portable_tool(tool);
        self
    }
    ...
    /// Add runtime-defined tools to the agent.
    pub fn dynamic_tools<T>(mut self, tools: Vec<T>) -> Self
    where
        T: DynamicToolRegistration,
    {
        let tools = ToolSet::from_dynamic_tools(tools);
