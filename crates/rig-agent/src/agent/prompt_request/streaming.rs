...
fn finalize_streamed_choice(
    ...
    // A text item is pushed just above, so `items` is non-empty here.
    Some(items)
}
...
mod migrated_tests {
    ...
    fn validate_follow_up_tool_history(request: &CompletionRequest) -> Result<(), String> {
        ...
        if !matches!(
            ...
            Some(Message::User { content })
                if matches!(
                    content.first(), Some(UserContent::Text(text)) if text.text == "do tool work")
        ) {
            return Err(format!(
    ...
    async fn streaming_reasoning_without_tools_does_not_duplicate_final_history() {
        ...
        assert!(matches!(
            ...
            Some(Message::User { content })
                if matches!(
                    content.first(), Some(UserContent::Text(text)) if text.text == "think before answering")
        ));
