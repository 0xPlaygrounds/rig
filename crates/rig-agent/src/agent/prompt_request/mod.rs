...
// deserialize through the shadow would make serialize write a bare list
...
pub(crate) fn is_empty_assistant_turn(choice: &[AssistantContent]) -> bool {
    // Genuinely empty is the shape rig produces now. The single empty-text
    // part is the sentinel providers were forced to fabricate while the
    // choice could not be empty; persisted histories still carry it, so it
    // stays recognised.
    choice.is_empty()
        || (choice.len() == 1
            && matches!(
                choice.first(),
                Some(AssistantContent::Text(text)) if text.text.is_empty() && text.additional_params.is_none()
            ))
}
...
mod tests {
    ...
    fn prompt_response_roundtrip_preserves_explicit_content() {
        ...
        let Some(AssistantContent::Text(text)) = round.content().iter().next() else {
    ...
    fn prompt_response_serialize_and_deserialize_agree_on_wire_shape() {
        ...
        // bare `the removed non-empty container` while deserialize expects an `Option`, breaking
