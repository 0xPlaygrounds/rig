    ...
    pub fn resolve_invalid_tool_call(
        ...
        match action {
            ...
            InvalidToolCallAction::Repair { tool_name } => {
                ...
                {
                    let original_name = tool_call.function.name.clone();
                    tool_call.function.name = tool_name;
                    if tool_call.call_id.is_none() && tool_call.id == original_name {
                        tool_call.id = tool_call.function.name.clone();
                    }
                }
...
mod tests {
    ...
    #[test]
    fn invalid_tool_call_repair_updates_name_shaped_provider_id() {
        let mut run = AgentRun::new("call something").max_turns(2);

        expect_call_model(&mut run);
        expect_needs_resolution(
            run.model_response(tool_call_turn("default_api", "default_api"))
                .expect("model_response should succeed"),
        );
        expect_continue(
            run.resolve_invalid_tool_call(InvalidToolCallAction::repair("add"))
        ...
        let calls = expect_call_tools(&mut run);
        assert_eq!(calls[0].tool_call.function.name, "add");
        assert_eq!(calls[0].tool_call.id, "add");
    }
