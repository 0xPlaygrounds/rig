...
/// A scripted model: `answer` decides its reply from the request. It is the
/// transport behind a [`Local`] wire and answers with the turn's events, so
/// the blocking and streaming surfaces reply alike.
#[derive(Clone)]
struct Scripted {
    provider: &'static str,
    total_tokens: u64,
