...
async fn a_run_over_a_dropped_host_bus_answers_bus_closed_not_a_hang() {
    ...
    driver
        .register(
            "model:host",
            ModelAdapter::completion("host", MockCompletionModel::text("never")),
        )
        .expect("register");
...
async fn a_nested_call_over_the_calls_scope_under_serial_serving_fails_fast() {
    ...
    driver
        .register_erased(
            model_key.clone(),
            rig_core::serve::ErasedHandler::new(ModelAdapter::completion(
                "default",
                MockCompletionModel::from_turns([
                    call_same(),
                    call_same(),
...
async fn two_agents_on_one_host_bus_keep_their_own_keys() {
    ...
    for label in ["left-host", "right-host"] {
        driver
            .register(
                format!("model:{label}"),
                ModelAdapter::completion(label, two_tool_calls_then_done()),
            )
            .expect("register");
    }
...
async fn a_bus_failure_on_a_tool_dispatch_is_a_run_error_not_a_tool_result() {
    ...
    driver
        .register(
            "model:host",
            ModelAdapter::completion("host", two_tool_calls_then_done()),
        )
        .expect("register");
    let agent = AgentBuilder::over_bus(
        dispatcher.clone(),
...
async fn a_model_registered_through_the_parts_registrar_serves_the_next_run() {
    ...
    registrar
        .register(
            key.clone(),
            ModelAdapter::completion("late", MockCompletionModel::text("late")),
        )
        .expect("a fresh key");
    assert!(
        dispatcher.descriptor(&key).is_some(),
