...
The bundled [`reqwest`](https://docs.rs/reqwest) HTTP transport for [Rig](https://crates.io/crates/rig): the `HttpClientExt` implementation for `reqwest::Client` (and `reqwest_middleware::ClientWithMiddleware`) and the construction convenience (`DefaultTransport`) that builds rig-core's erased default transport over it, so `rig::providers::openai::wire::OpenAI::from_env()?.bound()?` works with no transport named.
