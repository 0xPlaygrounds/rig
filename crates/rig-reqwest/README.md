...
Provider selection belongs to `rig` or `rig-core`: for example, enable
`rig-core/openai` and import `rig_reqwest::prelude::*` to construct an OpenAI
client with the bundled transport. `rig-reqwest` enables no concrete provider.
