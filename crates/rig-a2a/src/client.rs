...
use crate::card::describe_card;
use crate::error::{A2AError, AgentCardError};
use crate::model::A2AModel;
use crate::thread::{ConversationId, ThreadStore};
...
impl A2AClientBuilder {
    ...
    /// Override the Rig agent name derived from the remote card.
    ///
    /// The default is a provider-safe slug of the card name. The same name is
    /// used if the resulting [`Agent`](rig_agent::agent::Agent) is converted
    /// into a dynamic tool with
    ...
    pub async fn build(self) -> Result<A2AClient, A2AError> {
        ...
        let agent_name = self
            .agent_name
            .unwrap_or_else(|| crate::parts::agent_name(&card.name));
        Ok(A2AClient {
            card,
            interface,
            inner: Arc::new(inner),
...
impl A2AClient {
    ...
    /// The name assigned to Rig agents built from this client.
    ///
    /// A slug of the remote card name unless overridden with
    /// [`A2AClientBuilder::agent_name`].
    ...
    ///
    /// Use [`A2AConversationExt`](crate::A2AConversationExt) to name a
    /// conversation per request: one agent can then serve many independent
    /// remote threads.
    ///
    /// ```no_run
    ...
    /// [`a2a_conversation`](crate::A2AConversationExt::a2a_conversation) unless
    /// the agent serves one conversation for its whole lifetime. A conversation
    /// named on the run outranks this binding either way.
    ///
    /// [`Agent`]: rig_agent::agent::Agent
