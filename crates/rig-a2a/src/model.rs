...
//! [`Agent::into_tool`] projects it as a sub-agent. Where [`A2AClient::tool`]
//! hands a *local* model a remote agent to call, this hands *your code* a
//! remote agent shaped like any other Rig model.
//!
...
//! | `temperature`, `max_tokens`, `additional_params`, `model` | Ignored (logged at debug). Sampling belongs to the remote agent. |
//!
...
//! 2. **Name it per run.** [`A2AClient::agent`] registers
//!    [`A2AThreadHook`], which carries the run's
//!    [`conversation`](rig_agent::agent::AgentRunner::conversation) id to the
//!    model, so one agent serves every conversation. This is the usual choice.
...
impl A2AModel {
    ...
    /// [`A2AClient`](crate::A2AClient), so a model and a tool using the same id
    /// continue the same remote conversation.
    ...
    fn build_request(&self, request: CompletionRequest) -> Result<Outbound, A2AError> {
        ...
        // conversation. `A2AThreadHook` puts the run's id here.
...
/// than one undifferentiated block. Non-text content has no A2A projection here
/// and is skipped; the tool bridge documents the same limitation.
...
fn text_choice(text: String) -> OneOrMany<AssistantContent> {
...
///
/// The tool surface reaches the same split through text, because a tool result
/// has no channel but its output; see `EMPTY_COMPLETION` in [`crate::tool`].
...
///
/// Unlike the tool surface, which has [`ToolErrorKind`] to classify with, a
/// completion model has one provider-error channel; the state name is kept in
/// the message so callers can still distinguish them.
///
/// [`ToolErrorKind`]: rig_core::tool::ToolErrorKind
fn failure_for(
    state: &a2a::TaskState,
...
mod tests {
    ...
    fn request() -> CompletionRequest {
        CompletionRequest {
            model: None,
            preamble: None,
            chat_history: OneOrMany::one(RigMessage::user("hello")),
            documents: vec![],
            tools: vec![],
            temperature: None,
            max_tokens: None,
