...
//!
//! [`A2AModel`](crate::model::A2AModel) collapses agent responses into a single
//! string, so non-text parts (Data, Url) are stringified into the joined buffer
//! with a separator. Binary `Raw` parts are skipped with a debug log because
//! they are not routable to text-only Rig agents.
...
/// Provider-safe Rig agent name derived from a remote agent's card name.
///
/// The name is capped at a provider-safe tool-name length because Rig also uses
/// it when the agent is converted into a dynamic tool. Slugs are ASCII, so byte
/// truncation is safe.
pub(crate) fn agent_name(card_name: &str) -> String {
    const MAX_LEN: usize = 64;

    let mut agent = slug(card_name);
    agent.truncate(MAX_LEN);
    agent
}

/// Wire-format label for a task state, used in finish reasons and errors.
...
pub(crate) fn status_text_limited(task: &Task, limit: usize) -> Result<String, A2AError> {
    task.status
        .message
        .as_ref()
        .map(|message| parts_to_text_limited(&message.parts, limit, "A2A response"))
        .transpose()
...
pub(crate) fn task_body_limited(task: &Task, limit: usize) -> Result<String, A2AError> {
    ...
    if !status.is_empty() {
        if !out.is_empty() {
            push_limited(&mut out, "\n", Some((limit, "A2A response")))?;
        }
        push_limited(&mut out, &status, Some((limit, "A2A response")))?;
    }
    Ok(out)
}
...
pub(crate) fn message_body_limited(message: &Message, limit: usize) -> Result<String, A2AError> {
    parts_to_text_limited(&message.parts, limit, "A2A response")
}

fn push_artifact(out: &mut String, artifact: &Artifact, limit: usize) -> Result<(), A2AError> {
    let text = parts_to_text_limited(&artifact.parts, limit, "A2A response")?;
    if text.is_empty() {
        return Ok(());
    }
    if !out.is_empty() {
    ...
    push_limited(out, &text, Some((limit, "A2A response")))
}
...
mod tests {
    ...
    #[test]
    fn agent_name_is_slugged_card_name_capped_at_64() {
        assert_eq!(agent_name("Remote Agent"), "remote-agent");
        assert_eq!(agent_name("!!!"), "agent");

        let long = agent_name(&"x".repeat(200));
