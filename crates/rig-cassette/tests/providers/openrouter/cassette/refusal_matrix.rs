...
async fn blocking_raw_and_normalized_agree() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            ...
            let normalized = model.complete(request).await.expect("the turn");
...
async fn blocking_refusal_finishes_with_stop() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            ...
            let response = model.complete(request).await.expect("refusal turn");
...
async fn control_answerable_prompt_is_unchanged_blocking() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            ...
            let response = model.complete(request).await.expect("answered turn");
...
async fn control_tool_call_turn_is_unchanged() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            ...
            let response = model.complete(request).await.expect("tool-call turn");
            assert!(
                response
                    .choice
                    .iter()
