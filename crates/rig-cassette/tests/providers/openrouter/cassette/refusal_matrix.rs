...
async fn blocking_raw_model_surfaces_refusal() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            let model = client.completion(REFUSING_MODEL).on(rig::transport());
...
async fn blocking_agent_prompt_surfaces_refusal() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            let agent =
                rig::AgentBuilder::new(client.completion(REFUSING_MODEL).on(rig::transport()))
                    .max_tokens(CAP)
                    .additional_params(refusal_request_params("OpenAI"))
                    .build();
...
async fn blocking_gpt_4_1_refusal() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            let model = client
                .completion(SECOND_REFUSING_MODEL)
                .on(rig::transport());
            let request = CompletionRequestBuilder::new(REFUSED_PROMPT)
                .max_tokens(CAP)
                .additional_params(refusal_request_params("OpenAI"))
...
async fn control_mini_answers_inside_schema() {
    ...
    with_openrouter_refusal_cassette(
        ...
        |client| async move {
            let model = client.completion(NON_REFUSING_MODEL).on(rig::transport());
            let request = CompletionRequestBuilder::new(REFUSED_PROMPT)
                .max_tokens(CAP)
                .additional_params(refusal_request_params("OpenAI"))
                .build();
