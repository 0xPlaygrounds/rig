...
async fn batch_multi_extract_chain() -> Result<()> {
    with_openrouter_cassette_result(
        ...
        |client| async move {
            let names_extractor = rig::extractor::ExtractorBuilder::<Names>::new(client.completion(DEFAULT_MODEL).on(rig::transport()))
                .append_preamble("Extract names from the given text.")
                .retries(2)
                .build();
            let topics_extractor = rig::extractor::ExtractorBuilder::<Topics>::new(client.completion(DEFAULT_MODEL).on(rig::transport()))
