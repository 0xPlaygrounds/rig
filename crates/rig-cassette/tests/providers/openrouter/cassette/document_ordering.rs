...
#[rig_test_support::cassette(rig_test_support::recording::Scenario::live(
    "openrouter/document_ordering/chat_completions_keeps_documents_after_system_before_history"
))]
#[tokio::test]
...
fn recorded_request_body(scenario: &str) -> Value {
    let cassette_path = crate::cassettes::cassette_path("openrouter", scenario);
...
fn assert_chat_request_order(scenario: &str) {
    ...
    let messages = body["messages"]
        .as_array()
        .expect("OpenRouter request should contain messages[]");
    assert_eq!(
        messages.len(),
        4,
