...
async fn permission_control_prompt_example() -> Result<()> {
    ...
    let agent = OpenAI::from_env_with(&GROQ)
        ...
        .endpoint(|provider_config| provider_config.completion(PERMISSION_CONTROL_PROMPT_MODEL))
        .into_agent_builder()
...
async fn permission_control_streaming_example() -> Result<()> {
    ...
    let agent = OpenAI::from_env_with(&GROQ)
        ...
        .bound()
        .expect("transport should build")
        .endpoint(|provider_config| provider_config.completion(PERMISSION_CONTROL_STREAMING_MODEL))
        .into_agent_builder()
        .preamble("You are a helpful assistant that can read files using different methods.")
        .tool(ReadFileHead)
        .tool(ReadFileTail)
