...
crate::matrix::native_matrix! {
    ...
    #[tokio::test]
    output_tool_unary: ("corpus_breadth/output_tool_unary", cells::OUTPUT_TOOL_UNARY);
    #[tokio::test]
...
async fn endings_tool_call_delta_stop() {
    with_gemini_cassette(
        ...
        |client| async move {
            run_world(&wire(&client), &cells::ENDINGS_TOOL_CALL_DELTA_STOP).await;
        },
    )
    .await;
}
...
async fn output_tool_thinking() {
    with_gemini_cassette("corpus_matrix/output_tool_thinking", |client| async move {
        run_world(&reasoning_wire(&client), &cells::OUTPUT_TOOL_THINKING).await;
    })
    .await;
}
...
crate::matrix::native_matrix! {
    ...
    #[tokio::test]
    reasoning_capped_streamed: ("reasoning_matrix/capped_streamed", cells::REASONING_CAPPED_STREAMED);
}
