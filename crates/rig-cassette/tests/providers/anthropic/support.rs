...
pub(super) struct AnthropicFilesCassette {
    pub(super) bound: Model<Anthropic>,
    pub(super) base_url: String,
    pub(super) api_key: String,
}

async fn anthropic_cassette(spec: impl Into<CassetteSpec>) -> (ProviderCassette, Model<Anthropic>) {
...
pub(super) async fn with_anthropic_boxed_cassette<F, Fut>(
    ...
    F: FnOnce(Model<Anthropic, BoxedHttpClient>) -> Fut,
    Fut: Future<Output = ()>,
{
    let cassette = ProviderCassette::start(
        &crate::cassettes::cassette_root(),
