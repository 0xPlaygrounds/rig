        ...
        with_anthropic_corpus_hooks_cassette(
            ...
            |client| async move {
                ...
                crate::goldens::world_golden_effects(
                    "anthropic_hooks_observe_everything_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
...
async fn patch_tool_args_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette("corpus_hooks/patch_tool_args", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects(
                "anthropic_hooks_patch_tool_args_effect_log",
                &log,
            );
            assert_eq!(
                families(&log),
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_hooks_patch_tool_args_streamed_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
...
async fn deny_tool_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette("corpus_hooks/deny_tool", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_hooks_deny_tool_effect_log", &log);
            assert_eq!(
                families(&log),
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_hooks_deny_tool_streamed_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_hooks_replace_tool_result_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
...
async fn replace_answer_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette("corpus_hooks/replace_answer", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_hooks_replace_answer_effect_log", &log);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_hooks_preamble_override_effect_log",
                    &log,
                );
                assert_eq!(families(&log), [EffectFamily::Completion]);
                assert_eq!(
...
async fn demand_done_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette("corpus_hooks/demand_done", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_hooks_demand_done_effect_log", &log);
            assert_eq!(
                families(&log),
                [EffectFamily::Completion, EffectFamily::Completion]
            );
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_hooks_lookup_before_run_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
...
async fn two_hooks_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_hooks_cassette("corpus_hooks/two_hooks", |client| async move {
            ...
            assert!(output.contains(REPLACED_RESULT), "{}", output);
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_hooks_two_hooks_effect_log", &log);
            assert_eq!(tool_record_args(&log), [r#"{"x":40,"y":2}"#]);
            assert_eq!(tool_record_outputs(&log), ["42"]);
            assert_eq!(tool_result_texts(request_at(&log, 2)), [REPLACED_RESULT]);
        })
