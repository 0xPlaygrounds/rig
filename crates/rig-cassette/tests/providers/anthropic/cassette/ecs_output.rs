...
async fn tool_unary_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette("corpus_output/tool_unary", |client| async move {
            ...
            crate::goldens::world_golden_effects("anthropic_output_tool_unary_effect_log", &log);
...
async fn tool_streamed_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette("corpus_output/tool_streamed", |client| async move {
            ...
            assert_event(&output);
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_output_tool_streamed_effect_log", &log);
            assert_eq!(families(&log), [EffectFamily::Completion]);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_output_tool_with_real_tool_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_output_tool_choice_specific_output_effect_log",
                    &log,
                );
                assert_eq!(families(&log), [EffectFamily::Completion]);
                assert_eq!(tool_names(request_at(&log, 0)), ["final_result"]);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_output_tool_choice_required_effect_log",
                    &log,
                );
                assert_eq!(families(&log), [EffectFamily::Completion]);
            },
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_output_tool_under_none_degrades_effect_log",
                    &log,
                );
                assert_eq!(families(&log), [EffectFamily::Completion]);
                let request = request_at(&log, 0);
...
async fn tool_thinking_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_output_cassette("corpus_output/tool_thinking", |client| async move {
            ...
            assert_event(&output);
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_output_tool_thinking_effect_log", &log);
            assert_eq!(families(&log), [EffectFamily::Completion]);
        })
        .await;
    })
