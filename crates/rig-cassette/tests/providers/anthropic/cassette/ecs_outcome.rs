    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_outcome_cassette(
            ...
            |client| async move {
                let mut ecs =
                    EcsAgent::for_golden(
                        delivery::FirstDelta::tool(client.endpoint(|provider_config| {
                            provider_config.completion(CLAUDE_SONNET_4_6)
                        })),
                        NOTE_PREAMBLE,
                        true,
                    );
                ...
                crate::goldens::world_golden_effects(
                    "anthropic_outcome_cancel_after_tool_call_delta_effect_log",
                    &log,
                );
...
async fn tool_error_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_outcome_cassette("corpus_outcome/tool_error", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_outcome_tool_error_effect_log", &log);
            assert_eq!(
                families(&log),
                [
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_outcome_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_outcome_tool_error_streamed_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
...
async fn model_error_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_cassette_bogus_key("corpus_outcome/model_error", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_outcome_model_error_effect_log", &log);
            assert_eq!(families(&log), [EffectFamily::Completion]);
            let report = log.records[0]
                .outcome
                .as_ref()
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_cassette_bogus_key(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_outcome_model_error_streamed_effect_log",
                    &log,
                );
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_outcome_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_outcome_max_turns_exhausted_effect_log",
                    &log,
                );
                assert_eq!(
                    families(&log),
...
async fn default_max_turns_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_cassette("effect_corpus/tool_call_turn", |client| async move {
            ...
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects(
                "anthropic_outcome_default_max_turns_effect_log",
                &log,
            );
            assert_eq!(
                families(&log),
