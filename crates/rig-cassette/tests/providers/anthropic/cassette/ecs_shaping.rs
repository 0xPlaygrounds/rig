        ...
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_tool_choice_required_first_effect_log",
                    &log,
                );
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_tool_choice_none_on_committed_output_effect_log",
                    &log,
                );
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_extra_context_effect_log",
                    &log,
                );
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_extra_context_streamed_effect_log",
                    &log,
                );
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_merged_three_effect_log",
                    &log,
                );
                assert_eq!(families(&log), TOOL_TURN);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                        ...
                        RuntimeHandler {
                            inner: std::sync::Arc::new(
                                rig_core::serve::adapters::ModelAdapter::completion(
                                    "fast",
                                    client
                                        .endpoint(|provider| provider.completion(CLAUDE_HAIKU_4_5)),
                                ),
                            ),
                            runtime: io_runtime(),
                        },
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_route_on_first_turn_effect_log",
                    &log,
                );
                assert_eq!(families(&log), TOOL_TURN);
...
async fn late_route_effect_log() {
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette("corpus_shaping/late_route", |client| async move {
            ...
            assert!(output.contains("42"), "{output}");
            let log = ecs.effect_log();
            crate::goldens::world_golden_effects("anthropic_shaping_late_route_effect_log", &log);
            assert_eq!(families(&log), TOOL_TURN);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_max_tokens_second_turn_effect_log",
                    &log,
                );
                assert_eq!(families(&log), TOOL_TURN);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_thinking_second_turn_effect_log",
                    &log,
                );
                assert_eq!(families(&log), TOOL_TURN);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_preamble_second_turn_effect_log",
                    &log,
                );
                assert_eq!(families(&log), TOOL_TURN);
                assert!(
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_active_tools_none_second_turn_effect_log",
                    &log,
                );
                assert_eq!(families(&log), TOOL_TURN);
    ...
    crate::goldens::capture_world_programs(async {
        with_anthropic_corpus_shaping_cassette(
            ...
            |client| async move {
                ...
                let log = ecs.effect_log();
                crate::goldens::world_golden_effects(
                    "anthropic_shaping_history_first_turn_effect_log",
                    &log,
                );
                assert_eq!(families(&log), [EffectFamily::Completion]);
                assert!(
