//! Native anthropic lifecycle counterparts with shared original transport/assertion helpers.
    ...
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "anthropic_lifecycle_middleware_phases_observe_a_unary_completion",
                log,
            )
        },
    )
...
async fn middleware_response_phase_precedes_stream_consumption() {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "anthropic_lifecycle_middleware_response_phase_precedes_stream_consumption",
                log,
            )
        },
    )
...
async fn run_start_rewrite_reaches_the_provider() {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "anthropic_lifecycle_run_start_rewrite_reaches_the_provider",
                log,
            )
        },
    )
...
async fn entry_log_orders_and_turn_stamps_across_a_streamed_tool_run() {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "anthropic_lifecycle_entry_log_orders_and_turn_stamps_across_a_streamed_tool_run",
                log,
            )
        },
    )
...
}, |log| rig_test_support::goldens::world_golden_effects("anthropic_lifecycle_run_settles_once_across_a_multi_turn_tool_run_with_durable_state", log)).await
}
