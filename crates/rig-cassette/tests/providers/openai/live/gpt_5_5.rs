...
async fn responses_structured_output_smoke() {
    let client = OpenAI::from_env().expect("config should build from env");
    let agent =
        rig::AgentBuilder::new(client.completion(openai::GPT_5_5).on(rig::transport())).build();
...
async fn responses_extractor_smoke() {
    ...
    let extractor = rig::extractor::ExtractorBuilder::<SmokePerson>::new(
        client.completion(openai::GPT_5_5).on(rig::transport()),
    )
    .build();
...
async fn responses_websocket_smoke() -> anyhow::Result<()> {
    let client = OpenAI::from_env().expect("config should build from env");
    let model = client.responses(openai::GPT_5_5).on(rig::transport());
