...
async fn batch_multi_extract_chain() -> Result<()> {
    with_openai_cassette_result(
        ...
        |cassette| async move {
            ...
            let names_extractor =
                client.endpoint(|provider| provider.completion(openai::GPT_4O_MINI)).into_extractor_builder::<Names>()
                    .append_preamble("Extract names from the given text.")
                    .retries(2)
                    .build();
            let topics_extractor =
                client.endpoint(|provider| provider.completion(openai::GPT_4O_MINI)).into_extractor_builder::<Topics>()
                    .append_preamble("Extract topics from the given text.")
                    .retries(2)
                    .build();
            let sentiment_extractor =
                client.endpoint(|provider| provider.completion(openai::GPT_4O_MINI)).into_extractor_builder::<Sentiment>()
