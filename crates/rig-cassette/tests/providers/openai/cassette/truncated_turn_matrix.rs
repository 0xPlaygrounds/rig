...
async fn chat_blocking_gpt_5_1_budget_exhausted() {
    ...
    with_openai_truncation_cassette(
        ...
        |client| async move {
            let model = client.openai.chat("gpt-5.1").on(rig::transport());
...
async fn cross_surface_truncation_parity() {
    ...
    with_openai_truncation_cassette(
        ...
        |client| async move {
            let responses_model = client.openai.completion("gpt-5-nano").on(rig::transport());
            ...
            let chat_model = client.openai.chat("gpt-5-nano").on(rig::transport());
            let chat = chat_model
                .call(
                    CompletionRequestBuilder::new(LONG_PROMPT)
                        .max_tokens(TINY_CAP)
