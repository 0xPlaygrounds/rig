...
async fn chat_blocking_usage_survives_the_empty_turn() {
    ...
    with_openai_truncation_cassette(
        ...
        |client| async move {
            ...
            assert!(response.end.meta.usage.input_tokens.is_some_and(|n| n > 0));
            assert_eq!(response.end.meta.usage.output_tokens, Some(TINY_CAP));
            assert!(
                response
                    .end
                    .meta
...
async fn chat_blocking_raw_and_normalized_agree() {
    ...
    with_openai_truncation_cassette(
        ...
        |client| async move {
            ...
            let reply = ChatCompletionResponse::deserialize(&response.end.meta.raw)
...
async fn cross_surface_truncation_parity() {
    ...
    with_openai_truncation_cassette(
        ...
        |client| async move {
            ...
            assert_eq!(
                responses.end.finish_reason.clone(),
                Some(FinishReason::Length)
            );
            assert_eq!(
                chat.end.finish_reason.clone(),
                responses.end.finish_reason.clone()
            );
        },
    )
