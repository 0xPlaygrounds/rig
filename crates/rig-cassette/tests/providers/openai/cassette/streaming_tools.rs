...
async fn example_streaming_with_tools() {
    with_openai_cassette("streaming_tools/example_streaming_with_tools", |client| async move {
        let agent = rig::AgentBuilder::new(client.openai.completion(openai::GPT_4O).on(rig::transport()))
            .preamble(
...
async fn raw_responses_stream_preserves_tool_then_followup_text_ordering() {
    with_openai_cassette(
        ...
        |client| async move {
            let model = client.openai.completion(openai::GPT_4O).on(rig::transport());
