...
async fn the_native_width_comes_back_when_none_is_declared() {
    with_llamacpp_embeddings_cassette("embedding_matrix/native_width", |client| async move {
        let model = client.endpoint(|provider| provider.embeddings(CASSETTE_EMBEDDING_MODEL, None));
    ...
    with_llamacpp_embeddings_cassette(
        ...
        |client| async move {
            let model = client.endpoint(|provider| {
                provider.embeddings(CASSETTE_EMBEDDING_MODEL, Some(NATIVE_WIDTH))
            });
...
async fn a_declared_width_llamacpp_cannot_honour_is_refused() {
    with_llamacpp_embeddings_cassette(
        ...
        |client| async move {
            let model = client
                .endpoint(|provider| provider.embeddings(CASSETTE_EMBEDDING_MODEL, Some(128)));
            let error = model
                .embed_texts(["hello".to_string()])
                .await
