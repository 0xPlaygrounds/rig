...
async fn an_unknown_model_is_ignored_rather_than_rejected() {
    with_llamacpp_cassette(
        ...
        |client| async move {
            let model = client
                .endpoint(|provider| provider.completion("rig/definitely-not-a-llamacpp-model"));
            let response = model
                .complete(
                    model
...
async fn embeddings_without_the_flag_are_a_501() {
    with_llamacpp_cassette(
        ...
        |client| async move {
            let error = client
                .endpoint(|provider| provider.embeddings(CASSETTE_EMBEDDING_MODEL, None))
    ...
    with_llamacpp_causal_embeddings_cassette(
        ...
        |client| async move {
            let embeddings = client
                .endpoint(|provider| provider.embeddings(CASSETTE_MODEL, None))
                .embed_texts(["hello".to_string()])
                .await
...
async fn rerank_without_a_reranker_is_a_501() {
    with_llamacpp_cassette(
        ...
        |client| async move {
            let error = client
                .endpoint(|provider| provider.reranker(CASSETTE_RERANK_MODEL))
