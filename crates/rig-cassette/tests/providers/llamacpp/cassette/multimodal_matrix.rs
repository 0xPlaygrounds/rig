    ...
    with_llamacpp_large_vision_cassette(
        ...
        |client| async move {
            let model = client.endpoint(|provider_config| {
                provider_config.completion(CASSETTE_LARGE_VISION_MODEL)
            });
...
async fn an_image_and_a_tool_reach_the_model_together() {
    with_llamacpp_vision_cassette("multimodal_matrix/image_plus_tools", |client| async move {
        let model =
            client.endpoint(|provider_config| provider_config.completion(CASSETTE_VISION_MODEL));
        let response = model
            .complete(
                model
...
async fn a_malformed_data_uri_is_a_400() {
    with_llamacpp_vision_cassette(
        ...
        |client| async move {
            let model = client
                .endpoint(|provider_config| provider_config.completion(CASSETTE_VISION_MODEL));
            let error = model
                .complete(
                    model
                        .completion_request(Message::User {
