...
async fn llamacpp_cassette_on(
    ...
    let cassette = crate::cassettes::start_provider_cassette("llamacpp", spec, upstream).await;
...
{
    let cassette =
        crate::cassettes::start_provider_cassette("llamacpp", spec, &record_upstream()).await;
