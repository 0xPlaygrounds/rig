...
async fn sequential_complex_tool_calls_streaming() -> Result<()> {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "xai_tool_sessions_sequential_complex_tool_calls_streaming",
                log,
            )
        },
    )
...
async fn parallel_tool_calls_single_turn_streaming() -> Result<()> {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "xai_tool_sessions_parallel_tool_calls_single_turn_streaming",
                log,
            )
        },
    )
...
async fn multimodal_image_input_mixed_text_ordering() -> Result<()> {
    rig_test_support::goldens::world_golden_test(
        ...
        |client| async move {
            let mut agent = EcsAgent::new(
                client.endpoint(|provider| provider.completion(VISION_MODEL)),
    ...
    .await
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "xai_tool_sessions_multimodal_image_input_mixed_text_ordering",
                log,
            )
        },
    )
