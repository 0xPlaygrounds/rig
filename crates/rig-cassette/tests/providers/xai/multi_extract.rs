...
async fn batch_multi_extract_chain() -> Result<()> {
    with_xai_cassette_result(
        ...
        |client| async move {
            let names_extractor = client.extractor::<Names>(xai::GROK_3_MINI)
                .append_preamble("Extract names from the given text.")
                .retries(2)
                .build();
            let topics_extractor = client.extractor::<Topics>(xai::GROK_3_MINI)
                .append_preamble("Extract topics from the given text.")
                .retries(2)
                .build();
            let sentiment_extractor = client.extractor::<Sentiment>(xai::GROK_3_MINI)
                .append_preamble(
