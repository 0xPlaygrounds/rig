...
async fn chat_models_route_through_chat_completions() {
    let response = rig::AgentBuilder::new(rig::model(live_client().await.completion(LIVE_MODEL)))
...
async fn codex_models_route_through_responses() {
    with_copilot_cassette(
        ...
        |client| async move {
            let response =
                rig::AgentBuilder::new(rig::model(client.completion(live_responses_model())))
                    .preamble(BASIC_PREAMBLE)
                    .build()
