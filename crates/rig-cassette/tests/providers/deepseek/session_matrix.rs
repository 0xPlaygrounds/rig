...
fn models(
    ...
    (
        client.endpoint(|provider| provider.completion("deepseek-v4-flash")),
        client.endpoint(|provider| provider.completion("deepseek-v4-flash")),
        client.endpoint(|provider| provider.completion("deepseek-v4-pro")),
    )
}
...
async fn same_model() {
    ...
    with_deepseek_cassette("session_matrix/same_model", |client| async move {
...
async fn other_model() {
    ...
    with_deepseek_cassette("session_matrix/other_model", |client| async move {
...
async fn memory_unary() {
    ...
    with_deepseek_cassette("session_matrix/memory_unary", |client| async move {
...
async fn memory_streamed() {
    ...
    with_deepseek_cassette("session_matrix/memory_streamed", |client| async move {
        let (first, _, _) = models(client);
