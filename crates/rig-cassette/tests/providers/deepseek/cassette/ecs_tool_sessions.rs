    ...
    rig_test_support::goldens::world_golden_test(
                ...
                |client| async move {
                    ...
                    let mut agent = {
                        ...
                        ecs.app.world_mut().entity_mut(ecs.agent).insert((
                            ...
                            rig_ecs::agent::AdditionalParams(Some(json_utils_merge(
                                non_thinking_params(),
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "deepseek_tool_sessions_sequential_complex_tool_calls_nonstreaming",
                log,
            )
        },
    )
...
async fn sequential_complex_tool_calls_streaming() -> Result<()> {
    rig_test_support::goldens::world_golden_test(
        async {
            with_deepseek_cassette_result(
                ...
                |client| async move {
                    ...
                    let response = observation.final_response_text.as_deref().ok_or_else(|| {
                        anyhow::anyhow!("stream should produce final response text")
                    })?;
                    assert_contains_all_case_insensitive(
                        response,
                        &["EMPTY-OK", "MANIFEST-OK", "LABELS-OK", "ESCAPE-OK"],
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "deepseek_tool_sessions_sequential_complex_tool_calls_streaming",
                log,
            )
        },
    )
...
async fn parallel_tool_calls_single_turn_nonstreaming() -> Result<()> {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "deepseek_tool_sessions_parallel_tool_calls_single_turn_nonstreaming",
                log,
            )
        },
    )
...
async fn parallel_tool_calls_single_turn_streaming() -> Result<()> {
    rig_test_support::goldens::world_golden_test(
        ...
        |log| {
            rig_test_support::goldens::world_golden_effects(
                "deepseek_tool_sessions_parallel_tool_calls_single_turn_streaming",
                log,
            )
        },
    )
