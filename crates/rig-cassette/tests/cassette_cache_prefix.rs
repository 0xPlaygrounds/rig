...
fn determinism_probe_request() -> CompletionRequest {
    ...
    CompletionRequest {
        system: Some("You are a deterministic serialization probe.".into()),
        messages: rig_core::non_empty::NonEmpty::new(Message::User {
            content: rig_core::non_empty::NonEmpty::new(UserContent::text("probe")),
        }),
...
fn every_provider_suite_has_a_cache_suite() {
    ...
    // The provider suites have two homes: the cassette-backed ones are
    // targets of this package, the live-only ones stayed in the facade. The
    // census is over both, so a provider cannot escape it by which package
    // its suite lives in.
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let provider_roots = [
        manifest.join("tests/providers"),
        manifest.join("../../tests/providers"),
    ];
    let cassette_root = cassette_root();

    let mut providers: Vec<String> = Vec::new();
    for providers_root in &provider_roots {
        assert!(
