...
#[cfg(test)]
mod tests {
    use crate::types::assistent_content::RigAssistantContent;
    ...
    #[test]
    fn aws_converse_output_to_completion_reponse() {
    ...
    #[test]
    fn aws_content_block_to_assistent_content() {
        let content_block = aws_bedrock::ContentBlock::Text("text".into());
        let rig_assistent_content: Result<RigAssistantContent, _> = content_block.try_into();
        assert_eq!(rig_assistent_content.is_ok(), true);
        assert_eq!(
            rig_assistent_content.unwrap().0,
