...
//! and waits for a decision on stdin. Each decision maps to a
//! [`ToolCallAction`] — no special HITL machinery is required:
//!
//! | Human decision | Action | Effect |
//! |---|---|---|
//! | **approve** | [`ToolCallAction::Run`] | the tool executes as requested |
//! | **deny** | [`ToolCallAction::Skip`] | the tool does not run; the reason becomes model-visible feedback |
//! | **edit** | [`ToolCallAction::Rewrite`] | the tool executes with human-supplied arguments |
//! | **abort** | [`ToolCallAction::Stop`] | the run stops and surfaces the reason as an error |
//!
...
use anyhow::Result;
use rig::agent::{AgentHook, HookContext, ToolCallAction, ToolCallEvent};
...
impl<M: CompletionModel> AgentHook<M> for ApprovalHook {
    async fn on_tool_call(&self, _ctx: &HookContext, event: ToolCallEvent<'_>) -> ToolCallAction {
        ...
        let Some(choice) = ask("     [a]pprove / [d]eny / [e]dit args / a[b]ort run? ").await
            ...
            return rig::agent::ToolCallAction::stop("no reviewer input available (stdin closed)");
        };
        ...
        match choice.to_ascii_lowercase().as_str() {
            ...
            "d" | "deny" | "n" | "no" => {
                ...
                println!("     → denied");
                rig::agent::ToolCallAction::skip(reason)
            }
            ...
            "e" | "edit" => {
                    ...
                    .map(serde_json::from_str::<serde_json::Value>)
                {
                    Some(Ok(value)) => {
                        ...
                        rig::agent::ToolCallAction::rewrite(value)
                    }
                    other => {
                        println!("     ! no valid JSON ({other:?}); denying instead");
                        rig::agent::ToolCallAction::skip(
