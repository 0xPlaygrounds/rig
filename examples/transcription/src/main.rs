...
async fn whisper(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let whisper = openai.endpoint(|provider| provider.transcriptions(openai::WHISPER_1));
...
async fn gemini(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let model = gemini
        .endpoint(|provider| provider.transcriptions(gemini::completion::GEMINI_3_FLASH_PREVIEW));
...
async fn azure(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let whisper = azure.endpoint(|provider| provider.transcriptions("whisper"));
    let response = TranscriptionRequestBuilder::from_file(whisper, file_path)?
        .send()
        .await?;
...
async fn groq(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let whisper = groq.endpoint(|provider| provider.transcriptions(groq::WHISPER_LARGE_V3));
    let response = TranscriptionRequestBuilder::from_file(whisper, file_path)?
        .send()
        .await?;
...
async fn huggingface(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let whisper = huggingface.endpoint(|provider| provider.transcriptions("whisper-large-v3"));
    let response = TranscriptionRequestBuilder::from_file(whisper, file_path)?
        .send()
        .await?;
