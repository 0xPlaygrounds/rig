...
async fn whisper(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let whisper = openai.transcription(openai::WHISPER_1).on(rig::transport());
...
async fn gemini(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let model = gemini
        .transcription(gemini::completion::GEMINI_3_FLASH_PREVIEW)
        .on(rig::transport());
    let response = model
        .call(TranscriptionRequestBuilder::from_file(file_path)?.build())
        .await?;
...
async fn azure(file_path: &str) -> Result<(), anyhow::Error> {
    ...
    let whisper = azure.transcription("whisper").on(rig::transport());
    let response = whisper
        .call(TranscriptionRequestBuilder::from_file(file_path)?.build())
        .await?;
