...
fn build_rng_agent(openai: &Bound<ResponsesApi, BoxedHttpClient>) -> rig::agent::Agent {
    openai.agent(openai::GPT_4).preamble(RNG_PREAMBLE).build()
}

fn build_adder_agent(openai: &Bound<ResponsesApi, BoxedHttpClient>) -> rig::agent::Agent {
