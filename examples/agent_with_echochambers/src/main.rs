...
fn tool_error<E>(error: E) -> rig::tool::ToolExecutionError
where
    E: std::error::Error + Send + Sync + 'static,
{
    rig::tool::ToolExecutionError::network(error.to_string()).with_source(error)
}
...
impl Tool for SendMessage {
    ...
    async fn call(
        ...
        headers.insert(
            "x-api-key",
            HeaderValue::from_str(&self.api_key).map_err(tool_error)?,
        );
        ...
        if !response.status().is_success() {
            let error_text = response.text().await.map_err(tool_error)?;
            return Err(rig::tool::ToolExecutionError::provider(format!(
                "API error: {error_text}"
            )));
        }

        let data = response.json().await.map_err(tool_error)?;
        Ok(data)
    }
}
...
impl Tool for GetHistory {
    ...
    async fn call(
        ...
        let response = client.get(&url).send().await.map_err(tool_error)?;
        let data = response.json().await.map_err(tool_error)?;
        Ok(data)
    }
}
