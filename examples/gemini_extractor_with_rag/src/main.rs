...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let embedding_model =
        gemini_client.endpoint(|provider| provider.embeddings(gemini::EMBEDDING_001, None));
    ...
    let rag_extractor = gemini_client.endpoint(|provider| provider.completion("gemini-2.5-flash")).into_extractor_builder::<QuestionnaireResponses>()
        .append_preamble("
